...
@NamedQueries( { @NamedQuery(name = "ArtikelzuschlagfindByArtikelIIdTGueltigab", query = "SELECT OBJECT(C) FROM Artikelzuschlag c WHERE c.artikelIId = ?1 AND c.tGueltigab = ?2") })
@Entity
@Table(name = "PERS_ARTIKELZUSCHLAG")
public class Artikelzuschlag implements Serializable {
	@Id
	@Column(name = "I_ID")
	private Integer iId;
	...
	public Artikelzuschlag() {
		super();
	}

	public Artikelzuschlag(Integer id, Integer artikelIId, BigDecimal nZuschlag,
			Timestamp gueltigab) {
		setIId(id);

/*******************************************************************************
 * HELIUM V, Open Source ERP software for sustained success
 * at small and medium-sized enterprises.
 * Copyright (C) 2004 - 2015 HELIUM V IT-Solutions GmbH
 * 
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published 
 * by the Free Software Foundation, either version 3 of theLicense, or 
 * (at your option) any later version.
 * 
 * According to sec. 7 of the GNU Affero General Public License, version 3, 
 * the terms of the AGPL are supplemented with the following terms:
 * 
 * "HELIUM V" and "HELIUM 5" are registered trademarks of 
 * HELIUM V IT-Solutions GmbH. The licensing of the program under the 
 * AGPL does not imply a trademark license. Therefore any rights, title and
 * interest in our trademarks remain entirely with us. If you want to propagate
 * modified versions of the Program under the name "HELIUM V" or "HELIUM 5",
 * you may only do so if you have a written permission by HELIUM V IT-Solutions 
 * GmbH (to acquire a permission please contact HELIUM V IT-Solutions
 * at trademark@heliumv.com).
 * 
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 * 
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 * 
 * Contact: developers@heliumv.com
 ******************************************************************************/
package com.lp.server.personal.service;

import java.io.Serializable;
import java.math.BigDecimal;

import com.lp.server.artikel.service.ArtgruDto;
import com.lp.server.artikel.service.ArtklaDto;

public class ZeileArbeitszeitstatistikDto implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	private String cArtikelnummer;

	public String getCArtikelnummer() {
		return cArtikelnummer;
	}

	private BigDecimal bdDauer;

	public String getArtikelbzeichnung() {
		return artikelbzeichnung;
	}

	public BigDecimal getBdKosten() {
		return bdKosten;
	}

	public BigDecimal getBdDauer() {
		return bdDauer;
	}

	public ArtklaDto getArtikelklasseDto() {
		return artikelklasseDto;
	}

	public ArtgruDto getArtikelgruppeDto() {
		return artikelgruppeDto;
	}

	public Integer getArtikelIId() {
		return artikelIId;
	}

	public Integer getPersonalIId() {
		return personalIId;
	}

	public String getSPerson() {
		return sPerson;
	}

	public String getSBelegnummer() {
		return sBelegnummer;
	}

	private BigDecimal bdKosten;

	private String artikelbzeichnung;
	private ArtklaDto artikelklasseDto;
	private ArtgruDto artikelgruppeDto;
	private Integer artikelIId;
	private Integer personalIId;
	private String sPerson;
	private String sBelegnummer;

	public void setCArtikelnummer(String cArtikelnummer) {
		this.cArtikelnummer = cArtikelnummer;
	}

	public void setArtikelbzeichnung(String artikelbzeichnung) {
		this.artikelbzeichnung = artikelbzeichnung;
	}

	public void setBdKosten(BigDecimal bdKosten) {
		this.bdKosten = bdKosten;
	}

	public void setBdDauer(BigDecimal bdDauer) {
		this.bdDauer = bdDauer;
	}

	public void setArtikelklasseDto(ArtklaDto artikelklasseDto) {
		this.artikelklasseDto = artikelklasseDto;
	}

	public void setArtikelgruppeDto(ArtgruDto artikelgruppeDto) {
		this.artikelgruppeDto = artikelgruppeDto;
	}

	public void setArtikelIId(Integer artikelIId) {
		this.artikelIId = artikelIId;
	}

	public void setPersonalIId(Integer personalIId) {
		this.personalIId = personalIId;
	}

	public void setSPerson(String sPerson) {
		this.sPerson = sPerson;
	}

	public void setSBelegnummer(String sBelegnummer) {
		this.sBelegnummer = sBelegnummer;
	}

}

...
public class ZulageDtoAssembler {
	public static ZulageDto createDto(Zulage zulage) {
		ZulageDto zulageDto = new ZulageDto();
		if (zulage != null) {
			zulageDto.setIId(zulage.getIId());
			zulageDto.setCBez(zulage.getCBez());
		}
		return zulageDto;
	}

	public static ZulageDto[] createDtos(Collection<?> zulages) {
		List<ZulageDto> list = new ArrayList<ZulageDto>();
		if (zulages != null) {
			Iterator<?> iterator = zulages.iterator();
			while (iterator.hasNext()) {
				list.add(createDto((Zulage) iterator.next()));
			}
		}

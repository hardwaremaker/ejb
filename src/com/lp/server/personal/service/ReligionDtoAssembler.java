...
public class ReligionDtoAssembler {
	public static ReligionDto createDto(Religion religion) {
		ReligionDto religionDto = new ReligionDto();
		if (religion != null) {
			religionDto.setIId(religion.getIId());
			religionDto.setCNr(religion.getCNr());
		}
		return religionDto;
	}

	public static ReligionDto[] createDtos(Collection<?> religions) {
		List<ReligionDto> list = new ArrayList<ReligionDto>();
		if (religions != null) {
			Iterator<?> iterator = religions.iterator();
			while (iterator.hasNext()) {
				list.add(createDto((Religion) iterator.next()));
			}
		}

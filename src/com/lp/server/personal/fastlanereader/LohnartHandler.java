 ...
 * @version 1.0
 */
public class LohnartHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRLohnart artikelkommentarart = (FLRLohnart) resultListIterator
						.next();

				rows[row][col++] = artikelkommentarart.getI_id();
				rows[row][col++] = artikelkommentarart.getI_lohnart();
				rows[row][col++] = artikelkommentarart.getC_bez();
				rows[row][col++] = artikelkommentarart.getPersonalart_c_nr();
				row++;
				col = 0;
			}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRLohnart AS lohnart"
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" lohnart." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("lohnart.i_lohnart ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("lohnart.i_id") < 0) {
	...
	private String getFromClause() {
		return " FROM FLRLohnart AS lohnart";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRLohnart artikelkommentarart = (FLRLohnart) scrollableResult
								.get(0);

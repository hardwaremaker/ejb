 ...
 * @version 1.0
 */
public class NachrichtenartHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRNachrichtenart nachrichtenart = (FLRNachrichtenart) o[0];

				Iterator<?> sprsetIterator = nachrichtenart
						.getNachrichtenartsprset().iterator();
				...
				while (sprsetIterator.hasNext()) {
					FLRNachrichtenartspr nachrichtenartspr = (FLRNachrichtenartspr) sprsetIterator
							.next();
					if (nachrichtenartspr.getLocale().getC_nr()
							.compareTo(sLocUI) == 0) {
						spr = nachrichtenartspr.getC_bez();
						break;
					}
				}
				rows[row][col++] = spr;

				rows[row][col++] =Helper.short2Boolean(nachrichtenart.getB_muss_erledigt_werden());
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRNachrichtenart AS nachrichtenart"
					+ " LEFT JOIN nachrichtenart.nachrichtenartsprset AS nachrichtenartsprset "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
	...
	private String buildOrderByClause() {
		...
		if (getQuery() != null) {
			...
			if (orderBy.indexOf("c_nr") < 0) {
				...
				orderBy.append(" nachrichtenart.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
				orderBy.insert(0, " ORDER BY ");
	...
	private String getFromClause() {
		return "FROM FLRNachrichtenart AS nachrichtenart"
				+ " LEFT JOIN nachrichtenart.nachrichtenartsprset AS nachrichtenartsprset";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRNachrichtenart zeitmodell = (FLRNachrichtenart) scrollableResult
								.get(0);
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			, new String[] { "i_id", "c_nr", "nachrichtenartsprset.c_bez",
					"b_muss_erledigt_werden" }));

 ...
 * @version 1.0
 */
public class PersonalgruppeHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRPersonalgruppe maschinengruppe = (FLRPersonalgruppe) resultListIterator
						.next();
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(personalgruppe."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" personalgruppe."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("personalgruppe.c_bez ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("personalgruppe.c_bez") < 0) {
				...
				orderBy.append(" personalgruppe.c_bez ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRPersonalgruppe personalgruppe ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select personalgruppe.i_id from FLRPersonalgruppe personalgruppe "

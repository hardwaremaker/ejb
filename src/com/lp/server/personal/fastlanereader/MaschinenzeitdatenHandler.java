 ...
 * @version 1.0
 */
public class MaschinenzeitdatenHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = Helper.formatDatum(zeitdaten.getT_von(),
						theClientDto.getLocUi())
						+ " "
						+ new java.sql.Time(zeitdaten.getT_von().getTime())
								.toString();
				if (zeitdaten.getT_bis() != null) {
					rows[row][col++] = Helper.formatDatum(zeitdaten.getT_bis(),
							theClientDto.getLocUi())
							+ " "
							+ new java.sql.Time(zeitdaten.getT_bis().getTime())
									.toString();
				} else {
					rows[row][col++] = null;
				}
				...
				if (zeitdaten.getT_bis() != null) {
					...
					Double d=((double)l_zeitdec)/(3600000);
					rows[row][col++] = d;
				} else {
					rows[row][col++] = null;
				}
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].kritName
							.equals("flrlossollarbeitsplan.flrlos."
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (orderBy.indexOf("maschinenzeitdaten."
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							ZeiterfassungFac.FLR_MASCHINENZEITDATEN_FLRLOSSOLLARBEITSPLAN
									+ ".flrlos.c_nr",

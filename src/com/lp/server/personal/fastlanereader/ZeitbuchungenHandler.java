 ...
 * @version 1.0
 */
public class ZeitbuchungenHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = Helper.formatDatumZeit(zeitdaten.getT_zeit(),theClientDto.getLocUi());

				rows[row][col++] = zeitdaten.getFlrpersonal().getC_kurzzeichen();
				rows[row][col++] = zeitdaten.getC_bemerkungzubelegart();
				rows[row][col++] = zeitdaten.getX_kommentar();
				rows[row][col++] = zeitdaten.getX_kommentar_intern();
	...
	public TableInfo getTableInfo() {
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] { "Id", getTextRespectUISpr("lp.zeitpunkt", mandantCNr, locUI),
							getTextRespectUISpr("pers.kurzzeichen", mandantCNr, locUI),
							getTextRespectUISpr("lp.bemerkung", mandantCNr, locUI),
							getTextRespectUISpr("proj.kommentar.extern", mandantCNr, locUI),
							getTextRespectUISpr("proj.kommentar.intern", mandantCNr, locUI), },
					...
					new String[] { "i_id",
							...
							ZeiterfassungFac.FLR_ZEITDATEN_FLRPERSONAL + ".c_kurzzeichen", Facade.NICHT_SORTIERBAR,

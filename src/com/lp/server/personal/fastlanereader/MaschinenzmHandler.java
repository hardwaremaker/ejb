...
import com.lp.server.personal.fastlanereader.generated.FLRMaschinenzm;
 ...
 * @version 1.0
 */
public class MaschinenzmHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRMaschinenzm zeitmodell = (FLRMaschinenzm) resultListIterator.next();

				rows[row][col++] = zeitmodell.getI_id();
				rows[row][col++] = zeitmodell.getC_bez();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRMaschinenzm AS maschinenzm "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("maschinenzm." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("maschinenzm.c_bez ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("maschinenzm.c_bez") < 0) {
				...
				orderBy.append(" maschinenzm.c_bez ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRMaschinenzm AS maschinenzm ";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRMaschinenzm zeitmodell = (FLRMaschinenzm) scrollableResult
								.get(0);

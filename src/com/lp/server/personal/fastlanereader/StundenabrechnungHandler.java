...
public class StundenabrechnungHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRStundenabrechnung stundenabrechnung = (FLRStundenabrechnung) resultListIterator
						.next();
				...
				rows[row][col++] = stundenabrechnung.getN_uestpflichtig50();
				rows[row][col++] = stundenabrechnung.getN_uestfrei100();
				rows[row][col++] = stundenabrechnung.getN_uestpflichtig100();
				rows[row][col++] = stundenabrechnung.getN_uest200();
				rows[row][col++] = stundenabrechnung.getN_gutstunden();
				rows[row++][col++] = stundenabrechnung
						.getN_qualifikationspraemie();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" stundenabrechnung."
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("stundenabrechnung."
				...
				orderBy.append("stundenabrechnung."
						+ PersonalFac.FLR_STUNDENABRECHNUNG_D_DATUM + " DESC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("stundenabrechnung."
					+ PersonalFac.FLR_STUNDENABRECHNUNG_D_DATUM) < 0) {
	...
	private String getFromClause() {
		return "from FLRStundenabrechnung stundenabrechnung ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select stundenabrechnung.i_id from FLRStundenabrechnung stundenabrechnung "
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							getTextRespectUISpr("pers.gleitzeitsaldo.200", mandantCNr, locUI),

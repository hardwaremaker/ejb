...
import com.lp.server.personal.fastlanereader.generated.FLRPersonalgruppekosten;
 ...
 * @version 1.0
 */
public class PersonalgruppekostenHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = MaschinenkostenHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				FLRPersonalgruppekosten flrmaschinenkosten = (FLRPersonalgruppekosten) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRPersonalgruppekosten AS personalgruppekosten "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(personalgruppekosten."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" personalgruppekosten."
								+ filterKriterien[i].kritName);
					}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("personalgruppekosten."
				...
				orderBy.append("personalgruppekosten.t_gueltigab ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("personalgruppekosten.t_gueltigab") < 0) {
				...
				orderBy.append(" personalgruppekosten.t_gueltigab ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRPersonalgruppekosten AS personalgruppekosten";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRPersonalgruppekosten maschine = (FLRPersonalgruppekosten) scrollableResult
								.get(0);

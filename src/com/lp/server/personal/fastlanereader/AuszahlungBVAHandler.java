...
import com.lp.server.personal.fastlanereader.generated.FLRAuszahlungBVA;
...
public class AuszahlungBVAHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRAuszahlungBVA gleitzeitsaldo = (FLRAuszahlungBVA) resultListIterator
						.next();
				rows[row][col++] = gleitzeitsaldo.getI_id();

			
				rows[row][col++] = gleitzeitsaldo.getT_datum();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" auszahlung." + filterKriterien[i].kritName);
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("auszahlung." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("auszahlung.t_datum DESC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("auszahlung.t_datum") < 0) {
				...
				orderBy.append(" auszahlung.t_datum ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRAuszahlungBVA auszahlung ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select auszahlung.i_id from FLRAuszahlungBVA auszahlung "

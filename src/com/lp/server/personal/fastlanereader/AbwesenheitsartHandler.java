...
import com.lp.server.personal.fastlanereader.generated.FLRAbwesenheitsart;
import com.lp.server.personal.fastlanereader.generated.FLRAbwesenheitsartspr;
 ...
 * @version 1.0
 */
public class AbwesenheitsartHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRAbwesenheitsart massnahme = (FLRAbwesenheitsart) o[0];
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRAbwesenheitsartspr spr = (FLRAbwesenheitsartspr) iterUebersetzungenI
					.next();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("if.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("if.i_sort ") < 0) {
	...
	private String getFromClause() {
		return "from FLRAbwesenheitsart if LEFT JOIN if.sprset AS sprset";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				session = setFilter(session);
				String queryString = "select if.i_id from FLRAbwesenheitsart if  LEFT JOIN if.sprset AS sprset"
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			new String[] { "i_id", "if.c_nr", "sprset.c_bez" }));
		}

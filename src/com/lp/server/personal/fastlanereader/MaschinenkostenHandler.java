...
import com.lp.server.personal.fastlanereader.generated.FLRMaschinenkosten;
 ...
 * @version 1.0
 */
public class MaschinenkostenHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRMaschinenkosten flrmaschinenkosten = (FLRMaschinenkosten) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRMaschinenkosten AS maschinenkosten "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(maschinenkosten."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" maschinenkosten."
								+ filterKriterien[i].kritName);
					}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("maschinenkosten."
				...
				orderBy.append("maschinenkosten.t_gueltigab ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("maschinenkosten.t_gueltigab") < 0) {
				...
				orderBy.append(" maschinenkosten.t_gueltigab ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRMaschinenkosten AS maschinenkosten";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRMaschinenkosten maschine = (FLRMaschinenkosten) scrollableResult
								.get(0);

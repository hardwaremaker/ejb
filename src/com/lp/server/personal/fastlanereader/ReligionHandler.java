...
import com.lp.server.personal.fastlanereader.generated.FLRReligion;
 ...
 * @version 1.0
 */
public class ReligionHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = ReligionHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				...
				FLRReligion religion = (FLRReligion) o[0];

				Iterator<?> sprsetIterator = religion.getReligionsprset()
						.iterator();

				rows[row][col++] = religion.getI_id();
				rows[row][col++] = religion.getC_nr();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRReligionspr religionspr = (FLRReligionspr) iterUebersetzungenI
					.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRReligion AS religion"
					+ " LEFT JOIN religion.religionsprset AS religionsprset "
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(buildWhereClausePart("religion.",
							filterKriterien[i]));
				}
			}
			if (filterAdded) {
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("religion.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("religion.i_id") < 0) {
	...
	private String getFromClause() {
		return " FROM FLRReligion AS religion"
				+ " LEFT JOIN religion.religionsprset AS religionsprset";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRReligion religion = (FLRReligion) scrollableResult
								.get(0);
						Integer id = religion.getI_id();
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] { "religion.i_id", "religion.c_nr",
							"religionsprset.c_bez" }));

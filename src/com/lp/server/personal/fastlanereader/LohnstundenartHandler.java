...
import com.lp.server.personal.fastlanereader.generated.FLRLohnstundenart;
 ...
 * @version 1.0
 */
public class LohnstundenartHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRLohnstundenart artikelkommentarart = (FLRLohnstundenart) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRLohnstundenart AS lohnstundenart"
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" lohnstundenart." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("lohnstundenart.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("lohnstundenart.c_nr") < 0) {
				...
				orderBy.append(" lohnstundenart.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRLohnstundenart AS lohnstundenart";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRFarbcode artikelkommentarart = (FLRFarbcode) scrollableResult
								.get(0);

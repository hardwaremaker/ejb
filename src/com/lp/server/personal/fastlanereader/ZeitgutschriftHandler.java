...
import com.lp.server.personal.fastlanereader.generated.FLRZeitgutschrift;
 ...
 * @version 1.0
 */
public class ZeitgutschriftHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRZeitgutschrift zg = (FLRZeitgutschrift) resultListIterator
						.next();
				rows[row][col++] = zg.getI_id();
				rows[row][col++] = zg.getT_datum();
				rows[row][col++] = zg.getU_gutschrift_kommt();
				rows[row++][col++] = zg.getU_gutschrift_geht();
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(zeitgutschrift."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" zeitgutschrift."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("zeitgutschrift.t_datum DESC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("zeitgutschrift.t_datum") < 0) {
				...
				orderBy.append(" zeitgutschrift.t_datum ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRZeitgutschrift zeitgutschrift ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId instanceof Integer) {
			if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
				...
				try {
					session = factory.openSession();
					String queryString = "select zeitgutschrift.i_id from FLRZeitgutschrift zeitgutschrift "
							+ this.buildWhereClause()
							+ this.buildOrderByClause();
					Query query = session.createQuery(queryString);
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			new int[] {
					...
					new String[] { "i_id", "t_datum", Facade.NICHT_SORTIERBAR,

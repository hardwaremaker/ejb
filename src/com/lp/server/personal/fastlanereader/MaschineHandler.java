...
public class MaschineHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRMaschine flrmaschine = (FLRMaschine) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRMaschine AS maschine "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
	...
	private String getFromClause() {
		return " FROM FLRMaschine AS maschine";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRMaschine maschine = (FLRMaschine) scrollableResult
								.get(0);

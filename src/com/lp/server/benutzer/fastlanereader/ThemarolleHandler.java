 ...
 * @version 1.0
 */
public class ThemarolleHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRThemarolle artikelkommentarart = (FLRThemarolle) resultListIterator
						.next();

				rows[row][col++] = artikelkommentarart.getI_id();
				rows[row][col++] = artikelkommentarart.getFlrthema().getC_nr();
				rows[row][col++] = artikelkommentarart.getFlrsystemrolle()
						.getC_bez();
				row++;
				col = 0;
			}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRThemarolle AS themarolle"
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" themarolle." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("themarolle.flrthema.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("themarolle.flrthema.c_nr") < 0) {
				...
				orderBy.append(" themarolle.flrthema.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRThemarolle AS themarolle";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRThemarolle artikelkommentarart = (FLRThemarolle) scrollableResult
								.get(0);

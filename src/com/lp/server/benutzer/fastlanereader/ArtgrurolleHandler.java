 ...
 * @version 1.0
 */
public class ArtgrurolleHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRArtgrurolle artikelkommentarart = (FLRArtgrurolle) resultListIterator
						.next();

				rows[row][col++] = artikelkommentarart.getI_id();
				rows[row][col++] = artikelkommentarart.getFlrartikelgruppe()
						.getC_nr();
				rows[row][col++] = artikelkommentarart.getFlrartikelgruppe()
						.getMandant_c_nr();
				row++;
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRArtgrurolle AS artgrurolle"
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("artgrurolle.flrartikelgruppe.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy
					.indexOf("artgrurolle.flrartikelgruppe.c_nr") < 0) {
				...
				orderBy.append(" artgrurolle.flrartikelgruppe.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRArtgrurolle AS artgrurolle";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRArtgrurolle artikelkommentarart = (FLRArtgrurolle) scrollableResult
								.get(0);
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			new String[] {
					"i_id",
					BenutzerFac.FLR_ARTGRUROLLE_FLRFARTIKELGRUPPE
							+ ".c_nr",
					BenutzerFac.FLR_ARTGRUROLLE_FLRFARTIKELGRUPPE
							+ ".mandant_c_nr" }));

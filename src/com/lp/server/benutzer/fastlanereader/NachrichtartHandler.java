 ...
 * @version 1.0
 */
public class NachrichtartHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRNachrichtart nachrichtenart = (FLRNachrichtart) resultListIterator
						.next();
				...
				rows[row][col++] = new Boolean(Helper
						.short2Boolean(nachrichtenart.getB_archivieren()));
				rows[row++][col++] = new Boolean(Helper
						.short2Boolean(nachrichtenart.getB_popup()));
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(buildWhereClausePart("nachrichtart.",
							filterKriterien[i]));
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("nachrichtart." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("nachrichtart." + PersonalFac.FLR_BERUF_C_BEZ
						+ " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("nachrichtart." + PersonalFac.FLR_BERUF_C_BEZ) < 0) {
	...
	private String getFromClause() {
		return "from FLRNachrichtart nachrichtart ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select nachrichtart.i_id from FLRNachrichtart nachrichtart "

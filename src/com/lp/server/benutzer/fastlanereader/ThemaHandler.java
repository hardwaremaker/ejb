...
import com.lp.server.benutzer.fastlanereader.generated.FLRThema;
 ...
 * @version 1.0
 */
public class ThemaHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRThema thema = (FLRThema) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRThema AS thema"
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(buildWhereClausePart("thema.",
							filterKriterien[i]));
				}
			}
			if (filterAdded) {
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("thema.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("thema.c_nr") < 0) {
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRThema artikelkommentarart = (FLRThema) scrollableResult
								.get(0);
						String cNr = artikelkommentarart.getC_nr();
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { 

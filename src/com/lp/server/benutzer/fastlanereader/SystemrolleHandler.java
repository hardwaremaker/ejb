 ...
 * @version 1.0
 */
public class SystemrolleHandler extends UseCaseHandler {
	/**
	 * 
	 */
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRSystemrolle systemrolle = (FLRSystemrolle) resultListIterator
						.next();
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("systemrolle." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("systemrolle."
						+ BenutzerFac.FLR_SYSTEMROLLE_C_BEZ + " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("systemrolle."
					+ BenutzerFac.FLR_SYSTEMROLLE_C_BEZ) < 0) {
	...
	private String getFromClause() {
		return "from FLRSystemrolle systemrolle ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select systemrolle.i_id from FLRSystemrolle systemrolle "

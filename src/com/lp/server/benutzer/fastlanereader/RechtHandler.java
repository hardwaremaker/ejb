...
/**
 * <p>
 * Hier wird die FLR Funktionalit&auml;t f&uuml;r die Rechte implementiert. Pro UseCase
 ...
 * @version 1.0
 */
public class RechtHandler extends UseCaseHandler {
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(recht."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" recht." + filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("recht." + kriterien[i].kritName);
							orderBy.append(" ");
				...
				orderBy.append("recht.c_nr ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("recht.c_nr") < 0) {
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null /* && ( (String) selectedId).intValue() >= 0 */) {
			...
			try {
				session = factory.openSession();
				String queryString = "select recht.c_nr from FLRRecht recht "
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { String.class,
					...
					new String[] { "c_nr", "c_nr", Facade.NICHT_SORTIERBAR }));
		}

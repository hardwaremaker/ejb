 ...
 * @version 1.0
 */
public class BenutzerHandler extends UseCaseHandler {
	/** Logging utility. */
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" lower(benutzer."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" benutzer." + filterKriterien[i].kritName);
					}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("benutzer." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("benutzer."
						+ BenutzerFac.FLR_BENUTZER_C_BENUTZERKENNUNG + " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("benutzer."
	...
	private String getFromClause() {
		return "from FLRBenutzer benutzer ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select benutzer.i_id from FLRBenutzer benutzer "
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					String.class, String.class, java.sql.Date.class },
					new String[] {
							"Id",
							getTextRespectUISpr("lp.benutzerkennung",
									theClientDto.getMandant(),
									theClientDto.getLocUi()),
							getTextRespectUISpr("lp.defaultmandant",
									theClientDto.getMandant(),
									theClientDto.getLocUi()),
					...
					, new String[] {
							"i_id",
							BenutzerFac.FLR_BENUTZER_C_BENUTZERKENNUNG,
							BenutzerFac.FLR_BENUTZER_FLRMANDANTDEFAULT
									+ ".c_nr",
							BenutzerFac.FLR_BENUTZER_D_GUELTIGBIS, }));
		}
...
//		try {
//			benutzerDto = getBenutzerFac().benutzerFindByPrimaryKey(
...
////					+ theClientDto.getMandant() + "/"
////					+ LocaleFac.BELEGART_BENUTZER.trim() + "/"
////					+ LocaleFac.BELEGART_BENUTZER.trim() + "/"
////					+ benutzerDto.getCBenutzerkennung().replace("/", ".");
//			DocPath docPath = new DocPath(new DocNodeBenutzer(benutzerDto) );

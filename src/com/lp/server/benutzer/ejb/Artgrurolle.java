...
@NamedQueries({
		@NamedQuery(name = "ArtgrurollefindBySystemrolleIIdArtgruIId", query = "SELECT OBJECT(C) FROM Artgrurolle c WHERE c.systemrolleIId = ?1 AND c.artgruIId = ?2"),
		@NamedQuery(name = "ArtgrurollefindBySystemrolleIId", query = "SELECT OBJECT(C) FROM Artgrurolle c WHERE c.systemrolleIId = ?1") })
@Entity
@Table(name = "PERS_ARTGRUROLLE")
public class Artgrurolle implements Serializable {
	@Id
	@Column(name = "I_ID")
	private Integer iId;
	...
	public Artgrurolle(Integer id, Integer systemrolleIId,
			Integer artgruIId) {
		setIId(id);

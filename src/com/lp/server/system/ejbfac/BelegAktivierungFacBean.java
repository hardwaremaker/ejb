...
public class BelegAktivierungFacBean extends Facade implements BelegAktivierungFac {
	/**
	 * Belegarten f&uuml;r die &uuml;berpr&uuml;ft werden soll, ob ein ustpflichtiger Kunde Nicht-USt-Positionen hat,
	 * bzw. ein nicht-ustpflichtiger Kunde USt-Positionen im Beleg hat
	 */
	...
	@Override
	public BelegPruefungDto aktiviereBelegControlled(IAktivierbar facBean, Integer iid, Timestamp t, TheClientDto theClientDto)
			throws EJBExceptionLP, RemoteException {
		return aktiviereBelegControlledImpl(facBean, iid, t, true, theClientDto) ;
	}

	private BelegPruefungDto aktiviereBelegControlledImpl(IAktivierbar facBean, Integer iid, Timestamp t, boolean needsVerify, TheClientDto theClientDto) throws EJBExceptionLP, RemoteException {	
		...
		return needsVerify ? verify(facBean, iid, theClientDto) : null ;		
	}
	...
	@Override
	public BelegPruefungDto berechneAktiviereControlled(IAktivierbar facBean, 
			Integer iid, TheClientDto theClientDto) throws EJBExceptionLP, RemoteException {
		BelegPruefungDto pruefungDto = berechneBelegControlled(facBean, iid, theClientDto) ;
		aktiviereBelegControlledImpl(facBean, iid, pruefungDto.getBerechnungsZeitpunkt(), false, theClientDto);
		return pruefungDto ;
	}
	
	protected BelegPruefungDto verify(IAktivierbar facBean, Integer iid, TheClientDto theClientDto) throws EJBExceptionLP, RemoteException {
		BelegDto belegDto = facBean.getBelegDto(iid, theClientDto) ;
		if(!Helper.isOneOf(belegDto.getBelegartCNr(), ustVerkaufBelegarten)) return new BelegPruefungDto(belegDto.getIId());

		Integer kundeId = facBean.getKundeIdDesBelegs(belegDto, theClientDto) ;
		BelegPruefungDto pruefungDto = new BelegPruefungDto(belegDto.getIId(), kundeId) ;
		if(kundeId == null) return pruefungDto;
		...
		boolean b = getSystemFac().enthaeltEineVKPositionKeineMwstObwohlKundeSteuerpflichtig(
				kundeId, (BelegpositionVerkaufDto[])positionDtos, (BelegVerkaufDto) belegDto, theClientDto) ;
		pruefungDto.setKundeHatUstAberNichtUstPositionen(b);
		b = getSystemFac().enthaeltEinVKBelegUmsatzsteuerObwohlKundeSteuerfrei(
				kundeId, (BelegpositionVerkaufDto[])positionDtos, (BelegVerkaufDto) belegDto, theClientDto) ;

...
public class MandantDtoAssembler {
	public static MandantDto createDto(Mandant mandant) {
		...
		if (mandant != null) {
			...
			mandantDto.setLagerIIdZiellager(mandant.getLagerIIdZiellager());
			mandantDto.setCGlauebiger(mandant.getCGlauebiger());
			mandantDto.setWaehrungCNrZusaetzlich(mandant.getWaehrungCNrZusaetzlich());
			mandantDto.setVerrechnungsmodellIId(mandant.getVerrechnungsmodellIId());
			mandantDto.setZahlungszielIIdAnzahlung(mandant.getZahlungszielIIdAnzahlung());
			mandantDto.setIMaxpersonen(mandant.getIMaxpersonen());

			mandantDto.setBAgbAnfrage(mandant.getBAgbAnfrage());
			mandantDto.setBAgbAngebot(mandant.getBAgbAngebot());
			mandantDto.setBAgbAnhang(mandant.getBAgbAnhang());
			mandantDto.setBAgbAuftrag(mandant.getBAgbAuftrag());
			mandantDto.setBAgbBestellung(mandant.getBAgbBestellung());
			mandantDto.setBAgbLieferschein(mandant.getBAgbLieferschein());
			mandantDto.setBAgbRechnung(mandant.getBAgbRechnung());
			
			mandantDto.setBPreislisteFuerNeukunde(mandant.getBPreislisteFuerNeukunde());
			mandantDto.setCIntrastatRegion(mandant.getIntrastatRegion());
		}
		return mandantDto;
		/*
		 * AD if (mandantDto.validate()) { return mandantDto; } else { throw new
		 * EJBExceptionLP(EJBExceptionLP.FEHLER_UNGUELTIGE_INSTALLATION, null); }
		 */
	}

...
import com.lp.server.system.fastlanereader.generated.FLRPanel;
 ...
 */
public class PanelHandler extends UseCaseHandler {
	/**
	 * 
	 */
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = PanelHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				FLRPanel panel = (FLRPanel) resultListIterator.next();
				rows[row][col++] = panel.getC_nr();
				rows[row][col++] = panel.getC_nr();
				rows[row++][col++] = panel.getC_bez();
				col = 0;
			}
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(panel."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" panel." + filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("panel." + kriterien[i].kritName);
							orderBy.append(" ");
				...
				orderBy.append("panel.c_nr ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("panel.c_nr") < 0) {
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null) {
			...
			try {
				session = factory.openSession();
				String queryString = "select panel." + "c_nr"
						+ " from FLRPanel panel " + this.buildWhereClause()
						+ this.buildOrderByClause();

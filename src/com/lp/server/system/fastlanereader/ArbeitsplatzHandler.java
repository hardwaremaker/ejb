 ...
 * @version 1.0
 */
public class ArbeitsplatzHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = ArbeitsplatzHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				FLRArbeitsplatz arbeitsplatz = (FLRArbeitsplatz) resultListIterator
						.next();
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(arbeitsplatz."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" arbeitsplatz."
								+ filterKriterien[i].kritName);
					}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("arbeitsplatz." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("arbeitsplatz."
						+ SystemFac.FLR_ARBEITSPLATZ_C_PCNAME + " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("arbeitsplatz."
					+ SystemFac.FLR_ARBEITSPLATZ_C_PCNAME) < 0) {
				...
				orderBy.append(" arbeitsplatz."
						+ SystemFac.FLR_ARBEITSPLATZ_C_PCNAME + " ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRArbeitsplatz arbeitsplatz ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select arbeitsplatz.i_id from FLRArbeitsplatz arbeitsplatz "
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					"Id",
					getTextRespectUISpr("system.pcname", theClientDto
							.getMandant(), theClientDto.getLocUi()),
					getTextRespectUISpr("system.standort", theClientDto
							.getMandant(), theClientDto.getLocUi()) },

...
import com.lp.server.artikel.ejbfac.BmeCatKommentar;
import com.lp.server.artikel.ejbfac.BmeCatTransformer;
...
//		return allGroups ;
	...
	public ShopgroupsFlatResult getShopGroupsFlatFindAllChanged(WebshopAuthHeader header, String changedDateTime) {
		...
		try {
			...
			Map<Integer, ShopgruppeMapEntry> shopgroupMap = new HashMap<Integer, ShopgruppeMapEntry>() ;
			for (ShopgruppeDto shopgruppeDto : changedShopgruppen) {
				WebshopShopgroupFlat node = buildWebshopgroupFlat(shopgruppeDto) ;
				sgResult.getShopgroups().add(node) ;				
				
				// Damit der Connector funktioniert, muss die komplette Hierarchie(referenz) &uuml;bermittelt werden.
				shopgroupMap.put(shopgruppeDto.getIId(), new ShopgruppeMapEntry(shopgruppeDto, true)) ;
				while(shopgruppeDto != null && shopgruppeDto.getShopgruppeIId() != null) {
					if(!shopgroupMap.containsKey(shopgruppeDto.getShopgruppeIId())) {
						shopgruppeDto = getArtikelFac()
								.shopgruppeFindByPrimaryKey(shopgruppeDto.getShopgruppeIId(), webClientDto) ;
						if(shopgruppeDto != null) {
							shopgroupMap.put(shopgruppeDto.getIId(), new ShopgruppeMapEntry(shopgruppeDto, false)) ;
						}
					} else {
						shopgruppeDto = shopgroupMap.get(shopgruppeDto.getShopgruppeIId()).getShopgruppeDto() ;
					}
				}
			}

			for (ShopgruppeMapEntry mapEntry : shopgroupMap.values()) {
				if(!mapEntry.isAlreadyExported()) {
					WebshopShopgroupFlat node = buildWebshopgroupFlat(mapEntry.getShopgruppeDto()) ;
					sgResult.getShopgroups().add(node) ;
				}
			}
			sgResult.setOkay() ;
	...
	class ShopgruppeMapEntry {
		private boolean alreadyExported ;
		private ShopgruppeDto shopgruppeDto ;
		
		public ShopgruppeMapEntry(ShopgruppeDto dto, boolean alreadyExported) {
			this.setShopgruppeDto(dto) ;
			this.setAlreadyExported(alreadyExported) ;
		}
	...
	public WebshopItemsResult getItemsChanged(WebshopAuthHeader header, String changedDateTime) {
		...
		Timestamp changedStamp = HelperWebshop.normalizeDateTimeAsTimestamp(d, webClientDto.getLocMandant()) ;
		return calculateItemsChanged(changedStamp);
/*
 ...
 */		
	...
	@Override
	public WebshopItemsResult getItemsRestChanged(TheClientDto theClientDto, WebshopId shopId, Timestamp lastChanged) {
		if(lastChanged == null) {
			return new WebshopItemsResult(BaseRequestResult.ERROR_NULL_PARAMETER, "changedDateTime == null");
		}

		resetCaches();
		webClientDto = theClientDto;
		cachedWebshopIId = shopId.id();

		return calculateItemsChanged(lastChanged);
	}
	
	private WebshopItemsResult calculateItemsChanged(Timestamp changedStamp) {
		Set<Integer> itemIds = new HashSet<Integer>() ;
...
//			itemsResult.setDescription(e.getMessage()) ;			
	...
	private List<ArtikelkommentarDto> findImages(List<BmeCatKommentar> kommentare) {
		List<ArtikelkommentarDto> imageKommentare = new ArrayList<ArtikelkommentarDto>();
		
		for (BmeCatKommentar bmecatKommentar : kommentare) {
			ArtikelkommentarDto kommentarDto = bmecatKommentar.getKommentarDto() ;
			if(kommentarDto.getDatenformatCNr().startsWith("image/")) {
				if(null != kommentarDto.getArtikelkommentarsprDto()) {
					imageKommentare.add(kommentarDto) ;
				}
			}
		}
		
		return imageKommentare ;
	}	
	...
	private void buildItemImage(WebshopItem webItem, List<BmeCatKommentar> kommentare) {
		...
		List<WebshopItemImageEntry> imageEntries = new ArrayList<WebshopItemImageEntry>();
		for (ArtikelkommentarDto artikelkommentarDto : findImages(kommentare)) {
//			if(artikelkommentarDto.getIId().equals(kommentarDto.getIId())) continue ;
			
			WebshopItemImageEntry entry = new WebshopItemImageEntry() ;
			WebArtikelkommentarDto webKommentarDto = new WebArtikelkommentarDto(artikelkommentarDto) ;
			entry.setImage(webKommentarDto.getArtikelkommentarsprDto().getOMedia());
			entry.setName(webKommentarDto.getImageUrl());
			entry.setId(webKommentarDto.getIId());
			
			imageEntries.add(entry) ;
		}

		webItem.setImageEntries(imageEntries);
	}
...
//		if(p != null) return p ;
	...
	private WebshopItemResult getItemFindByIIdImpl(ArtikelDto itemDto, String searchKey) throws RemoteException {
		...
		Date belegDate = Helper.cutDate(Calendar.getInstance().getTime());
		List<PriceInfo> priceInfos = calculatePrices(itemDto, belegDate) ;
...
//			satzDto = getMandantFac().mwstsatzFindByMwstsatzbezIIdAktuellster(mwstsatzbezIId, webClientDto) ;
			satzDto = getMandantFac().mwstsatzZuDatumValidate(mwstsatzbezIId,
					new Timestamp(belegDate.getTime()), webClientDto);
		}

...
@NamedQueries( {
		@NamedQuery(name = "ArtikelbestelltfindByBelegartCNrIBelegartpositionid", query = "SELECT OBJECT(c) FROM Artikelbestellt c WHERE c.cBelegartnr = ?1 AND c.iBelegartpositionid = ?2"),
		@NamedQuery(name = "ArtikelbestelltfindByArtikelIId", query = "SELECT OBJECT(C) FROM Artikelbestellt c WHERE c.artikelIId = ?1") })
@Entity
@Table(name = "WW_ARTIKELBESTELLT")
public class Artikelbestellt implements Serializable {
	@Id
	@Column(name = "I_ID")
	private Integer iId;
	...
	public Artikelbestellt() {
		super();
	}

	public Artikelbestellt(Integer id, String belegartnr,
			Integer belegartpositionid, Integer artikelIId,
			Timestamp liefertermin, BigDecimal menge) {
		setIId(id);

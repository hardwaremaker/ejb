 ...
 * @version 1.0
 */
public class ArtikelklaHandler extends UseCaseHandler {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				Iterator<?> sprsetIterator = artikelklasse
						.getArtikelklassesprset().iterator();
				...
				if( artikelklasse.getFlrartikelklasse() != null){
					...
					.getFlrartikelklasse().getArtikelklassesprset().iterator();
				...
				rows[row][col++] = Helper.short2Boolean(artikelklasse
						.getB_tops());
				row++;
				col = 0;
			}
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRArtikelklassespr artikelklassespr = (FLRArtikelklassespr) iterUebersetzungenI
					.next();
			if (artikelklassespr.getLocale().getC_nr().compareTo(sLocaleI) == 0) {
				sUebersetzung = artikelklassespr.getC_bez();
				break;
			}
		}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRArtikelklasse AS artikelklasse"
					+ " LEFT JOIN artikelklasse.artikelklassesprset AS artikelklassesprset "
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("artikelklasse.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("artikelklasse.c_nr") < 0) {
				...
				orderBy.append(" artikelklasse.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRArtikelklasse AS artikelklasse"
				+ " LEFT JOIN artikelklasse.artikelklassesprset AS artikelklassesprset "
				+ " LEFT OUTER JOIN artikelklasse.flrartikelklasse.artikelklassesprset AS vaterklassesprset ";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRArtikelklasse artikelklasse = (FLRArtikelklasse) scrollableResult
								.get(0);
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] {
							...
							getTextRespectUISpr("lp.bezeichnung",
									theClientDto.getMandant(),
									theClientDto.getLocUi()),
							getTextRespectUISpr("lp.vaterklasse",
									theClientDto.getMandant(),
									theClientDto.getLocUi()),
							getTextRespectUISpr("lp.bezeichnung",
									theClientDto.getMandant(),

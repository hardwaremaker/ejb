...
import com.lp.server.artikel.fastlanereader.generated.FLRErsatztypen;
...
public class ErsatztypenHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRErsatztypen flr = (FLRErsatztypen) resultListIterator.next();

				Object[] rowToAddCandidate = new Object[colCount];

				rowToAddCandidate[getTableColumnInformation().getViewIndex("i_id")] = flr.getI_id();
				rowToAddCandidate[getTableColumnInformation().getViewIndex("artikel.ersatzartikel")] = flr
						.getFlrartikel_ersatz().getC_nr();

				rowToAddCandidate[getTableColumnInformation()
						.getViewIndex("artikel.artikelnummerlang")] = getArtikelFac()
								.artikelFindByPrimaryKeySmall(flr.getFlrartikel_ersatz().getI_id(), theClientDto)
								.formatArtikelbezeichnung();

				if (bArtikelfreigabe == true && flr.getFlrartikel_ersatz().getT_freigabe()!=null) {
					rowToAddCandidate[getTableColumnInformation()
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" ersatztypen." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName.endsWith(Facade.NICHT_SORTIERBAR)) {
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("ersatztypen." + kriterien[i].kritName);
							orderBy.append(" ");
				...
				orderBy.append("ersatztypen.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("ersatztypen.i_id") < 0) {
				...
				orderBy.append(" ersatztypen.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private static final String fromClause = "from FLRErsatztypen ersatztypen ";
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien, Object selectedId) throws EJBExceptionLP {
		...
		if (selectedId != null /* && ((String) selectedId).length() > 0 */) {
			...
			try {
				session = factory.openSession();
				String queryString = "select ersatztypen.i_id from FLRErsatztypen ersatztypen "
	...
	private TableColumnInformation createColumnInformation(String mandant, Locale locUi) {
		...
		columns.add("artikel.ersatzartikel", String.class, getTextRespectUISpr("artikel.ersatzartikel", mandant, locUi),
				QueryParameters.FLR_BREITE_SHARE_WITH_REST, "flrartikel_ersatz.c_nr");
		...
		if (bArtikelfreigabe == true) {
			columns.add("IconFreigabe", Icon.class, getTextRespectUISpr("ww.freigabe", mandant, locUi),
					QueryParameters.FLR_BREITE_S, "flrartikel_ersatz.t_freigabe");
		}
		return columns;
	}

...
import com.lp.server.system.jcr.service.docnode.DocNodeVerschleissteil;
 ...
 * @version 1.0
 */
public class VerschleissteilHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRVerschleissteil flr = (FLRVerschleissteil) resultListIterator.next();

				rows[row][col++] = flr.getI_id();
				rows[row][col++] = flr.getC_nr();
				rows[row][col++] = flr.getC_bez();
				row++;
				col = 0;
			}
	...
	public PrintInfoDto getSDocPathAndPartner(Object key) {
		VerschleissteilDto verschleissteilDto = null;
		try {
			verschleissteilDto = getArtikelFac()
					.verschleissteilFindByPrimaryKey((Integer) key);
		} catch (Exception e) {
			// Nicht gefunden
		}
		if (verschleissteilDto != null) {

			String pfad = verschleissteilDto.getCNr();

			DocPath docPath = new DocPath(new DocNodeVerschleissteil(
					verschleissteilDto, theClientDto.getMandant(), pfad));
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRVerschleissteil AS verschleissteil"
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" verschleissteil." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("verschleissteil.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("verschleissteil.i_id") < 0) {
				...
				orderBy.append(" verschleissteil.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRVerschleissteil AS verschleissteil";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRVerschleissteil flr = (FLRVerschleissteil) scrollableResult.get(0);
						Integer iId = flr.getI_id();

 ...
 * @version 1.0
 */
public class ArtikelreservierungHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final String fromClause = "from FLRArtikelreservierung artikelreservierung ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRArtikelreservierung auftragreservierung = (FLRArtikelreservierung) resultListIterator
						.next();
				...
				if (auftragreservierung.getC_belegartnr().equals(
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" artikelreservierung."
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("artikelreservierung."
				...
				orderBy
						.append("artikelreservierung."
								+ ReservierungFac.FLR_ARTIKELRESERVIERUNG_D_LIEFERTERMIN
								+ " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("artikelreservierung.i_id") < 0) {
				...
				orderBy.append(" artikelreservierung.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null /* && ((String) selectedId).length() > 0 */) {
			...
			try {
				session = factory.openSession();
				String queryString = "select artikelreservierung."
						+ ReservierungFac.FLR_ARTIKELRESERVIERUNG_C_BELEGARTNR
						+ " from FLRArtikelreservierung artikelreservierung "
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(
					...
					new String[] {
							"PK",
							getTextRespectUISpr("lp.auftrag", theClientDto
									.getMandant(), theClientDto.getLocUi()),
							getTextRespectUISpr("lp.kundenname", theClientDto
									.getMandant(), theClientDto.getLocUi()),
							getTextRespectUISpr("lp.projektbezeichnung",
									theClientDto.getMandant(), theClientDto
											.getLocUi()),
							getTextRespectUISpr("lp.liefertermin", theClientDto
									.getMandant(), theClientDto.getLocUi()),
					...
					new String[] {
							...
							"",
							ReservierungFac.FLR_ARTIKELRESERVIERUNG_D_LIEFERTERMIN,
							ReservierungFac.FLR_ARTIKELRESERVIERUNG_F_MENGE }));
		}

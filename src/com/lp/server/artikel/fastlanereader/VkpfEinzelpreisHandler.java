 ...
 * @version 1.0
 */
public class VkpfEinzelpreisHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final String FLR_VKPFARTIKELPREIS = "artikelpreis.";
	public static final String FLR_VKPFARTIKELPREIS_FROM_CLAUSE = " from FLRVkpfartikelpreis artikelpreis";
	int iPreisbasis = 0;
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRVkpfartikelpreis artikelpreis = (FLRVkpfartikelpreis) resultListIterator
						.next();

				rows[row][col++] = artikelpreis.getI_id();
				rows[row][col++] = artikelpreis.getFlrpreisliste().getC_nr();
				rows[row][col++] = artikelpreis.getN_artikelfixpreis();
				rows[row][col++] = artikelpreis
						.getN_artikelstandardrabattsatz();

				rows[row++][col++] = artikelpreis.getT_preisgueltigab();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" " + FLR_VKPFARTIKELPREIS
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append(FLR_VKPFARTIKELPREIS
				...
				orderBy.append(FLR_VKPFARTIKELPREIS + "flrpreisliste.c_nr, "
						+ FLR_VKPFARTIKELPREIS + "t_preisgueltigab");

				sortAdded = true;
			}

			if (orderBy.indexOf(FLR_VKPFARTIKELPREIS + "t_preisgueltigab") < 0) {
				...
				orderBy.append(FLR_VKPFARTIKELPREIS + "t_preisgueltigab");
	...
	private String getFromClause() {
		return FLR_VKPFARTIKELPREIS_FROM_CLAUSE;
	}
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					String.class, BigDecimal.class, Double.class,
					java.util.Date.class }, new String[] { "i_id",
					...
					new int[] { QueryParameters.FLR_BREITE_SHARE_WITH_REST,
							...
							QueryParameters.FLR_BREITE_SHARE_WITH_REST,
							QueryParameters.FLR_BREITE_SHARE_WITH_REST, },
					new String[] { "i_id", "flrpreisliste.c_nr",
							"n_artikelfixpreis", "n_artikelstandardrabattsatz",
							"t_preisgueltigab" }));
		}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select " + FLR_VKPFARTIKELPREIS + "i_id"
						+ FLR_VKPFARTIKELPREIS_FROM_CLAUSE

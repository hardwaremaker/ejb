...
import com.lp.server.artikel.fastlanereader.generated.FLRWaffentypFein;
 ...
 * @version 1.0
 */
public class WaffentypFeinHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRWaffentypFein flr = (FLRWaffentypFein) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRWaffentypFein AS waffentypFein"
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("waffentypFein.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("waffentypFein.i_id") < 0) {
				...
				orderBy.append(" waffentypFein.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRWaffentypFein AS waffentypFein";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRWaffentypFein flr = (FLRWaffentypFein) scrollableResult
								.get(0);

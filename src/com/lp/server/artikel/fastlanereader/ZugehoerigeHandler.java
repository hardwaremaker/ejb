...
import com.lp.server.artikel.fastlanereader.generated.FLRZugehoerige;
 ...
 * @version 1.0
 */
public class ZugehoerigeHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRZugehoerige flrzugehoerige = (FLRZugehoerige) resultListIterator
						.next();
				...
				ArtikelDto artikelDto = getArtikelFac()
						.artikelFindByPrimaryKeySmall(
								flrzugehoerige.getFlrartikel().getI_id(),
								theClientDto);
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(flrzugehoerige."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" flrzugehoerige."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("flrzugehoerige.flrartikel.c_nr ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("flrzugehoerige.i_id") < 0) {
				...
				orderBy.append(" flrzugehoerige.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRZugehoerige as flrzugehoerige ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select flrzugehoerige.i_id from FLRZugehoerige flrzugehoerige "

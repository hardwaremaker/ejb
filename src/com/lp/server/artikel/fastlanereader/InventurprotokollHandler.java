...
import com.lp.server.artikel.fastlanereader.generated.FLRInventurprotokoll;
 ...
 * @version 1.0
 */
public class InventurprotokollHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRInventurprotokoll inventurprotokoll = (FLRInventurprotokoll) resultListIterator
						.next();
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(inventurprotokoll."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" inventurprotokoll."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
				...
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("inventurprotokoll."
			...
			if (orderBy.indexOf("inventurprotokoll.i_id") < 0) {
				...
				orderBy.append(" inventurprotokoll.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRInventurprotokoll inventurprotokoll ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select inventurprotokoll.i_id from FLRInventurprotokoll inventurprotokoll "

 ...
 * @version 1.0
 */
public class ArtikelkommentarHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRArtikelkommentar artikelkommentar = (FLRArtikelkommentar) resultListIterator
						.next();
				rows[row][col++] = artikelkommentar.getI_id();
				rows[row][col++] = artikelkommentar.getFlrartikelkommentarart()
						.getC_nr();
				rows[row][col++] = artikelkommentar.getDatenformat_c_nr();

				try {
					ArtikelkommentarsprDto dto = getArtikelkommentarFac()
							.artikelkommentarsprFindByPrimaryKey(
									artikelkommentar.getI_id(), locale);

					if (dto != null) {
						String sStrippedHTML = null;
						if (dto.getXKommentar() != null) {
							sStrippedHTML = Helper.strippHTML(dto
									.getXKommentar());
						}
				...
				rows[row][col++] = new Boolean(Helper
						.short2boolean(artikelkommentar.getB_defaultbild()));
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(flrartikelkommentar."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" flrartikelkommentar."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("flrartikelkommentar."
				...
				orderBy
						.append("flrartikelkommentar.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("flrartikelkommentar.i_id") < 0) {
				...
				orderBy.append(" flrartikelkommentar.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRArtikelkommentar as flrartikelkommentar ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select flrartikelkommentar.i_id from FLRArtikelkommentar flrartikelkommentar "
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(
					...
					new String[] {
							"i_id",
							getTextRespectUISpr("lp.kommentarart", theClientDto
							...
							getTextRespectUISpr("lp.kommentar", theClientDto
									.getMandant(), theClientDto.getLocUi()),
							getTextRespectUISpr("lp.defaultbild", theClientDto
									.getMandant(), theClientDto.getLocUi()) },
					new String[] {
							"i_id",
							...
							ArtikelkommentarFac.FLR_ARTIKELKOMMENTAR_B_DEFAULTBILD }));

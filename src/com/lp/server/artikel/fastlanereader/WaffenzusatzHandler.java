...
import com.lp.server.artikel.fastlanereader.generated.FLRWaffenzusatz;
 ...
 * @version 1.0
 */
public class WaffenzusatzHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRWaffenzusatz flr = (FLRWaffenzusatz) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRWaffenzusatz AS waffenzusatz"
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("waffenzusatz.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("waffenzusatz.i_id") < 0) {
				...
				orderBy.append(" waffenzusatz.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRWaffenzusatz AS waffenzusatz";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRWaffenzusatz flr = (FLRWaffenzusatz) scrollableResult
								.get(0);

...
public class SnrChnrFuerReklamationHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRLagerbewegung lagerbewegung = (FLRLagerbewegung) o[0];
				rows[row][col++] = lagerbewegung.getI_id();
				rows[row][col++] = lagerbewegung.getC_seriennrchargennr();
				rows[row][col++] = lagerbewegung.getFlrartikel().getC_nr();
				rows[row][col++] = o[1];

				try {
					BelegInfos biDto = getLagerFac().getBelegInfos(
							lagerbewegung.getC_belegartnr(),
							lagerbewegung.getI_belegartid(),
							lagerbewegung.getI_belegartpositionid(),
							theClientDto);

					rows[row][col++] = lagerbewegung.getC_belegartnr();
					rows[row][col++] = biDto.getBelegnummer();
					rows[row][col++] = biDto.getKundeLieferant();
					if (biDto.getBelegdatum() != null) {
						rows[row][col++] = new java.util.Date(biDto
								.getBelegdatum().getTime());
					}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			session = factory.openSession();
			String queryString = "select count(*) from FLRLagerbewegung lagerbewegung "
	...
	private String buildWhereClause() {
		...
		if (!getMandantFac().darfAnwenderAufZusatzfunktionZugreifen(
				MandantFac.ZUSATZFUNKTION_ZENTRALER_ARTIKELSTAMM, theClientDto) || (getMandantFac().darfAnwenderAufZusatzfunktionZugreifen(
						MandantFac.ZUSATZFUNKTION_ZENTRALER_ARTIKELSTAMM, theClientDto) && getMandantFac().darfAnwenderAufZusatzfunktionZugreifen(
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].kritName.equals("flrartikel.c_nr")) {
						String s = filterKriterien[i].value.toLowerCase();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("lagerbewegung."
						+ LagerFac.FLR_LAGERBEWEGUNG_C_SERIENNRCHARGENNR + " ASC ");
				sortAdded = true;
			}
	...
	private String getFromClause() {
		return "SELECT lagerbewegung,(SELECT a.c_bez FROM FLRArtikellistespr a WHERE a.locale_c_nr='"
				+ theClientDto.getLocUiAsString()
				+ "' AND a.Id.artikelliste.i_id=lagerbewegung.artikel_i_id) from FLRLagerbewegung lagerbewegung ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null /* && ((Integer) selectedId).intValue() >= 0 */) {
			...
			try {
				session = factory.openSession();
				String queryString = "select lagerbewegung."
						+ LagerFac.FLR_LAGERBEWEGUNG_C_SERIENNRCHARGENNR
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							Facade.NICHT_SORTIERBAR, Facade.NICHT_SORTIERBAR,
							LagerFac.FLR_LAGERBEWEGUNG_T_BELEGDATUM, }));
		}

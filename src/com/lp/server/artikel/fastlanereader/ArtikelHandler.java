...
public class ArtikelHandler extends UseCaseHandler {
	...
	public static final String FLR_ARTIKEL = "flrartikel.";
	public static final String FLR_ARTIKEL_FROM_CLAUSE = " from FLRArtikel flrartikel ";
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(artikel."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" artikel." + filterKriterien[i].kritName);
					}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("artikel." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("artikel.c_nr ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("artikel.i_id") < 0) {
	...
	private String getFromClause() {
		return "from FLRArtikel artikel ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select artikel.i_id from FLRArtikel artikel "
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { Object.class,
					Object.class, Object.class }, new String[] {
					"Id",
					getTextRespectUISpr("bes.artikelcnr", theClientDto
							.getMandant(), theClientDto.getLocUi()),
					getTextRespectUISpr("lp.artikelart",

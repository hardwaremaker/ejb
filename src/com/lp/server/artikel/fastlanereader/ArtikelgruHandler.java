 ...
 * @version 1.0
 */
public class ArtikelgruHandler extends UseCaseHandler {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				if( artikelgruppe.getFlrartikelgruppe() != null){
					...
					.getFlrartikelgruppe().getArtikelgruppesprset().iterator();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRArtikelgruppe AS artikelgruppe"
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRArtikelgruppe artikelgruppe = (FLRArtikelgruppe) scrollableResult
								.get(0);

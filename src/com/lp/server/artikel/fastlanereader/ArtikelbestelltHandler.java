...
/**
 * <p>
 * Hier wird die FLR Funktionalit&auml;t f&uuml;r die Bestellisten implementiert. Pro
...
public class ArtikelbestelltHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final String fromClause = "from FLRArtikelbestellt artikelbestellt ";
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("artikelbestellt."
				...
				orderBy
						.append("artikelbestellt."
								+ ReservierungFac.FLR_ARTIKELRESERVIERUNG_D_LIEFERTERMIN
								+ " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("artikelbestellt.i_id") < 0) {
				...
				orderBy.append(" artikelbestellt.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null /* && ((String) selectedId).length() > 0 */) {
			...
			try {
				session = factory.openSession();
				String queryString = "select artikelbestellt."
						+ ArtikelbestelltFac.FLR_ARTIKELBESTELLT_C_BELEGARTNR
						+ " from FLRArtikelbestellt artikelbestellt "
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							getTextRespectUISpr("bes.bestellung", mandantCNr,
									locUI),
							getTextRespectUISpr("bes.lieferantenname",
									mandantCNr, locUI),
					...
					new String[] {
							...
							"",
							"",
							ArtikelbestelltFac.FLR_ARTIKELBESTELLT_D_LIEFERTERMIN,
							ArtikelbestelltFac.FLR_ARTIKELBESTELLT_F_MENGE }));
		}

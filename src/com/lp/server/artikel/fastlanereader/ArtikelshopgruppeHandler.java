 ...
 * @version 1.0
 */
public class ArtikelshopgruppeHandler extends UseCaseHandler {
	 ...
	 * Konstruktor.
	 */
	public ArtikelshopgruppeHandler() {
	}
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRArtikelshopgruppe flrArtikelshopgruppe = (FLRArtikelshopgruppe) resultListIterator
						.next();

				rows[row][col++] = flrArtikelshopgruppe.getI_id();
				rows[row][col++] = flrArtikelshopgruppe.getFlrshopgruppe()
						.getC_nr();
				rows[row++][col++] = getArtikelFac()
						.shopgruppeFindByPrimaryKey(
								flrArtikelshopgruppe.getShopgruppe_i_id(),
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			session = factory.openSession();
			String queryString = "select count(*) " + ArtikelshopgruppeHandler.getFromClause()
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" artikelshopgruppe."
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("artikelshopgruppe."
				...
				orderBy.append("artikelshopgruppe.flrshopgruppe.c_nr ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("artikelshopgruppe.i_id") < 0) {
				...
				orderBy.append(" artikelshopgruppe.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private static final String fromClause = "from FLRArtikelshopgruppe artikelshopgruppe ";
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							"PK",
							getTextRespectUISpr("lp.shopgruppe", mandantCNr, locUI),

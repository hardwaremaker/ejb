...
import com.lp.server.artikel.fastlanereader.generated.FLRWaffenkaliber;
 ...
 * @version 1.0
 */
public class WaffenkaliberHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRWaffenkaliber flr = (FLRWaffenkaliber) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRWaffenkaliber AS waffenkaliber"
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("waffenkaliber.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("waffenkaliber.i_id") < 0) {
				...
				orderBy.append(" waffenkaliber.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRWaffenkaliber AS waffenkaliber";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRWaffenkaliber flr = (FLRWaffenkaliber) scrollableResult
								.get(0);

 ...
 * @version 1.0
 */
public class InventurstandHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRInventurstand inventurliste = (FLRInventurstand) resultListIterator
						.next();
				rows[row][col++] = inventurliste.getI_id();

				if (!lastArtikelId.equals(inventurliste.getFlrartikel()
						.getI_id())) {
					artikelDto = getArtikelFac().artikelFindByPrimaryKeySmall(
							inventurliste.getFlrartikel().getI_id(),
							theClientDto);
				}
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(inventurstand."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" inventurstand."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (orderBy.indexOf("inventurstand."
	...
	private String getFromClause() {
		return "from FLRInventurstand inventurstand ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select inventurstand.i_id from FLRInventurstand inventurstand "

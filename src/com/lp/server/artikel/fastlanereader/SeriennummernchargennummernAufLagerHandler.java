...
public class SeriennummernchargennummernAufLagerHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = lagerbewegung.getC_seriennrchargennr();
				rows[row++][col++] = new java.sql.Date(lagerbewegung
						.getT_buchungszeit().getTime())
						+ " - "
						+ new java.sql.Time(lagerbewegung.getT_buchungszeit()
								.getTime()).toString();
				col = 0;
			}
	...
	private String buildWhereClause() {
		...
		where.append(" lagerbewegung."
				+ LagerFac.FLR_LAGERBEWEGUNG_B_VOLLSTAENDIGVERBRAUCHT
				+ "=0 AND lagerbewegung." + LagerFac.FLR_LAGERBEWEGUNG_B_ABGANG
				+ "=0 AND lagerbewegung."
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("lagerbewegung."
						+ LagerFac.FLR_LAGERBEWEGUNG_T_BUCHUNGSZEIT + " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("lagerbewegung."
					+ LagerFac.FLR_LAGERBEWEGUNG_T_BUCHUNGSZEIT) < 0) {
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Object.class,
					...
					getTextRespectUISpr("lp.buchungszeitpunkt", mandantCNr,
							locUI) }, new String[] { "PK",
					LagerFac.FLR_LAGERBEWEGUNG_C_SERIENNRCHARGENNR,
					LagerFac.FLR_LAGERBEWEGUNG_T_BUCHUNGSZEIT }));
		}

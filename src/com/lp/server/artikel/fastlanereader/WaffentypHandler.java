...
import com.lp.server.artikel.fastlanereader.generated.FLRWaffentyp;
 ...
 * @version 1.0
 */
public class WaffentypHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRWaffentyp flr = (FLRWaffentyp) resultListIterator
						.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRWaffentyp AS waffentyp"
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("waffentyp.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("waffentyp.i_id") < 0) {
	...
	private String getFromClause() {
		return " FROM FLRWaffentyp AS waffentyp";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRWaffentyp flr = (FLRWaffentyp) scrollableResult
								.get(0);

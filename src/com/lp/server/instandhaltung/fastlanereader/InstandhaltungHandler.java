...
import com.lp.server.instandhaltung.fastlanereader.generated.FLRInstandhaltung;
 ...
 * @version 1.0
 */
public class InstandhaltungHandler extends UseCaseHandler {
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT count(distinct instandhaltung.i_id) FROM FLRInstandhaltung AS instandhaltung LEFT JOIN instandhaltung.standortset AS standortset LEFT JOIN standortset.flrpartner AS partner  LEFT JOIN partner.flrlandplzort AS landplzort LEFT JOIN landplzort.flrort AS ort"
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].kritName.equals("partner.c_strasse")) {
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("instandhaltung.flrkunde."
						+ KundeFac.FLR_PARTNER + "."
						+ PartnerFac.FLR_PARTNER_NAME1NACHNAMEFIRMAZEILE1 + " ");

				sortAdded = true;
			}
			if (orderBy.indexOf("instandhaltung.i_id") < 0) {
				...
				orderBy.append(" instandhaltung.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRInstandhaltung AS instandhaltung LEFT JOIN instandhaltung.standortset AS standortset LEFT JOIN standortset.flrpartner AS partner LEFT JOIN partner.flrlandplzort AS landplzort LEFT JOIN landplzort.flrort AS ort";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRInstandhaltung artikelkommentarart = (FLRInstandhaltung) scrollableResult
								.get(0);
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			new String[] {
					"instandhaltung.i_id",
					"instandhaltung.flrkunde." + KundeFac.FLR_PARTNER + "."
							+ PartnerFac.FLR_PARTNER_NAME1NACHNAMEFIRMAZEILE1,
					"instandhaltung.flrkunde." + KundeFac.FLR_PARTNER + "."

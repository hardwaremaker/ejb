...
public class IskategorieHandler extends UseCaseHandler {
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(iskategorie."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" iskategorie." + filterKriterien[i].kritName);
					}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("iskategorie." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("iskategorie.c_bez ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("iskategorie.c_bez") < 0) {
				...
				orderBy.append(" iskategorie.c_bez ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRIskategorie iskategorie ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select iskategorie.i_id from FLRIskategorie iskategorie "

 ...
 * @version 1.0
 */
public class StandortHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = artikelkommentarart.getFlrinstandhaltung()
						.getFlrkunde().getFlrpartner()
						.getC_name1nachnamefirmazeile1();
				rows[row][col++] = artikelkommentarart.getFlrpartner()
						.getC_name1nachnamefirmazeile1();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRStandort AS standort left join standort.flrpartner.flrlandplzort as flrlandplzort "
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("standort.flrpartner.c_name1nachnamefirmazeile1 ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("standort.i_id") < 0) {
	...
	private String getFromClause() {
		return " FROM FLRStandort AS standort left join standort.flrpartner.flrlandplzort as flrlandplzort ";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRStandort artikelkommentarart = (FLRStandort) scrollableResult
								.get(0);
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							getTextRespectUISpr("lp.ort",
									theClientDto.getMandant(),
									theClientDto.getLocUi()),
							getTextRespectUISpr("lp.strasse",
									theClientDto.getMandant(),
									theClientDto.getLocUi()) },
					new String[] {
							"i_id",
							"standort.flrpartner.c_name1nachnamefirmazeile1",
							"standort.flrinstandhaltung.flrkunde.flrpartner.c_name1nachnamefirmazeile1",
							"standort."+KundeFac.FLR_PARTNER + "."

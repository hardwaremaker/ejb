 ...
 * @version 1.0
 */
public class WartungsschritteHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			String queryString = "SELECT wartungsschritte FROM FLRWartungsschritte wartungsschritte"
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = new BigDecimal(dDauer);
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(wartungsschritte."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" wartungsschritte."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("wartungsschritte."
				...
				orderBy.append("wartungsschritte.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("wartungsschritte.i_sort") < 0) {
				...
				orderBy.append(" wartungsschritte.i_sort ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRWartungsschritte wartungsschritte ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select wartungsschritte.i_id from FLRWartungsschritte wartungsschritte "
						+ this.buildWhereClause() + this.buildOrderByClause();
				Query query = session.createQuery(queryString);
		...
		try {
			if (super.getTableInfo() == null) {
				...
				setTableInfo(new TableInfo(
						...
						new String[] { "id", "flrartikel.c_nr",
								Facade.NICHT_SORTIERBAR,
								"auftragwiederholungsintervall_c_nr", "l_dauer" }));
			}
		} catch (RemoteException ex) {
			throwEJBExceptionLPRespectOld(ex);
			return null;

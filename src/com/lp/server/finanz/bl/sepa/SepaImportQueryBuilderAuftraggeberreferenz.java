...
/**
 * Sucht nach der Auftraggeberreferenz im Zahlungsvorschlag
 * 
 * @author andi
 *
 */
public class SepaImportQueryBuilderAuftraggeberreferenz extends
		SepaImportQueryBuilderEr {

	public SepaImportQueryBuilderAuftraggeberreferenz(String mandantCNr, List<String> stati) {
		super(mandantCNr, stati);
	}
	
	@Override
	protected void buildFromQuery() {
		froms.add(SELECT_EINGANGSRECHNUNG_I_ID + FROM_ZAHLUNGSVORSCHLAG);
		froms.add(JOIN_EINGANGSRECHNUNG);
	}

	@Override
	protected void buildWhereCriterion() {
//		wheres.add(Zahlungsvorschlag + ".c_auftraggeberreferenz=" + getCritValueWithSingleQuotes());
		addWhere(Zahlungsvorschlag + ".c_auftraggeberreferenz=?1", getCritValue());
	}

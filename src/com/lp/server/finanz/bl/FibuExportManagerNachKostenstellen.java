	...
	public FibuexportDto[] getExportdatenEingangsrechnung(
		...
		try {
			...
			if (erDto.getNBetrag().compareTo(new BigDecimal(0)) != 0) {
				...
				// Mehrfachkontierung
				EingangsrechnungKontierungDto[] erKontoDto = super
				...
				String laenderartCNr = super
						.getLaenderartZuPartner(lieferantDto.getPartnerDto());
				...
				if (erKontoDto == null) {
					...
					for (int i = 0; i < erKontoDto.length; i++) {
						...
						exportDtos[iAktuelleZeileGegenbuchung].setValutadatum(dStichtag);
						...
						BigDecimal bdNettobetragFW = bdNettobetrag.divide(erDto
								.getNKurs(), FinanzFac.NACHKOMMASTELLEN, BigDecimal.ROUND_HALF_EVEN);
						exportDtos[iAktuelleZeileGegenbuchung]
			...
			if (reDto.getNWert() != null
				...
				if (iAnzahlGegenkonten > 1) {
					...
					if (reKontierungDto == null) {
						...
						KostenstelleDto kstDto = super.pruefeKostenstelle(
								kstIId, reDto);
						exportDtos[0].setKost(kstDto);
						KontoDto kontoDtoGegenkontoUebersetzt = uebersetzeKontoNachLandBzwLaenderart(
								kstDto.getKontoIId(), sLaenderart, debitorenkontoDto.getFinanzamtIId(), 
				...
				if (reKontierungDto == null) {
					...
					KostenstelleDto kstDtoHW = pruefeKostenstelle(reDto
							.getKostenstelleIId(), reDto);
					KontoDto kontoDtoGegenkontoUebersetzt = uebersetzeKontoNachLandBzwLaenderart(
							kstDtoHW.getKontoIId(), sLaenderart, debitorenkontoDto.getFinanzamtIId(), 
							debitorenkontoDto.getMandantCNr(), reDto, iLand);
					// buchung aufs HW-Konto
					...
					for (int i = 0; i < reKontierungDto.length; i++) {
						...
						exportDtos[1 + i].setWaehrung(theClientDto
								.getSMandantenwaehrung());
						// Sollbetrag ist netto
						BigDecimal bdNettobetrag = reDto.getNWert().multiply(
								reKontierungDto[i].getNProzentsatz())
								.movePointLeft(2);
						BigDecimal bdNettobetragFW = bdNettobetrag.divide(reDto
								.getNKurs(), FinanzFac.NACHKOMMASTELLEN, BigDecimal.ROUND_HALF_EVEN);
						...
						// Steuerbetrag ist der UST-Betrag
						BigDecimal bdUst = reDto.getNWertust().multiply(
								reKontierungDto[i].getNProzentsatz())
								.movePointLeft(2);
 ...
 * Satzaufbaues ist auch notwendig, dass die Datens&auml;tze buchhalterisch richtig
 * sind. Es ist genau zu beachten, welcher Betrag im Soll bzw. im Haben zu
 * stehen hat und ob dies der Netto- oder der Bruttobetrag ist. Aus den Fakturen
 * m&uuml;ssen Buchungss&auml;tze gebildet werden und nur diese k&ouml;nnen mit dieser
 * Schnittstelle &uuml;bernommen werden. Die folgenden Beispiele sollen als Hilfe bei
 * der Erstellung der Datens&auml;tze dienen, sie k&ouml;nnen jedoch
 * Buchhaltungskenntnisse nicht ersetzen. 2.3.2. Ausgangsrechnungen mit
 * Gegenbuchung Eine Ausgangsrechnung ist dann in dieser Buchungsart zu
 * verbuchen, wenn nur ein Gegenkonto vorliegt. Eine solche Rechnung ist in zwei
 * Buchungss&auml;tze aufzuspalten: Buchung am Kundenkonto und Gegenbuchung am
 * Erl&ouml;skonto. Am Kundenkonto wird der Bruttobetrag, auf den Erl&ouml;skonten der
 * Nettobetrag und der Steuerbetrag verbucht. Nettobetrag + Steuer muss den
 * Bruttobetrag ergeben. 2.3.2.1. Ausgangsrechnung in EUR Ausgangsrechnung &uuml;ber
 * EUR 10.000,-- (netto) + 2.000,-- USt EUR 12.000,-- brutto. Rechnungsdatum 15.
 * 10. 2002, Rechnungsnummer 100, Kundenkonto 20100, Erl&ouml;skonto 4120. Buchung
 * auf das Kundenkonto
 ...
 * Ausgangsrechnung in einer Drittw&auml;hrung Bei Rechnungen in Drittw&auml;hrungen ist
 * im Feld ???Fremdw&auml;hrung??? der ISO-Code der Drittw&auml;hrung und in den Feldern
 * ???Fremdw&auml;hrung-Sollbetrag??? bzw. ???Fremdw&auml;hrung-Habenbetrag??? der
 * Drittw&auml;hrungsbetrag einzugeben. Die Fremdw&auml;hrungsbetr&auml;ge sind in die
 * Grundw&auml;hrung der Buchhaltung umzurechnen und im Feld ???W&auml;hrung??? der ISO-Code
 * der Grundw&auml;hrung sowie unter ???Sollbetrag??? bzw. ???Habenbetrag??? die
 * umgerechneten Betr&auml;ge einzugeben. Ausgangsrechnung &uuml;ber USD 900,-- (netto) +
 * 180,-- USt USD 1.080,-- brutto. Rechnungsdatum 20. 7. 2002, Rechnungsnummer
 * 100, Kundenkonto 20100, Erl&ouml;skonto 4120. Die umgerechnete Betr&auml;ge sind EUR
 * 1000,-- + 200,-- 1200,--
 ...
 * zuerst die Buchung am Erl&ouml;skonto und dann die Gegenbuchung am Debitorenkonto.
 * Gutschrift &uuml;ber EUR 1.000,-- (netto) + 200,-- USt EUR 1.200,-- brutto.
 * Rechnungsdatum 17. 1. 2002, Rechnungsnummer 101, Kundenkonto 20100,
 * Erl&ouml;skonto 4120.
 ...
 * Erl&ouml;skonto vorliegt. Es erfolgt zuerst die Split-Sammelbuchung am Debitor und
 * dann die Gegenbuchungen auf den einzelnen Erl&ouml;skonten 2.3.4.1. Splitbuchung
 * mit gleichen USt-S&auml;tzen Beispiel: Ausgangsrechnung &uuml;ber brutto EUR 60.000,--,
 * am Konto 4120 sind netto 20.000,-- + 20 % USt (4.000,---) , am Konto 4122
 * netto 30.000,-- + 20 % USt (6.000,--) zu verbuchen. Split-Sammelbuchung:
 ...
 * USt-S&auml;tzen liegt im Feld ???USt-Prozentsatz??? und ???USt-Code??? . In der
 * Split-Sammelbuchung sind diese Felder bei verschiedenen USt- S&auml;tzen Null.
 * Beispiel: Ausgangsrechnung &uuml;ber brutto EUR 25.000,--, am Konto 4120 sind
 * netto 10.000,-- + 20 % USt, am Konto 4110 netto 11818.18 + 10 % USt zu
 ...
 * 2;2;0;1;;;;Innergem. Lieferung;;DE123456788 2.3.6. Lieferung in Drittl&auml;nder
 * (Exporterl&ouml;s) Beispiel: Ausgangsrechnung &uuml;ber USD 900,-- (= EUR 1.000,--)
 ...
 * Bruttobetrag im Haben. Im Feld ???USt-Code??? ist 1 f&uuml;r Vorsteuer einzugeben.
 * Eingangsrechnung &uuml;ber EUR 10.000,-- (netto) + 2.000,-- USt EUR 12.000,--
 * brutto. Rechnungsdatum 17. 1. 2002, Rechnungsnummer 100, Lieferantenkonto
 * 30100, Einkaufskonto 5100.
 ...
 * Gegenbuchung Gutschrift &uuml;ber EUR 1.000,-- (netto) + 200,-- USt EUR 1.200,--
 * brutto. Rechnungsdatum 17. 1. 2002, Rechnungsnummer 101, Lieferantenkonto
 * 20100, Aufwandskonto 5100.
 ...
 * USt-S&auml;tzen Beispiel: Eingangsrechnung &uuml;ber brutto EUR 60.000,--, am Konto
 * 5100 sind netto 20.000,-- + 20 % USt (4.000,---) , am Konto 5200 netto
 ...
 * 20;1;0;3;;;;Split-ER mit;gleichen USt-S&auml;tzen; 2.3.10.2. ER-Splitbuchung mit
 * verschiedenen USt-S&auml;tzen Der Unterschied zur Splitbuchung mit gleichen
 * USt-S&auml;tzen liegt im Feld ???USt-Prozentsatz??? und ???USt-Code??? . In der
 * Split-Sammelbuchung sind diese Felder bei verschiedenen USt- S&auml;tzen Null.
 * Beispiel: Eingangsrechnung &uuml;ber brutto EUR 25.000,--, am Konto 5100 sind
 * netto 10.000,-- + 20 % USt, am Konto 5300 netto 11818.18 + 10 % USt zu
 * verbuchen. Split-Sammelbuchung:
 ...
 * Bauleistungen (ab 1. 10. 2002) Gem&auml;&szlig; ?? 19 1a USt geht die Steuerschuld auf
 * den Leistungsempf&auml;nger &uuml;ber. Von der Buchungssystematik &auml;hnlich wie eine
 * Eingangsrechnung mit Gegenbuchung, jedoch ist der Sollbetrag und der
 * Habenbetrag identisch und als Sondercode ist 15 zu setzen. Eingangsrechnung
 * &uuml;ber EUR 10.000,-- (brutto=netto). Rechnungsdatum 17. 10. 2002,
 * Rechnungsnummer 100, Lieferantenkonto 30100, Einkaufskonto 5110. Der
 * Rechnungsempf&auml;nger ist zum Vorsteuerabzug berechtigt.

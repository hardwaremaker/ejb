...
@NamedQueries( { @NamedQuery(name = "MahntextfindByMandantLocaleCNr", query = "SELECT OBJECT (o) FROM Mahntext o WHERE o.mandantCNr=?1 AND o.localeCNr=?2 AND o.mahnstufeIId=?3") })
@Entity
@Table(name = "FB_MAHNTEXT")
public class Mahntext implements Serializable {
	@Id
	@Column(name = "I_ID")
	private Integer iId;
	...
	/*@ManyToOne
	@JoinColumns( {
			@JoinColumn(name = "mahnstufe_i_id", referencedColumnName = "i_id"),
			@JoinColumn(name = "mandant_c_nr", referencedColumnName = "mandant_c_nr") })
	private Mahnstufe fbMahnstufe;*/
	...
	public Mahntext(Integer id, String mandantCNr, String localeCNr,
			Integer mahnstufeIId, String textinhalt) {
		setIId(id);

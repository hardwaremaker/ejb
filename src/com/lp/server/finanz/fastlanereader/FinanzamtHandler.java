...
/**
 * <p>
 * Hier wird die FLR Funktionalitaet fuer die Finanzaemter implementiert. Pro
...
public class FinanzamtHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRFinanzFinanzamt finanzamt = (FLRFinanzFinanzamt) resultListIterator
						.next();
				...
				if (partner.getFlrlandplzort() != null) {
					...
					rows[row][col++] = lplzort.getC_plz()
							+ " "
							+ lplzort.getFlrort().getC_name();
...
//					.getFlrland().getC_lkz();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" finanzamt." + filterKriterien[i].kritName);
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("finanzamt." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("finanzamt.id ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("finanzamt.id") < 0) {
	...
	private String getFromClause() {
		return "from FLRFinanzFinanzamt finanzamt ";
	}
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							"LKZ",
							getTextRespectUISpr("lp.steuernummer", mandantCNr,
									locUI),
							getTextRespectUISpr("lp.referat", mandantCNr, locUI) },
					new String[] {
							FinanzFac.FLR_FINANZAMT_I_ID,
							FinanzFac.FLR_FINANZAMT_FLRPARTNER
									+ "."
							...
							FinanzFac.FLR_FINANZAMT_C_STEUERNUMMER,
							FinanzFac.FLR_FINANZAMT_C_REFERAT }));
		}

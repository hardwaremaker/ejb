...
/**
 * <p>
 * Hier wird die FLR Funktionalitaet fuer die Buchungsjournale implementiert.
...
public class ExportlaufHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRFinanzExportlauf buchungsjournal = (FLRFinanzExportlauf) resultListIterator
						.next();
				...
				if (buchungsjournal.getExportdatenset().size() > 0) {
					...
					// Belegs
					for (Iterator<?> iter = buchungsjournal.getExportdatenset()
							.iterator(); iter.hasNext();) {
						FLRFinanzExportdaten item = (FLRFinanzExportdaten) iter
								.next();

						sBelegart = item.getBelegart_c_nr();
						Set<?> sprSet = item.getFlrbelegart()
				...
				rows[row][col++] = new Integer(buchungsjournal
						.getExportdatenset().size());
				row++;
				col = 0;
			}
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" exportlauf." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("exportlauf.t_aendern DESC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("exportlauf.t_aendern") < 0) {
				...
				orderBy.append(" exportlauf.t_aendern ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRFinanzExportlauf exportlauf ";
	}
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					new Class[] { Integer.class, Timestamp.class, Date.class,
					...
					new String[] {
							...
							getTextRespectUISpr("lp.stichtag", mandantCNr,
									locUI),
							getTextRespectUISpr("bes.belegart2", mandantCNr,
									locUI),
					...
					new String[] { FinanzFac.FLR_EXPORTLAUF_I_ID,
							FinanzFac.FLR_EXPORTLAUF_T_AENDERN,
							FinanzFac.FLR_EXPORTLAUF_T_STICHTAG,

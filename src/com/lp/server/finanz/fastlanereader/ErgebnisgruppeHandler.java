...
/**
 * <p>
 * Hier wird die FLR Funktionalitaet fuer die Ergebnisgruppen implementiert. Pro
...
public class ErgebnisgruppeHandler extends UseCaseHandler {
	...
	private final static int INDEX_BEZ = 1;
	private final static int INDEX_SUMMENGRUPPE = 2;
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRFinanzErgebnisgruppe ergebnisgruppe = (FLRFinanzErgebnisgruppe) resultListIterator
						.next();
				...
				if (ergebnisgruppe.getFlrergebnisgruppe_summe() != null) {
					rows[row][INDEX_SUMMENGRUPPE] = ergebnisgruppe
							.getFlrergebnisgruppe_summe().getC_bez();
				} else {
					rows[row][INDEX_SUMMENGRUPPE] = "";
				}
				rows[row][INDEX_SUMMENNEG] = Helper
						.short2Boolean(ergebnisgruppe.getB_summe_negativ());
				rows[row][INDEX_INVERTIERT] = Helper
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("ergebnisgruppe."
						+ FinanzFac.FLR_ERGEBNISGRUPPE_I_REIHUNG + " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("ergebnisgruppe."
					+ FinanzFac.FLR_ERGEBNISGRUPPE_I_REIHUNG) < 0) {
				...
				orderBy.append(" ergebnisgruppe."
						+ FinanzFac.FLR_ERGEBNISGRUPPE_I_REIHUNG);
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRFinanzErgebnisgruppe ergebnisgruppe ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select ergebnisgruppe.i_id from FLRFinanzErgebnisgruppe ergebnisgruppe "
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							getTextRespectUISpr("lp.bezeichnung", mandantCNr,
									locUI),
							getTextRespectUISpr("fb.summengruppe", mandantCNr,
									locUI),
							getTextRespectUISpr("fb.vorzeichen", mandantCNr,
									locUI),
					...
					new String[] {
							...
							FinanzFac.FLR_ERGEBNISGRUPPE_FLRERGEBNISGRUPPE_SUMME
									+ "." + FinanzFac.FLR_ERGEBNISGRUPPE_C_BEZ,

...
/**
 * <p>
 * Hier wird die FLR Funktionalitaet fuer die Buchungen implementiert. Pro
...
public class BuchungHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRFinanzBuchung buchung = (FLRFinanzBuchung) resultListIterator
						.next();
				...
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].kritName.equals("c_belegnummer")) {
						...
						if (filterKriterien[i].isBIgnoreCase()) {
							...
							where.append(" buchung." + filterKriterien[i].kritName);
						}
						where.append(" " + filterKriterien[i].operator);
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("buchung." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("buchung.id ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("buchung.id") < 0) {
	...
	private String getFromClause() {
		return "from FLRFinanzBuchung buchung ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select buchung.id from FLRFinanzBuchung buchung "
			...
			setTableInfo(
				new TableInfo(
					...
					new String[] {
							...
							getTextRespectUISpr("lp.buchungsdatum", mandantCNr, locUI),
							getTextRespectUISpr("lp.buchungtypeb", mandantCNr, locUI),
							getTextRespectUISpr("lp.buchungsart", mandantCNr, locUI),
							getTextRespectUISpr("bes.belegnummer", mandantCNr, locUI),
					...
					new int[] {
							...
							QueryParameters.FLR_BREITE_S },
					new String[] { FinanzFac.FLR_BUCHUNG_I_ID,

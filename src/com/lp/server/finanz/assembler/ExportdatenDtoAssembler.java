...
public class ExportdatenDtoAssembler {
	public static ExportdatenDto createDto(Exportdaten exportdaten) {
		ExportdatenDto exportdatenDto = new ExportdatenDto();
		if (exportdaten != null) {
			exportdatenDto.setIId(exportdaten.getIId());
			exportdatenDto.setExportlaufIId(exportdaten.getExportlaufIId());
			exportdatenDto.setBelegartCNr(exportdaten.getBelegartCNr());
			exportdatenDto.setIBelegiid(exportdaten.getIBelegiid());
		}
		return exportdatenDto;
	}

	public static ExportdatenDto[] createDtos(Collection<?> exportdatens) {
		List<ExportdatenDto> list = new ArrayList<ExportdatenDto>();
		if (exportdatens != null) {
			Iterator<?> iterator = exportdatens.iterator();
			while (iterator.hasNext()) {
				list.add(createDto((Exportdaten) iterator.next()));
			}
		}

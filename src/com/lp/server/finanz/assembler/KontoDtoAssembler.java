...
public class KontoDtoAssembler {
	public static KontoDto createDto(Konto konto) {
		...
		if (konto != null) {
			...
			kontoDto.setKontoIIdWeiterfuehrendUst(konto.getKontoIIdWeiterfuehrendust());
			kontoDto.setRechenregelCNrWeiterfuehrendUst(konto.getRechenregelCNrWeiterfuehrendust());
			kontoDto.setRechenregelCNrWeiterfuehrendBilanz(konto.getRechenregelCNrWeiterfuehrendbilanz());
			kontoDto.setKontoIIdWeiterfuehrendSkonto(konto.getKontoIIdWeiterfuehrendskonto());
			kontoDto.setRechenregelCNrWeiterfuehrendSkonto(konto.getRechenregelCNrWeiterfuehrendskonto());
			...
			kontoDto.setBAutomeroeffnungsbuchung(konto.getBAutomeroeffnungsbuchung());
			...
			kontoDto.setILetzteselektiertebuchung(konto.getILetzteselektiertebuchung());
...
//					.getSteuerkategorieIIdReverse());
			...
			kontoDto.setErgebnisgruppeIId_negativ(konto.getErgebnisgruppeIId_negativ());
			kontoDto.setBOhneUst(konto.getBOhneUst());
			kontoDto.setcSteuerart(konto.getcSteuerart());
			kontoDto.setSteuerkategorieCnr(konto.getSteuerkategorieCNr());
			kontoDto.setMwstsatzIId(konto.getMwstsatzIId());
		}
		return kontoDto;
	}

	public static KontoDto[] createDtos(Collection<?> kontos) {
		List<KontoDto> list = createDtoList(kontos);
...
//		return (KontoDto[]) list.toArray(returnArray);
	...
	public static List<KontoDto> createDtoList(Collection<?> kontos) {
		List<KontoDto> list = new ArrayList<KontoDto>();
		if (kontos != null) {
			Iterator<?> iterator = kontos.iterator();
			while (iterator.hasNext()) {

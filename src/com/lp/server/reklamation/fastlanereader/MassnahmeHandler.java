...
import com.lp.server.reklamation.fastlanereader.generated.FLRMassnahme;
import com.lp.server.reklamation.fastlanereader.generated.FLRMassnahmespr;
 ...
 * @version 1.0
 */
public class MassnahmeHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRMassnahme massnahme = (FLRMassnahme) o[0];
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("massnahme.c_bez ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("massnahme.c_bez ") < 0) {
				...
				orderBy.append(" massnahme.c_bez ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRMassnahme massnahme LEFT JOIN massnahme.sprset AS sprset ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select massnahme.i_id from FLRMassnahme massnahme  LEFT JOIN massnahme.sprset AS sprset "
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRMassnahmespr spr = (FLRMassnahmespr) iterUebersetzungenI.next();
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] { "massnahme.id", "massnahme.c_bez", "sprset.c_bez" }));
		}

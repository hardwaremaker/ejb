...
import com.lp.server.reklamation.fastlanereader.generated.FLRWirksamkeit;
import com.lp.server.reklamation.fastlanereader.generated.FLRWirksamkeitspr;
 ...
 * @version 1.0
 */
public class WirksamkeitHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRWirksamkeit wirksamkeit = (FLRWirksamkeit) o[0];
				rows[row][col++] = wirksamkeit.getI_id();
				rows[row][col++] = wirksamkeit.getC_bez();
				Iterator<?> sprsetIterator = wirksamkeit.getSprset()
				.iterator();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRWirksamkeitspr spr = (FLRWirksamkeitspr) iterUebersetzungenI
					.next();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("wirksamkeit.c_bez ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("wirksamkeit.c_bez ") < 0) {
				...
				orderBy.append(" wirksamkeit.c_bez ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRWirksamkeit wirksamkeit  LEFT JOIN wirksamkeit.sprset AS sprset";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				session = setFilter(session);
				String queryString = "select wirksamkeit.i_id from FLRWirksamkeit wirksamkeit LEFT JOIN wirksamkeit.sprset AS sprset"
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] { "wirksamkeit.id","wirksamkeit.c_bez", "sprset.c_bez" }));
		}

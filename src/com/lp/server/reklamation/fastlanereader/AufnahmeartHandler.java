...
import com.lp.server.reklamation.fastlanereader.generated.FLRAufnahmeart;
import com.lp.server.reklamation.fastlanereader.generated.FLRAufnahmeartspr;
 ...
 * @version 1.0
 */
public class AufnahmeartHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRAufnahmeart aufnahmeart = (FLRAufnahmeart) o[0];
				rows[row][col++] = aufnahmeart.getI_id();
				rows[row][col++] = aufnahmeart.getC_bez();
				Iterator<?> sprsetIterator = aufnahmeart.getSprset()
				.iterator();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRAufnahmeartspr spr = (FLRAufnahmeartspr) iterUebersetzungenI
					.next();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("aufnahmeart.c_bez ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("aufnahmeart.c_bez ") < 0) {
				...
				orderBy.append(" aufnahmeart.c_bez ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRAufnahmeart aufnahmeart  LEFT JOIN aufnahmeart.sprset AS sprset";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				session = setFilter(session);
				String queryString = "select aufnahmeart.i_id from FLRAufnahmeart aufnahmeart  LEFT JOIN aufnahmeart.sprset AS sprset"
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] { "aufnahmeart.id","aufnahmeart.c_bez", "sprset.c_bez" }));
		}

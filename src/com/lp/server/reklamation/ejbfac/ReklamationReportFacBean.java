...
public class ReklamationReportFacBean extends LPReport implements
		ReklamationReportFac {
	...
	private static int REPORT_REKLAMATIONSJOURNAL_ANZAHL_SPALTEN = 68;
	...
	private static int REPORT_LIEFERANTENTERMINTREUE_MENGE = 8;
	...
	private static int REPORT_LIEFERANTENBEURTEILUNG_ANZAHL_SPALTEN = 23;
	...
	private static int REPORT_FEHLERART_ANZAHL_SPALTEN = 7;
	...
	private static int REPORT_OFFENEREKLAMATIONENEINESARTIKEL_ANZAHL_SPALTEN = 6;

	@TransactionAttribute(TransactionAttributeType.NEVER)
	public JasperPrintLP printLieferantenbeurteilung(Timestamp tVon,
			Timestamp tBis, Integer lieferantIId, Integer brancheIId,
		...
		tBis = Helper
				.cutTimestamp(new Timestamp(tBis.getTime() + 24 * 3600000));
		...
		while (resultListIterator.hasNext()) {
			...
			data[row][REPORT_LIEFERANTENBEURTEILUNG_LIEFERANT] = wareneingangspositionen
					.getFlrwareneingang().getFlrbestellung().getFlrlieferant()
			...
			data[row][REPORT_LIEFERANTENBEURTEILUNG_LIEFERANT_KBEZ] = wareneingangspositionen
					.getFlrwareneingang().getFlrbestellung().getFlrlieferant()
					.getFlrpartner().getC_kbez();
			if (wareneingangspositionen.getFlrwareneingang().getFlrbestellung()
					.getFlrlieferant().getFlrpartner().getFlrlandplzort() != null) {
			...
			Timestamp weDatum = Helper.cutTimestamp(new Timestamp(
					wareneingangspositionen.getFlrwareneingang()
							.getT_wareneingansdatum().getTime()));

			data[row][REPORT_LIEFERANTENBEURTEILUNG_WARENEINGANGSDATUM] = weDatum;
			data[row][REPORT_LIEFERANTENBEURTEILUNG_WEIID] = wareneingangspositionen
					.getFlrwareneingang().getI_id();
			data[row][REPORT_LIEFERANTENBEURTEILUNG_BESTELLNUMMER] = wareneingangspositionen
					.getFlrwareneingang().getFlrbestellung().getC_nr();
			data[row][REPORT_LIEFERANTENBEURTEILUNG_WELIEFERSCHEIN] = wareneingangspositionen
					.getFlrwareneingang().getC_lieferscheinnr();
			data[row][REPORT_LIEFERANTENBEURTEILUNG_MENGE] = wareneingangspositionen
					.getN_geliefertemenge();
			data[row][REPORT_LIEFERANTENBEURTEILUNG_ARTIKELNUMMER] = wareneingangspositionen
					.getFlrbestellposition().getFlrartikel().getC_nr();

			hmLieferanten.put(wareneingangspositionen.getFlrwareneingang()
				...
				LieferantbeurteilungDto[] lbDtos = getLieferantFac()
						.lieferantbeurteilungfindByLetzteBeurteilungByLieferantIId(
								wareneingangspositionen.getFlrwareneingang()
										...
										.getI_id(), tBeurteilungsdatum);
				if (lbDtos != null && lbDtos.length > 0) {
					data[row][REPORT_LIEFERANTENBEURTEILUNG_KLASSE_BISHER] = lbDtos[0]
							.getCKlasse();
					data[row][REPORT_LIEFERANTENBEURTEILUNG_PUNKTE_BISHER] = lbDtos[0]
							.getIPunkte();
				}
			} catch (RemoteException e1) {
				throwEJBExceptionLPRespectOld(e1);
			...
			ArtikelDto artikelDto = getArtikelFac()
					.artikelFindByPrimaryKeySmall(
							wareneingangspositionen.getFlrbestellposition()
									.getFlrartikel().getI_id(), theClientDto);
			data[row][REPORT_LIEFERANTENBEURTEILUNG_BEZEICHNUNG] = artikelDto
					.formatBezeichnung();
			...
			if (sollTermin == null
					&& wareneingangspositionen.getFlrwareneingang()
							.getFlrbestellung().getT_liefertermin() != null) {
				sollTermin = wareneingangspositionen.getFlrwareneingang()
						.getFlrbestellung().getT_liefertermin();
			}
			...
			if(!hmWareneingaengeAbgehandelt.containsKey(wareneingangspositionen
				...
				try {
					...
					if (reklamationDto != null && reklamationDto.length > 0) {
						...
						data[row][REPORT_LIEFERANTENBEURTEILUNG_REKLAMATIONSNUMMER] = reklamationDto[0]
								.getCNr();

						if (reklamationDto[0].getSchwereIId() != null) {
							iPunkteSchwere = getReklamationFac()
									.schwereFindByPrimaryKey(
											reklamationDto[0].getSchwereIId())
									.getIPunkte();
						}
			...
			if ((iLetzterLieferant != -1 && (iLetzterLieferant != wareneingangspositionen
				...
				if (iAnzahlWareneingaengeProLieferant != 0) {
					iPunkte = iAnzahlPunkteProLieferant
							/ iAnzahlWareneingaengeProLieferant;
				}
				...
				try {
					LieferantbeurteilungDto dto = getLieferantFac()
							.lieferantbeurteilungfindByLieferantIIdTDatum(
					...
					try {
						getLieferantFac().updateLieferantbeurteilung(dto,
								theClientDto);
					} catch (RemoteException e) {
					...
					try {
						getLieferantFac().createLieferantbeurteilung(lbDto,
								theClientDto);
			...
			iLetzterLieferant = wareneingangspositionen.getFlrbestellposition()
					.getFlrbestellung().getFlrlieferant().getI_id();
			row++;
		}

		initJRDS(parameter, ReklamationReportFac.REPORT_MODUL,
				ReklamationReportFac.REPORT_LIEFERANTENBEURTEILUNG,
				theClientDto.getMandant(), theClientDto.getLocUi(),
				theClientDto);

		JasperPrintLP print = getReportPrint();

		print.putAdditionalInformation("LIEFERANTEN", (Integer[]) hmLieferanten
				.keySet().toArray(new Integer[hmLieferanten.keySet().size()]));
	...
	public JasperPrintLP printOffeneReklamationenEinesArtikels(
		...
		org.hibernate.Criteria crit = session
				.createCriteria(FLRReklamation.class);
		...
		crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRARTIKEL, "a",
		...
		int row = 0;
		while (resultListIterator.hasNext()) {
			FLRReklamation flrReklamation = (FLRReklamation) resultListIterator
					.next();
			data[row][REPORT_OFFENEREKLAMATIONENEINESARTIKEL_ART] = flrReklamation
					.getReklamationart_c_nr();
			data[row][REPORT_OFFENEREKLAMATIONENEINESARTIKEL_BELEGNUMMER] = flrReklamation
					.getC_nr();
			data[row][REPORT_OFFENEREKLAMATIONENEINESARTIKEL_DATUM] = new java.sql.Timestamp(
					flrReklamation.getT_belegdatum().getTime());
			data[row][REPORT_OFFENEREKLAMATIONENEINESARTIKEL_GRUND] = flrReklamation
					.getC_grund();

			if (flrReklamation.getKunde_i_id() != null) {
				KundeDto kundeDto = getKundeFac().kundeFindByPrimaryKey(
						flrReklamation.getKunde_i_id(), theClientDto);
				data[row][REPORT_OFFENEREKLAMATIONENEINESARTIKEL_KUNDE] = kundeDto
			...
			if (flrReklamation.getLieferant_i_id() != null) {
				LieferantDto lieferantDto = getLieferantFac()
						.lieferantFindByPrimaryKey(
								flrReklamation.getLieferant_i_id(),
								theClientDto);
				data[row][REPORT_OFFENEREKLAMATIONENEINESARTIKEL_LIEFERANT] = lieferantDto
						.getPartnerDto().formatFixTitelName1Name2();
			}
		}
		...
		initJRDS(parameter, ReklamationReportFac.REPORT_MODUL,
				ReklamationReportFac.REPORT_OFFENEREKLAMATIONENEINESARTIKEL,
				theClientDto.getMandant(), theClientDto.getLocUi(),
				theClientDto);
	...
	public JasperPrintLP printLieferantentermintreue(Timestamp tVon,
		...
		if (lieferantIId != null) {
			...
			parameter.put("P_LIEFERANT", lieferantDto.getPartnerDto()
		...
		while (resultListIterator.hasNext()) {
			...
			data[row][REPORT_LIEFERANTENTERMINTREUE_LIEFERANT] = wareneingangspositionen
					.getFlrwareneingang().getFlrbestellung().getFlrlieferant()
					.getFlrpartner().getC_name1nachnamefirmazeile1();
			...
			data[row][REPORT_LIEFERANTENTERMINTREUE_BESTELLNUMMER] = wareneingangspositionen
					.getFlrwareneingang().getFlrbestellung().getC_nr();
			data[row][REPORT_LIEFERANTENTERMINTREUE_WELIEFERSCHEIN] = wareneingangspositionen
					.getFlrwareneingang().getC_lieferscheinnr();
			data[row][REPORT_LIEFERANTENTERMINTREUE_MENGE] = wareneingangspositionen
					.getN_geliefertemenge();
			data[row][REPORT_LIEFERANTENTERMINTREUE_ARTIKELNUMMER] = wareneingangspositionen
					.getFlrbestellposition().getFlrartikel().getC_nr();
			...
			ArtikelDto artikelDto = getArtikelFac()
					.artikelFindByPrimaryKeySmall(
							wareneingangspositionen.getFlrbestellposition()
									.getFlrartikel().getI_id(), theClientDto);
			data[row][REPORT_LIEFERANTENTERMINTREUE_BEZEICHNUNG] = artikelDto
					.formatBezeichnung();
			...
			data[row][REPORT_LIEFERANTENTERMINTREUE_VERSPAETUNG] = getVerspaetungstage(
					weDatum, sollTermin);
		...
		initJRDS(parameter, ReklamationReportFac.REPORT_MODUL,
				ReklamationReportFac.REPORT_LEFERANTENTERMINTREUE,
				theClientDto.getMandant(), theClientDto.getLocUi(),
				theClientDto);
	...
	private Integer getPunkteTermin(Integer iTageAbweichung,
			TermintreueDto[] dtos) {
	...
	public JasperPrintLP printReklamation(Integer reklamationIId,
			boolean druckeUnterartLieferant, TheClientDto theClientDto) {
		...
		try {
			...
			if (reklamationDto.getSchwereIId() != null) {
				schwere = getReklamationFac().schwereFindByPrimaryKey(
						reklamationDto.getSchwereIId()).formatBezeichnung();
			}
			...
			if (reklamationDto.getMaschineIId() != null) {
				...
				parameter.put("P_MASCHINE_IDENTIFIKATIONSNUMMER",
						maschineDto.getCIdentifikationsnr());
				parameter.put("P_MASCHINE_INVENTARNUMMER",
						maschineDto.getCInventarnummer());
			}

			parameter.put("P_BELEGNUMMER", reklamationDto.getCNr());
			parameter.put("P_SCHWERE", schwere);
			...
			parameter.put("P_KOSTENMATERIAL",
					reklamationDto.getNKostenmaterial());
			...
			parameter
					.put("P_RUECKSPRACHEAM", reklamationDto.getTRuecksprache());
			parameter.put("P_RUECKSPRACHEMIT",
					reklamationDto.getCRuecksprachemit());
			parameter.put("P_REKLAMATIONSART",
					reklamationDto.getReklamationartCNr());
			parameter.put("P_BERECHTIGT",
					Helper.short2Boolean(reklamationDto.getBBerechtigt()));
			parameter.put("P_FREMDPRODUKT",
					Helper.short2Boolean(reklamationDto.getBFremdprodukt()));
			parameter.put("P_MASSNAHMEKURZEINGEFUEHRT",
					reklamationDto.getTEingefuehrtkurz());
			parameter.put("P_MASSNAHMEMITTELEINGEFUEHRT",
					reklamationDto.getTEingefuehrtmittel());
			parameter.put("P_MASSNAHMELANGEINGEFUEHRT",
					reklamationDto.getTEingefuehrtlang());
			parameter.put("P_MASSNAHMEKURZKOMMENTAR",
					reklamationDto.getXMassnahmeKurz());
			parameter.put("P_MASSNAHMEMITTELKOMMENTAR",
					reklamationDto.getXMassnahmeMittel());
			parameter.put("P_MASSNAHMELANGKOMMENTAR",
					reklamationDto.getXMassnahmeLang());
			parameter.put("P_MASSNAHMEKURZGEPLANT",
					reklamationDto.getTMassnahmebiskurz());
			parameter.put("P_MASSNAHMEMITTELGEPLANT",
					reklamationDto.getTMassnahmebismittel());
			parameter.put("P_MASSNAHMELANGGEPLANT",
					reklamationDto.getTMassnahmebislang());

			parameter.put("P_WIRKSAMKEITGEPLANT",
					reklamationDto.getTWirksamkeitbis());
			parameter.put("P_WIRKSAMKEITDURCHGEFUEHRT",
					reklamationDto.getTWirksamkeiteingefuehrt());

			parameter.put("P_BETRIFFTLAGERSTAND", Helper
					.short2Boolean(reklamationDto.getBBetrifftlagerstand()));
			parameter.put("P_BETRIFFTGELIEFERTE", Helper
					.short2Boolean(reklamationDto.getBBetrifftgelieferte()));

			parameter.put("P_STUECKLAGERSTAND",
					reklamationDto.getNStuecklagerstand());
			parameter.put("P_STUECKGELIEFERTE",
					reklamationDto.getNStueckgelieferte());

			if (Helper.short2boolean(reklamationDto.getBArtikel()) == true
					&& reklamationDto.getArtikelIId() != null) {
				ArtikelDto artikelDto = getArtikelFac()
						.artikelFindByPrimaryKeySmall(
								reklamationDto.getArtikelIId(), theClientDto);
			...
			if (reklamationDto.getPersonalIIdAnlegen() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								...
								theClientDto);
				parameter.put("P_PERSONANGELEGT", personalDto.getPartnerDto()
						.formatTitelAnrede());
			}
			...
			if (reklamationDto.getPersonalIIdAufnehmer() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								...
								theClientDto);
				parameter.put("P_PERSONAUFNEHMER", personalDto.getPartnerDto()
						.formatTitelAnrede());
			}
			if (reklamationDto.getPersonalIIdEingefuehrtkurz() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								...
								theClientDto);
				parameter.put("P_PERSONMASSNAHMEKURZ", personalDto
			...
			if (reklamationDto.getPersonalIIdEingefuehrtmittel() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								...
								theClientDto);
				parameter.put("P_PERSONMASSNAHMEMITTEL", personalDto
						.getPartnerDto().formatTitelAnrede());
			}
			if (reklamationDto.getPersonalIIdEingefuehrtlang() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								...
								theClientDto);
				parameter.put("P_PERSONMASSNAHMELANG", personalDto
						.getPartnerDto().formatTitelAnrede());
			}
			if (reklamationDto.getPersonalIIdRuecksprache() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								...
								theClientDto);
				parameter.put("P_PERSONRUECKSPRACHE", personalDto
						.getPartnerDto().formatTitelAnrede());
			}
			...
			if (reklamationDto.getPersonalIIdWirksamkeit() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								reklamationDto.getPersonalIIdWirksamkeit(),
								theClientDto);
				parameter.put("P_PERSONWIRKSAMKEIT", personalDto
						.getPartnerDto().formatTitelAnrede());
			}
			...
			if (reklamationDto.getPersonalIIdErledigt() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								reklamationDto.getPersonalIIdErledigt(),
								theClientDto);
				parameter.put("P_PERSONERLEDIGT", personalDto.getPartnerDto()
						.formatTitelAnrede());
			}
			...
			if (reklamationDto.getReklamationartCNr().equals(
				...
				if (reklamationDto.getLosIId() != null) {
					...
					if (losDto.getAuftragIId() != null) {
						...
						parameter.put("P_KUNDELIEFERANT",
								beleg.getKundeLieferant());
				...
				if (reklamationDto.getPersonalIIdVerursacher() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									...
									theClientDto);
					parameter.put("P_PERSONVERURSACHER", personalDto
							.getPartnerDto().formatTitelAnrede());
				}
			...
			if (reklamationDto.getReklamationartCNr().equals(
				...
				if (reklamationDto.getLieferantIId() != null) {
					...
					if (druckeUnterartLieferant == true) {
						locDruck = Helper.string2Locale(lieferantDto
					...
					if (lieferantDto != null) {
						loc = Helper.string2Locale(lieferantDto.getPartnerDto()
				...
				if (reklamationDto.getBestellungIId() != null) {
					BelegInfos beleg = getLagerFac().getBelegInfos(
							...
							reklamationDto.getBestellungIId(), null,
							theClientDto);
				...
				if (reklamationDto.getKundeIId() != null) {
					...
					partnerDto = kundeDto.getPartnerDto();
					parameter.put("P_KUNDELIEFERANT", kundeDto.getPartnerDto()
							.formatFixTitelName1Name2());
				...
				if (reklamationDto.getLieferscheinIId() != null) {
					BelegInfos beleg = getLagerFac().getBelegInfos(
							...
							reklamationDto.getLieferscheinIId(), null,
							theClientDto);
					parameter.put("P_REKLAMATIONSBELEG",
							"LS" + beleg.getBelegnummer());
				}
				if (reklamationDto.getRechnungIId() != null) {
					BelegInfos beleg = getLagerFac()
							.getBelegInfos(LocaleFac.BELEGART_RECHNUNG,
									reklamationDto.getRechnungIId(), null,
									theClientDto);
					parameter.put("P_REKLAMATIONSBELEG2",
							"RE" + beleg.getBelegnummer());
				}
				...
				if (reklamationDto.getWareneingangIId() != null) {
					WareneingangDto weDto = getWareneingangFac()
							.wareneingangFindByPrimaryKey(
									reklamationDto.getWareneingangIId());
					parameter.put("P_REKLAMATIONSBELEG_ZUSATZ",
							weDto.getCLieferscheinnr());
				}
			...
			if (reklamationDto.getFehlerIId() != null) {
				parameter.put(
						"P_FEHLER",
						getReklamationFac().fehlerFindByPrimaryKey(
								reklamationDto.getFehlerIId(), theClientDto)
								.getBezeichnung());
			}
			if (reklamationDto.getAufnahmeartIId() != null) {
				parameter.put(
						"P_AUFNAHMEART",
						getReklamationFac().aufnahmeartFindByPrimaryKey(
			...
			if (reklamationDto.getWirksamkeitIId() != null) {
				parameter.put(
						"P_WIRKSAMKEIT",
						getReklamationFac().wirksamkeitFindByPrimaryKey(
								reklamationDto.getWirksamkeitIId(),
								theClientDto).getBezeichnung());
			}

			parameter.put("P_WIRKSAMKEITKOMMENTAR",
					reklamationDto.getXWirksamkeit());

			if (reklamationDto.getFehlerangabeIId() != null) {
				parameter.put(
						"P_FEHLERANGABE",
						getReklamationFac().fehlerangabeFindByPrimaryKey(
			...
			if (reklamationDto.getMassnahmeIIdKurz() != null) {
				parameter.put(
						"P_MASSNAHMEKURZ",
						getReklamationFac().massnahmeFindByPrimaryKey(
			...
			parameter.put("P_TELEFON_ANSPRECHPARTNER",
					reklamationDto.getCTelansprechpartner());
			parameter.put("P_TELEFON_ANSPRECHPARTNER_LIEFERANT",
					reklamationDto.getCTelansprechpartnerLieferant());
			if (partnerDto != null) {
				loc = Helper.string2Locale(partnerDto
						.getLocaleCNrKommunikation());
				parameter.put(
						"P_KUNDE_ADRESSBLOCK",
						formatAdresseFuerAusdruck(
		...
		try {
			ReklamationbildDto[] dtos = getReklamationFac()
					.reklamationbildFindByReklamationIId(reklamationIId);
		...
		initJRDS(parameter, ReklamationReportFac.REPORT_MODUL,
				sAktuellerReport, theClientDto.getMandant(), locDruck,
				theClientDto);
	...
	public JasperPrintLP printFehlerarten(java.sql.Timestamp tVon,
		...
		crit.add(Restrictions.ge(ReklamationFac.FLR_REKLAMATION_T_BELEGDATUM,
				tVon));
		crit.add(Restrictions.lt(ReklamationFac.FLR_REKLAMATION_T_BELEGDATUM,
				tBis));

		crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRLIEFERANT, "l",
				CriteriaSpecification.LEFT_JOIN);
		crit.createAlias(ReklamationFac.FLR_REKLAMATION_FRLKUNDE, "k",
				CriteriaSpecification.LEFT_JOIN);

		crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRLOS, "lo",
				CriteriaSpecification.LEFT_JOIN);
		crit.createAlias("lo." + FertigungFac.FLR_LOSREPORT_FLRAUFTRAG, "au",
				CriteriaSpecification.LEFT_JOIN);

		crit.createAlias("l." + LieferantFac.FLR_PARTNER, "pl",
				CriteriaSpecification.LEFT_JOIN);
		crit.createAlias("k." + KundeFac.FLR_PARTNER, "kl",
				CriteriaSpecification.LEFT_JOIN);
		crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRFEHLERANGABE, "f",
				CriteriaSpecification.LEFT_JOIN);

		if (bNurBerechtigte == true) {
			crit.add(Restrictions.eq(
					ReklamationFac.FLR_REKLAMATION_B_BERECHTIGT,
		...
		if (kundeIId != null) {
			crit.add(Restrictions.or(Restrictions.eq(
					"au.kunde_i_id_auftragsadresse", kundeIId), Restrictions
					.eq(ReklamationFac.FLR_REKLAMATION_KUNDE_I_ID, kundeIId)));
		...
		if (iGruppierung == ReklamationReportFac.SORTIERUNG_FEHLERART_MASCHINENGRUPPE
			...
			crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRMASCHINE,
					"masch", CriteriaSpecification.LEFT_JOIN);
			crit.createAlias("masch."
					+ ZeiterfassungFac.FLR_MASCHINE_FLR_MASCHINENGRUPPE,
					"mgru", CriteriaSpecification.LEFT_JOIN);
		...
		if (iGruppierung == ReklamationReportFac.SORTIERUNG_FEHLERART_MITARBEITER
				|| iGruppierung == ReklamationReportFac.SORTIERUNG_FEHLERART_MASCHINENGRUPPE_MITARBEITER) {
			crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRVERURSACHER,
					"ver", CriteriaSpecification.LEFT_JOIN);
			crit.createAlias("ver." + PersonalFac.FLR_PERSONAL_FLRPARTNER,
					"part", CriteriaSpecification.LEFT_JOIN);
			crit.addOrder(Order.asc("part."
					+ PartnerFac.FLR_PARTNER_C_NAME1NACHNAMEFIRMAZEILE1));
			crit.addOrder(Order.asc("part."
					+ PartnerFac.FLR_PARTNER_C_NAME2VORNAMEFIRMAZEILE2));
		}
		...
		if (iGruppierung == ReklamationReportFac.SORTIERUNG_FEHLERART_FEHLERART) {
			parameter
					.put("P_SORTIERUNG",
							getTextRespectUISpr("rekla.fehlerart",
									theClientDto.getMandant(),
			...
			parameter
					.put("P_SORTIERUNG",
							getTextRespectUISpr("rekla.verursacher",
									theClientDto.getMandant(),
									theClientDto.getLocUi()));
		...
		while (resultListIterator.hasNext()) {
			...
			data[row][REPORT_FEHLERART_FEHLERART] = flrReklamation
					.getFlrfehlerangabe().getC_bez();
			data[row][REPORT_FEHLERART_REKLAMATIONSNUMMER] = flrReklamation
					.getC_nr();
			data[row][REPORT_FEHLERART_REKLAMATIONSART] = flrReklamation
					.getReklamationart_c_nr();
			data[row][REPORT_FEHLERART_MASCHINENGRUPPE] = "";

			if (flrReklamation.getFlrmaschine() != null) {
				data[row][REPORT_FEHLERART_MASCHINENGRUPPE] = flrReklamation
			...
			if (flrReklamation.getFlrverursacher() != null) {
				PersonalDto personalDto = getPersonalFac()
						.personalFindByPrimaryKey(
								flrReklamation.getFlrverursacher().getI_id(),
								theClientDto);

				data[row][REPORT_FEHLERART_VERURSACHER] = personalDto
						.formatAnrede();
			}
			data[row][REPORT_FEHLERART_KUNDE] = "";
			KundeDto kundeDto = null;
			if (flrReklamation.getReklamationart_c_nr().equals(
					ReklamationFac.REKLAMATIONART_KUNDE)
			...
			} else if (flrReklamation.getReklamationart_c_nr().equals(
					ReklamationFac.REKLAMATIONART_FERTIGUNG)
					&& flrReklamation.getFlrlos() != null
					&& flrReklamation.getFlrlos().getFlrauftrag() != null) {
				kundeDto = getKundeFac().kundeFindByPrimaryKey(
						flrReklamation.getFlrlos().getFlrauftrag()
								.getKunde_i_id_auftragsadresse(), theClientDto);
			}

			if (kundeDto != null) {
				data[row][REPORT_FEHLERART_KUNDE] = kundeDto.getPartnerDto()
						.formatTitelAnrede();
			}
		...
		initJRDS(parameter, ReklamationReportFac.REPORT_MODUL,
				ReklamationReportFac.REPORT_FEHLERART,
				theClientDto.getMandant(), theClientDto.getLocUi(),
				theClientDto);
	...
	public JasperPrintLP printMitarbeiterreklamation(java.sql.Timestamp tVon,
		...
		crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRVERURSACHER, "ver",
				CriteriaSpecification.LEFT_JOIN);
		crit.createAlias("ver." + PersonalFac.FLR_PERSONAL_FLRPARTNER, "part",
				CriteriaSpecification.LEFT_JOIN);
		crit.addOrder(Order.asc("part."
				+ PartnerFac.FLR_PARTNER_C_NAME1NACHNAMEFIRMAZEILE1));
		crit.addOrder(Order.asc("part."
		...
		while (resultListIterator.hasNext()) {
			...
			data[row][REPORT_MITARBEITERREKLAMATION_REKLAMATIONSNUMMER] = flrReklamation
					.getC_nr();
			data[row][REPORT_MITARBEITERREKLAMATION_REKLAMATIONSART] = flrReklamation
					.getReklamationart_c_nr();
			...
			if (flrReklamation.getFlrverursacher() != null) {
				...
				data[row][REPORT_MITARBEITERREKLAMATION_VERURSACHER] = personalDto
						.formatAnrede();
			}
			...
			if (kundeDto != null) {
				data[row][REPORT_MITARBEITERREKLAMATION_KUNDE] = kundeDto
						.getPartnerDto().formatTitelAnrede();
			}
	...
	public JasperPrintLP printMaschinenreklamation(java.sql.Timestamp tVon,
		...
		crit.createAlias(ReklamationFac.FLR_REKLAMATION_FLRMASCHINE, "masch",
				CriteriaSpecification.LEFT_JOIN);
		crit.createAlias("masch."
				+ ZeiterfassungFac.FLR_MASCHINE_FLR_MASCHINENGRUPPE, "mgru",
				CriteriaSpecification.LEFT_JOIN);
		...
		while (resultListIterator.hasNext()) {
			...
			data[row][REPORT_MASCHINENREKLAMATION_REKLAMATIONSNUMMER] = flrReklamation
					.getC_nr();
			data[row][REPORT_MASCHINENREKLAMATION_REKLAMATIONSART] = flrReklamation
					.getReklamationart_c_nr();

			if (flrReklamation.getFlrmaschine() != null) {
				data[row][REPORT_MASCHINENREKLAMATION_MASCHINENGRUPPE] = flrReklamation
						.getFlrmaschine().getFlrmaschinengruppe().getC_bez();
			}
			...
			if (flrReklamation.getFlrverursacher() != null) {
				...
				data[row][REPORT_MASCHINENREKLAMATION_VERURSACHER] = personalDto
						.formatAnrede();
			}
			...
			if (kundeDto != null) {
				data[row][REPORT_MASCHINENREKLAMATION_KUNDE] = kundeDto
						.getPartnerDto().formatTitelAnrede();
			}
		...
		initJRDS(parameter, ReklamationReportFac.REPORT_MODUL,
				ReklamationReportFac.REPORT_MASCHINENREKLAMATION,
				theClientDto.getMandant(), theClientDto.getLocUi(),
				theClientDto);
	...
	public JasperPrintLP printReklamationsjournal(Integer kostenstelleIId,
		...
		if (bFertigung == true || bKunde == true || bLieferant == true) {
			...
			if (bFertigung == true) {
				...
				sQuery += " ( r.reklamationart_c_nr='"
						+ ReklamationFac.REKLAMATIONART_FERTIGUNG
						+ "' OR (r.reklamationart_c_nr='"
						+ ReklamationFac.REKLAMATIONART_KUNDE
						+ "' AND r.i_kundeunterart="
						+ ReklamationFac.REKLAMATION_KUNDEUNTERART_FERTIGUNG
						+ "  )) ";
			...
			if (bLieferant == true) {
				sQuery += " ( r.reklamationart_c_nr='"
						...
						+ ReklamationFac.REKLAMATIONART_KUNDE
						+ "' AND r.i_kundeunterart="
						+ ReklamationFac.REKLAMATION_KUNDEUNTERART_LIEFERANT
						+ "  )) ";
		...
		if (iSortierung == ReklamationReportFac.SORTIERGUN_REKLAMATIONSJOURNAL_BELEGNR) {
			...
			parameter.put(
					"P_SORTIERUNG",
					getTextRespectUISpr("lp.fehler", theClientDto.getMandant(),
							theClientDto.getLocUi()));
		...
		while (resultListIterator.hasNext()) {
			...
			reklamationDto = getReklamationFac().reklamationFindByPrimaryKey(
					flrReklamation.getI_id());

			data[row][REPORT_REKLAMATIONSJOURNAL_KNDLSNR] = reklamationDto
					.getCKdlsnr();
			data[row][REPORT_REKLAMATIONSJOURNAL_KNDREKLANR] = reklamationDto
					.getCKdreklanr();
			data[row][REPORT_REKLAMATIONSJOURNAL_SNRCHNR] = reklamationDto
			...
			if (reklamationDto.getMaschineIId() != null) {
				...
				data[row][REPORT_REKLAMATIONSJOURNAL_MASCHINE_BEZEICHNUNG] = maschineDto
						.getCBez();
				data[row][REPORT_REKLAMATIONSJOURNAL_MASCHINE_IDENTIFIKATIONSNUMMER] = maschineDto
						.getCIdentifikationsnr();
				data[row][REPORT_REKLAMATIONSJOURNAL_MASCHINE_INVENTARNUMMER] = maschineDto
						.getCInventarnummer();

			}

			data[row][REPORT_REKLAMATIONSJOURNAL_BELEGNR] = flrReklamation
					.getReklamationart_c_nr().substring(0, 1)
					+ flrReklamation.getC_nr();
			data[row][REPORT_REKLAMATIONSJOURNAL_DATUM] = new java.sql.Timestamp(
					flrReklamation.getT_belegdatum().getTime());
			data[row][REPORT_REKLAMATIONSJOURNAL_GRUND] = flrReklamation
					.getC_grund();
			data[row][REPORT_REKLAMATIONSJOURNAL_GRUND_KOMMENTAR] = reklamationDto
					.getXGrundLang();
			data[row][REPORT_REKLAMATIONSJOURNAL_REKLAMATIONSART] = flrReklamation
					.getReklamationart_c_nr();

			if (flrReklamation.getI_kundeunterart() != null) {

				if (flrReklamation.getI_kundeunterart().equals(
						ReklamationFac.REKLAMATION_KUNDEUNTERART_FERTIGUNG)) {
					data[row][REPORT_REKLAMATIONSJOURNAL_KUNDEUNTERART] = ReklamationFac.REKLAMATIONART_FERTIGUNG;
				} else if (flrReklamation.getI_kundeunterart().equals(
						ReklamationFac.REKLAMATION_KUNDEUNTERART_LIEFERANT)) {
					data[row][REPORT_REKLAMATIONSJOURNAL_KUNDEUNTERART] = ReklamationFac.REKLAMATIONART_LIEFERANT;
				}
			}

			data[row][REPORT_REKLAMATIONSJOURNAL_MATERIALKOSTEN] = flrReklamation
					.getN_kostenmaterial();
			data[row][REPORT_REKLAMATIONSJOURNAL_AZKOSTEN] = flrReklamation
					.getN_kostenarbeitszeit();
			data[row][REPORT_REKLAMATIONSJOURNAL_PROJEKT] = flrReklamation
					.getC_projekt();
			data[row][REPORT_REKLAMATIONSJOURNAL_MENGE] = flrReklamation
					.getN_menge();

			if (flrReklamation.getFlrfehler() != null) {
				data[row][REPORT_REKLAMATIONSJOURNAL_FEHLER] = flrReklamation
						.getFlrfehler().getC_bez();
			}

			try {
				if (flrReklamation.getFlrartikel() != null) {
					data[row][REPORT_REKLAMATIONSJOURNAL_ARTIKELNR] = flrReklamation
							.getFlrartikel().getC_nr();
					ArtikelDto artikelDto = getArtikelFac()
							.artikelFindByPrimaryKeySmall(
									flrReklamation.getFlrartikel().getI_id(),
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_ARTIKELBEZEICHNUNG] = artikelDto
							.formatBezeichnung();
				} else {
					data[row][REPORT_REKLAMATIONSJOURNAL_ARTIKELBEZEICHNUNG] = reklamationDto
							.getCHandartikel();
				}
				...
				if (flrReklamation.getReklamationart_c_nr().equals(
					...
					LosDto losDto = getFertigungFac().losFindByPrimaryKey(
							flrReklamation.getLos_i_id());
					data[row][REPORT_REKLAMATIONSJOURNAL_REKLAMATIONSBELEG] = "LO"
							+ losDto.getCNr();
					...
					if (losDto.getAuftragIId() != null) {
						BelegInfos beleg = getLagerFac().getBelegInfos(
								LocaleFac.BELEGART_AUFTRAG,
								losDto.getAuftragIId(), null, theClientDto);
						data[row][REPORT_REKLAMATIONSJOURNAL_KUNDELIEFERANT] = beleg
								.getKundeLieferant();
					}
				} else if (flrReklamation.getReklamationart_c_nr().equals(
						ReklamationFac.REKLAMATIONART_KUNDE)) {
					if (flrReklamation.getKunde_i_id() != null) {
						KundeDto kundeDto = getKundeFac()
								.kundeFindByPrimaryKey(
										flrReklamation.getKunde_i_id(),
										theClientDto);
						data[row][REPORT_REKLAMATIONSJOURNAL_KUNDELIEFERANT] = kundeDto
								.getPartnerDto().formatFixTitelName1Name2();
					}
					...
					if (flrReklamation.getLieferschein_i_id() != null) {
						BelegInfos beleg = getLagerFac().getBelegInfos(
								LocaleFac.BELEGART_LIEFERSCHEIN,
								flrReklamation.getLieferschein_i_id(), null,
								theClientDto);
						data[row][REPORT_REKLAMATIONSJOURNAL_REKLAMATIONSBELEG] = "LS"
								+ beleg.getBelegnummer();
					} else if (flrReklamation.getRechnung_i_id() != null) {
						BelegInfos beleg = getLagerFac().getBelegInfos(
								LocaleFac.BELEGART_RECHNUNG,
								flrReklamation.getRechnung_i_id(), null,
								theClientDto);
						data[row][REPORT_REKLAMATIONSJOURNAL_REKLAMATIONSBELEG] = "RE"
								+ beleg.getBelegnummer();
					}
					...
					if (flrReklamation.getBestellung_i_id() != null) {
						BelegInfos beleg = getLagerFac().getBelegInfos(
								LocaleFac.BELEGART_BESTELLUNG,
								flrReklamation.getBestellung_i_id(), null,
								theClientDto);
					...
					if (flrReklamation.getLieferant_i_id() != null) {
						LieferantDto lieferantDto = getLieferantFac()
								.lieferantFindByPrimaryKey(
										flrReklamation.getLieferant_i_id(),
										theClientDto);
						data[row][REPORT_REKLAMATIONSJOURNAL_KUNDELIEFERANT] = lieferantDto
								.getPartnerDto().formatFixTitelName1Name2();
					}
					if (flrReklamation.getBestellung_i_id() != null) {
						BelegInfos beleg = getLagerFac().getBelegInfos(
								...
								flrReklamation.getBestellung_i_id(), null,
								theClientDto);
						data[row][REPORT_REKLAMATIONSJOURNAL_REKLAMATIONSBELEG] = "BS"
								+ beleg.getBelegnummer();
					}
				}
				...
				if (reklamationDto.getSchwereIId() != null) {
					data[row][REPORT_REKLAMATIONSJOURNAL_SCHWERE] = getReklamationFac()
							.schwereFindByPrimaryKey(
									reklamationDto.getSchwereIId())
							.formatBezeichnung();
				}

				data[row][REPORT_REKLAMATIONSJOURNAL_RUECKSPRECHEZEIT] = reklamationDto
						.getTRuecksprache();
				data[row][REPORT_REKLAMATIONSJOURNAL_RUECKSPRECHEMIT] = reklamationDto
						.getCRuecksprachemit();

				data[row][REPORT_REKLAMATIONSJOURNAL_B_BERECHTIGT] = Helper
						.short2Boolean(reklamationDto.getBBerechtigt());

				data[row][REPORT_REKLAMATIONSJOURNAL_MASSNAHME_BIS_KURZ] = reklamationDto
						.getTMassnahmebiskurz();
				data[row][REPORT_REKLAMATIONSJOURNAL_MASSNAHME_BIS_MITTEL] = reklamationDto
						.getTMassnahmebismittel();
				data[row][REPORT_REKLAMATIONSJOURNAL_MASSNAHME_BIS_LANG] = reklamationDto
						.getTMassnahmebislang();
				data[row][REPORT_REKLAMATIONSJOURNAL_WIRKSAMKEIT_BIS] = reklamationDto
						.getTWirksamkeitbis();
				...
				data[row][REPORT_REKLAMATIONSJOURNAL_ANALYSE] = reklamationDto
						.getXAnalyse();
				data[row][REPORT_REKLAMATIONSJOURNAL_KOMMENTAR] = reklamationDto
						.getXKommentar();
				...
				data[row][REPORT_REKLAMATIONSJOURNAL_STUECKLAGERSTAND] = reklamationDto
						.getNStuecklagerstand();
				data[row][REPORT_REKLAMATIONSJOURNAL_STUECKGELIEFERTE] = reklamationDto
						.getNStueckgelieferte();

				if (reklamationDto.getAufnahmeartIId() != null) {
					data[row][REPORT_REKLAMATIONSJOURNAL_AUFNAHMEART] = getReklamationFac()
							.aufnahmeartFindByPrimaryKey(
									reklamationDto.getAufnahmeartIId(),
									theClientDto).getBezeichnung();
				}
				if (reklamationDto.getWirksamkeitIId() != null) {
				...
				data[row][REPORT_REKLAMATIONSJOURNAL_WIRKSAMKEIT_KOMMENTAR] = reklamationDto
						.getXWirksamkeit();
				...
				if (reklamationDto.getPersonalIIdAufnehmer() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									reklamationDto.getPersonalIIdAufnehmer(),
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_PERSON_AUFNEHMER] = personalDto
							.getPartnerDto().formatTitelAnrede();
				}
				if (reklamationDto.getPersonalIIdEingefuehrtkurz() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									...
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_PERSON_MASSNAHME_KURZ] = personalDto
							.getPartnerDto().formatTitelAnrede();
				}
				if (reklamationDto.getPersonalIIdEingefuehrtmittel() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									reklamationDto
											.getPersonalIIdEingefuehrtmittel(),
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_PERSON_MASSNAHME_MITTEL] = personalDto
							.getPartnerDto().formatTitelAnrede();
				}
				if (reklamationDto.getPersonalIIdEingefuehrtlang() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									...
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_PERSON_MASSNAHME_LANG] = personalDto
							.getPartnerDto().formatTitelAnrede();
				}
				if (reklamationDto.getPersonalIIdRuecksprache() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									reklamationDto.getPersonalIIdRuecksprache(),
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_PERSON_RUECKSPRACHE] = personalDto
							.getPartnerDto().formatTitelAnrede();
				}
				if (reklamationDto.getPersonalIIdWirksamkeit() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									reklamationDto.getPersonalIIdWirksamkeit(),
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_PERSON_WIRKSAMKEIT] = personalDto
							.getPartnerDto().formatTitelAnrede();
				}
				...
				if (reklamationDto.getPersonalIIdErledigt() != null) {
					PersonalDto personalDto = getPersonalFac()
							.personalFindByPrimaryKey(
									reklamationDto.getPersonalIIdErledigt(),
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_PERSON_ERLEDIGT] = personalDto
							.getPartnerDto().formatTitelAnrede();
				}
				data[row][REPORT_REKLAMATIONSJOURNAL_ERLEDIGTDATUM] = reklamationDto
						.getTErledigt();
				...
				if (reklamationDto.getAnsprechpartnerIId() != null) {
					ansprechpartnerDto = getAnsprechpartnerFac()
							.ansprechpartnerFindByPrimaryKey(
									reklamationDto.getAnsprechpartnerIId(),
									theClientDto);
					data[row][REPORT_REKLAMATIONSJOURNAL_ANSPRECHPARTNER] = ansprechpartnerDto
							.getPartnerDto().formatTitelAnrede();
				}
		...
		initJRDS(parameter, ReklamationReportFac.REPORT_MODUL,
				ReklamationReportFac.REPORT_REKLAMATIONSJOURNAL,
				theClientDto.getMandant(), theClientDto.getLocUi(),
				theClientDto);
	...
	public Object getFieldValue(JRField jRField) throws JRException {
		...
		if (sAktuellerReport.equals(ReklamationReportFac.REPORT_REKLAMATION)
				|| sAktuellerReport
						.equals(ReklamationReportFac.REPORT_REKLAMATION_LIEFERANT)) {
			if ("F_BILD".equals(fieldName)) {
				...
				.equals(ReklamationReportFac.REPORT_FEHLERART)) {
			if ("Fehlerart".equals(fieldName)) {
				...
				.equals(ReklamationReportFac.REPORT_MITARBEITERREKLAMATION)) {
			if ("Kunde".equals(fieldName)) {
				...
				.equals(ReklamationReportFac.REPORT_MASCHINENREKLAMATION)) {
			if ("Kunde".equals(fieldName)) {
				...
				.equals(ReklamationReportFac.REPORT_REKLAMATIONSJOURNAL)) {
			if ("Belegnummer".equals(fieldName)) {
				...
				.equals(ReklamationReportFac.REPORT_LEFERANTENTERMINTREUE)) {
			if ("Lieferant".equals(fieldName)) {
				...
				.equals(ReklamationReportFac.REPORT_LIEFERANTENBEURTEILUNG)) {
			if ("Lieferant".equals(fieldName)) {
				...
				.equals(ReklamationReportFac.REPORT_OFFENEREKLAMATIONENEINESARTIKEL)) {
			if ("Lieferant".equals(fieldName)) {

...
public class AnfragevorschlagHandler extends UseCaseHandler {
	...
	private boolean bLagerminJeLager = false;
	private boolean bPersoenlicherBestellvorschlag = false;
	private java.sql.Date dDatumFuerBereitsAngefragt = new java.sql.Date(System.currentTimeMillis());
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				if (o[5] != null && ((Long) o[5]) > 0) {
					rows[row][getTableColumnInformation().getViewIndex("angefragt")] = o[5];
				}
				if (o[6] != null && ((Long) o[6]) > 0) {
					rows[row][getTableColumnInformation().getViewIndex("angeboten")] = o[6];
				}
				...
				if (bestellvorschlag.getFlrartikel().getFlrliefergruppe() != null) {
					rows[row][getTableColumnInformation().getViewIndex("artikel.liefergruppe")] = bestellvorschlag
				...
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName.endsWith(Facade.NICHT_SORTIERBAR)) {
						if (kriterien[i].isKrit) {
							...
							else if (kriterien[i].kritName.toString().startsWith("anzahl")) {
	...
	private String getFromClause() {
		return "SELECT bestellvorschlag,  (SELECT max(s.flrsperren.c_bez) FROM FLRArtikelsperren as s WHERE s.artikel_i_id=bestellvorschlag.artikel_i_id AND ( s.flrsperren.b_gesperrteinkauf=1 OR s.flrsperren.b_gesperrt=1) ) as sperren, aspr, (SELECT count(*) from FLRBestellvorschlag bv2 WHERE bv2.mandant_c_nr=bestellvorschlag.mandant_c_nr AND bv2.artikel_i_id=bestellvorschlag.artikel_i_id AND bv2.personal_i_id<>bestellvorschlag.personal_i_id) as anzahlbvandererbenutzer, vz.c_nr,(SELECT count(*) FROM FLRAnfrageposition afp WHERE afp.flranfrage.t_belegdatum >'"
				+ Helper.formatDateWithSlashes(dDatumFuerBereitsAngefragt)
				+ "' AND afp.flrartikel.i_id=bestellvorschlag.artikel_i_id AND afp.flranfrage.anfragestatus_c_nr<>'"
				+ LocaleFac.STATUS_STORNIERT
				+ "' ) as anzahlangefragt,(SELECT count(*) FROM FLRAnfragepositionlieferdaten afp WHERE afp.flranfrageposition.flranfrage.t_belegdatum >'" 
				+ Helper.formatDateWithSlashes(dDatumFuerBereitsAngefragt) + 
				"' AND afp.flranfrageposition.flrartikel.i_id=bestellvorschlag.artikel_i_id AND afp.n_anliefermenge > 0 AND afp.flranfrageposition.flranfrage.anfragestatus_c_nr<>'" + 
				 LocaleFac.STATUS_STORNIERT + 
				 "' ) as anzahlangeboten from FLRBestellvorschlag bestellvorschlag LEFT OUTER JOIN bestellvorschlag.flrartikelliste.artikelsprset AS aspr LEFT OUTER JOIN bestellvorschlag.flrartikelliste.flrvorzug AS vz LEFT OUTER JOIN bestellvorschlag.flrartikel.flrliefergruppe AS lg LEFT OUTER JOIN bestellvorschlag.flrartikelliste.flrartikelgruppe AS ag ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien, Object selectedId) throws EJBExceptionLP {
		...
		if (selectedId != null) {
			...
			try {
				session = factory.openSession();
				String queryString = "select bestellvorschlag.i_id, (SELECT count(*) FROM FLRAnfrageposition afp WHERE afp.flranfrage.t_belegdatum >'" + 
						 Helper.formatDateWithSlashes(dDatumFuerBereitsAngefragt) + 
						"' AND afp.flrartikel.i_id=bestellvorschlag.artikel_i_id AND afp.flranfrage.anfragestatus_c_nr<>'" + 
						LocaleFac.STATUS_STORNIERT + 
						"' ) as anzahlangefragt,(SELECT count(*) FROM FLRAnfragepositionlieferdaten afp WHERE afp.flranfrageposition.flranfrage.t_belegdatum >'" + 
	...
	private TableColumnInformation createColumnInformation(String mandant, Locale locUi) {
		...
		try {
			...
			columns.add("angefragt", Long.class, getTextRespectUISpr("anfragevorschlag.angefragt", mandant, locUi),
					QueryParameters.FLR_BREITE_S, "anzahlangefragt" );

			columns.add("angeboten", Long.class, getTextRespectUISpr("anfragevorschlag.angeboten", mandant, locUi),
					QueryParameters.FLR_BREITE_S, "anzahlangeboten");
			...
			columns.add("lp.bearbeitet", Boolean.class, getTextRespectUISpr("lp.bearbeitet", mandant, locUi),
					QueryParameters.FLR_BREITE_XS, BestellvorschlagFac.FLR_BESTELLVORSCHLAG_T_BEARBEITET
							+ ",bestellvorschlag." + BestellvorschlagFac.FLR_BESTELLVORSCHLAG_FLRARTIKEL + ".c_nr");
	...
	public TableInfo getTableInfo() {
		...
		try {
			...
			param = getParameterFac().getMandantparameter(theClientDto.getMandant(), ParameterFac.KATEGORIE_ANFRAGE,
					ParameterFac.PARAMETER_BEREITS_ANGEFRAGT);
			int iBereitsAngefragt = (Integer) param.getCWertAsObject();

			dDatumFuerBereitsAngefragt = Helper.addiereTageZuDatum(new java.sql.Date(System.currentTimeMillis()),
					-iBereitsAngefragt);

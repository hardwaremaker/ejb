 ...
 */
public class BSZahlungsplanHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final String FLR_ZAHLUNGSPLAN = "flrbszahlungsplan.";
	public static final String FLR_ZAHLUNGSPLAN_FROM_CLAUSE = " from FLRBSZahlungsplan flrbszahlungsplan ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRBSZahlungsplan zeitplan = (FLRBSZahlungsplan) resultListIterator
						.next();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append(FLR_ZAHLUNGSPLAN + kriterien[i].kritName);
						orderBy.append(" ");
			...
			if (orderBy.indexOf(FLR_ZAHLUNGSPLAN
	...
	private String getFromClause() {
		return FLR_ZAHLUNGSPLAN_FROM_CLAUSE;
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select " + FLR_ZAHLUNGSPLAN
						+ AuftragteilnehmerFac.FLR_AUFTRAGTEILNEHMER_I_ID
						+ FLR_ZAHLUNGSPLAN_FROM_CLAUSE
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					java.sql.Date.class, BigDecimal.class, 

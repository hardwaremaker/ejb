 ...
 *         </p>
 * 
 * @version $Revision: 1.2 $ Date $Date: 2011/07/25 14:33:43 $
 */
public class MahngruppeHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				Iterator<?> sprsetIterator = status.getFlrartikelgruppe()
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRArtikelgruppespr statusspr = (FLRArtikelgruppespr) iterUebersetzungenI
					.next();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" mahngruppe." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("mahngruppe.flrartikelgruppe.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("mahngruppe.flrartikelgruppe.c_nr") < 0) {
				...
				orderBy.append(" mahngruppe.flrartikelgruppe.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRMahngruppe AS mahngruppe"
				+ " LEFT JOIN mahngruppe.flrartikelgruppe.artikelgruppesprset AS artgruset";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRMahngruppe status = (FLRMahngruppe) scrollableResult.get(0);
						Integer artgruIId = status.getArtgru_i_id();
						if (selectedId.equals(artgruIId)) {

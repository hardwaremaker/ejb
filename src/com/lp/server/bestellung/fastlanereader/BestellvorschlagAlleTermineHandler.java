...
public class BestellvorschlagAlleTermineHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			query = session
					.createQuery("select distinct bestellvorschlagalletermine.t_liefertermin from "
							+ "FLRBestellvorschlagAlleTermine bestellvorschlagalletermine "
							+ "WHERE bestellvorschlagalletermine.t_liefertermin IS NOT NULL "
							+ "AND bestellvorschlagalletermine.mandant_c_nr = "
							+ "'"
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = t_liefertermin == null ? null
						: t_liefertermin;
				row++;
				col = 0;
			}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			session = factory.openSession();
			String queryString = "select count(distinct bestellvorschlagalletermine.t_liefertermin) "
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" bestellvorschlagalletermine."
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("bestellvorschlagalletermine."
				...
				orderBy.append("bestellvorschlagalletermine.").append(
						"t_liefertermin").append(" ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("bestellvorschlagalletermine."
					+ "t_liefertermin") < 0) {
	...
	private String getFromClause() {
		return "from FLRBestellvorschlagAlleTermine bestellvorschlagalletermine ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null) {
			...
			try {
				session = factory.openSession();
				String queryString = "select bestellvorschlagalletermine."
						+ "t_liefertermin"
						+ " from FLRBestellvorschlagAlleTermine bestellvorschlagalletermine "

...
public class BSMahnwesenFacBean extends Facade implements BSMahnwesenFac {
	...
	public BSMahnungDto createBSMahnung(BSMahnungDto bSMahnungDto, TheClientDto theClientDto) throws EJBExceptionLP {
		...
		bSMahnungDto.setIId(getPKGeneratorObj().getNextPrimaryKey(PKConst.PK_BSMAHNUNG));

		try {
			Bsmahnung bSMahnung = new Bsmahnung(bSMahnungDto.getIId(), bSMahnungDto.getMandantCNr(),
					bSMahnungDto.getMahnlaufIId(), bSMahnungDto.getMahnstufeIId(), bSMahnungDto.getBestellungIId(),
					bSMahnungDto.getTMahndatum(), bSMahnungDto.getPersonalIIdAnlegen(),
					bSMahnungDto.getPersonalIIdAendern(), bSMahnungDto.getBestellpositionIId());
			...
			getBestellungFac().updateBestellungMahnstufe(bSMahnungDto.getBestellungIId(),
	...
	private void setBSMahnstufeInBestellung(Integer BestellungId, TheClientDto theClientDto)
			throws EJBExceptionLP, RemoteException {
		// Alle restlichen Mahnungen der Bestellung holen
		BSMahnungDto[] bsMahnungDto = bsmahnungFindByBestellungIId(BestellungId, theClientDto);
		...
		if (bsMahnungDto != null) {
			...
			for (int i = 0; i < bsMahnungDto.length; i++) {
				if (neueMahnung == null) {
					...
					if (neueMahnung.getTMahndatum().before(bsMahnungDto[i].getTMahndatum())) {
						neueMahnung = bsMahnungDto[i];
					}
				}
			}
		...
		if (neueMahnung == null) {
			getBestellungFac().updateBestellungMahnstufe(bestellungDto.getIId(), null, theClientDto);
		} else {
			getBestellungFac().updateBestellungMahnstufe(bestellungDto.getIId(), neueMahnung.getMahnstufeIId(),
					theClientDto);
			bestellungDto.setIMahnstufeIId(neueMahnung.getMahnstufeIId());
		}
	}

	public void removeBSMahnung(Integer iId, TheClientDto theClientDto) throws EJBExceptionLP {
		try {
	 ...
	 * 
	 * @param bSMahnungDto BSMahnungDto
	 * @param theClientDto String
	 * @throws EJBExceptionLP
	 */
	public void removeBSMahnungAusMahnlauf(BSMahnungDto bSMahnungDto, TheClientDto theClientDto) throws EJBExceptionLP {
		myLogger.entry();
		if (bSMahnungDto != null) {
			...
			try {
				 ...
				 * em.find(Bsmahnung.class, iId); if (toRemove == null) { throw new
				 * EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, null); } try {
	...
	public void removeBSMahnung(BSMahnungDto bSMahnungDto, TheClientDto theClientDto) throws EJBExceptionLP {
		myLogger.entry();
		if (bSMahnungDto != null) {
			if (bSMahnungDto.getTGedruckt() != null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_FINANZ_MAHNLAUF_WURDE_SCHON_UEBERNOMMEN, "");
				...
				// try {
				/*
				 * Bsmahnung toRemove = em.find(Bsmahnung.class, ); if (toRemove == null) {
				 * throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, null); }
				 * try { em.remove(toRemove); em.flush(); } catch (EntityExistsException er) {
				 * throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_LOESCHEN, er); }
				 */
				// }
	...
	public BSMahnungDto updateBSMahnung(BSMahnungDto bSMahnungDto, TheClientDto theClientDto) throws EJBExceptionLP {
		if (bSMahnungDto != null) {
			Integer iId = bSMahnungDto.getIId();
			try {
				Bsmahnung bSMahnung = em.find(Bsmahnung.class, iId);
				setBSMahnungFromBSMahnungDto(bSMahnung, bSMahnungDto);
				setBSMahnstufeInBestellung(bSMahnungDto.getBestellungIId(), theClientDto);
			} catch (Exception e) {
				throw new EJBExceptionLP(e);
			}
		}
	...
	public void updateBSMahnungs(BSMahnungDto[] bSMahnungDtos, TheClientDto theClientDto) throws EJBExceptionLP {
	...
	public BSMahnungDto bsmahnungFindByPrimaryKey(Integer iId) throws EJBExceptionLP {
		try {
			Bsmahnung bsmahnung = em.find(Bsmahnung.class, iId);
			if (bsmahnung == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, "");
	...
	private void setBSMahnungFromBSMahnungDto(Bsmahnung bsmahnung, BSMahnungDto bSMahnungDto) {
	...
	public Integer createBSMahnlauf(BSMahnlaufDto bSMahnlaufDto, TheClientDto theClientDto) throws EJBExceptionLP {
		try {
			BSMahnlaufDto bsmahnlaufDto = new BSMahnlaufDto();
			bsmahnlaufDto.setIId(getPKGeneratorObj().getNextPrimaryKey(PKConst.PK_BSMAHNLAUF));
			...
			Bsmahnlauf bsmahnlauf = new Bsmahnlauf(bsmahnlaufDto.getIId(), bsmahnlaufDto.getMandantCNr(),
					bsmahnlaufDto.getPersonalIIdAnlegen(), bsmahnlaufDto.getPersonalIIdAendern());
	...
	public void removeBSMahnlauf(Integer iId, TheClientDto theClientDto) throws EJBExceptionLP {
		// try {
		Bsmahnlauf toRemove = em.find(Bsmahnlauf.class, iId);
		if (toRemove == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, "");
		}
		BSMahnungDto[] bsmahnungen = bsmahnungFindByBSMahnlaufIId(toRemove.getIId(), theClientDto);
		...
		try {
			for (int i = 0; i < bsmahnungen.length; i++) {
				context.getBusinessObject(BSMahnwesenFac.class).removeBSMahnungAusMahnlauf(bsmahnungen[i],
						theClientDto);
			}
			context.getBusinessObject(BSMahnwesenFac.class).removeBSMahnlauf(iId);
	...
	@TransactionAttribute(TransactionAttributeType.NEVER)
	public void removeBSMahnlauf(BSMahnlaufDto bSMahnlaufDto, TheClientDto theClientDto) throws EJBExceptionLP {
		if (bSMahnlaufDto != null) {
			Integer iId = bSMahnlaufDto.getIId();
			// zuerst die angehaengten Mahnungen loeschen
			try {
				BSMahnungDto[] bsmahnungen = bsmahnungFindByBSMahnlaufIId(iId, theClientDto);
				for (int i = 0; i < bsmahnungen.length; i++) {
					if (bsmahnungen[i].getTGedruckt() != null) {
						throw new EJBExceptionLP(EJBExceptionLP.FEHLER_FINANZ_MAHNLAUF_WURDE_SCHON_UEBERNOMMEN, "");
					}
	...
	public void updateBSMahnlauf(BSMahnlaufDto bSMahnlaufDto, TheClientDto theClientDto) throws EJBExceptionLP {
		if (bSMahnlaufDto != null) {
			Integer iId = bSMahnlaufDto.getIId();
			try {
	...
	public void updateBSMahnlaufs(BSMahnlaufDto[] bSMahnlaufDtos, TheClientDto theClientDto) throws EJBExceptionLP {
	...
	public BSMahnlaufDto bsmahnlaufFindByPrimaryKey(Integer iId) throws EJBExceptionLP {
		try {
			Bsmahnlauf bsmahnlauf = em.find(Bsmahnlauf.class, iId);
			if (bsmahnlauf == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, "");
	...
	private void setBSMahnlaufFromBSMahnlaufDto(Bsmahnlauf bSMahnlauf, BSMahnlaufDto bSMahnlaufDto) {
	...
	public Integer createBSMahnstufe(BSMahnstufeDto bSMahnstufeDto, TheClientDto theClientDto) throws EJBExceptionLP {
		myLogger.logData(bSMahnstufeDto);
		bSMahnstufeDto.setMandantCNr(theClientDto.getMandant());
		try {
			Bsmahnstufe bSMahnstufe = new Bsmahnstufe(bSMahnstufeDto.getIId(), bSMahnstufeDto.getMandantCNr(),
	...
	public void removeBSMahnstufe(Integer iId, TheClientDto theClientDto) throws EJBExceptionLP {
		// try {
	...
	public void removeBSMahnstufe(BSMahnstufeDto bSMahnstufeDto, TheClientDto theClientDto) throws EJBExceptionLP {
		myLogger.entry();
		if (bSMahnstufeDto == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_DTO_IS_NULL, new Exception("bSMahnstufeDto == null"));
		}
		if (bSMahnstufeDto != null) {
			// try {
			Integer iId = bSMahnstufeDto.getIId();
			if (iId.intValue() == BSMahnwesenFac.MAHNSTUFE_1 || iId.intValue() == BSMahnwesenFac.MAHNSTUFE_2
					|| iId.intValue() == BSMahnwesenFac.MAHNSTUFE_3 || iId.intValue() == BSMahnwesenFac.MAHNSTUFE_99
				...
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_FINANZ_MAHNSTUFEN_1_2_3_DUERFEN_NICHT_GELOESCHT_WERDEN,
						new Exception("BSMahnstufe " + iId.intValue() + " darf nicht geloescht werden"));
			}
			Bsmahnstufe toRemove = em.find(Bsmahnstufe.class, new BsmahnstufePK(iId, bSMahnstufeDto.getMandantCNr()));
			if (toRemove == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, "");
			}
			try {
	...
	public void updateBSMahnstufe(BSMahnstufeDto bSMahnstufeDto, TheClientDto theClientDto) throws EJBExceptionLP {
		if (bSMahnstufeDto != null) {
			// try {
			Bsmahnstufe bSMahnstufe = em.find(Bsmahnstufe.class,
					new BsmahnstufePK(bSMahnstufeDto.getIId(), bSMahnstufeDto.getMandantCNr()));
			if (bSMahnstufe == null) {
	...
	public void updateBSMahnstufes(BSMahnstufeDto[] bSMahnstufeDtos, TheClientDto theClientDto) throws EJBExceptionLP {
	...
	public BSMahnstufeDto bsmahnstufeFindByPrimaryKeyOhneExc(BsmahnstufePK bsmahnstufePK) throws EJBExceptionLP {
		// try {
	...
	private void setBSMahnstufeFromBSMahnstufeDto(Bsmahnstufe bSMahnstufe, BSMahnstufeDto bSMahnstufeDto) {
	...
	public Integer createBSMahntext(BSMahntextDto bSMahntextDto, TheClientDto theClientDto) throws EJBExceptionLP {
		myLogger.logData(bSMahntextDto);
		if (bSMahntextDto == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_DTO_IS_NULL, new Exception("bSMahntextDto == null"));
		}

		bSMahntextDto.setIId(getPKGeneratorObj().getNextPrimaryKey(PKConst.PK_BSMAHNTEXT));
		...
		try {
			BSMahnstufeDto bsmahnstufeDto = getBSMahnwesenFac().bsmahnstufeFindByPrimaryKeyOhneExc(
					new BsmahnstufePK(bSMahntextDto.getMahnstufeIId(), bSMahntextDto.getMandantCNr()));
			if (bsmahnstufeDto == null) {
				...
				getBSMahnwesenFac().createBSMahnstufe(bsmahnstufeDto, theClientDto);
			}
			Bsmahntext bsmahntext = new Bsmahntext(bSMahntextDto.getIId(), bSMahntextDto.getMandantCNr(),
					bSMahntextDto.getLocaleCNr(), bSMahntextDto.getMahnstufeIId(), bSMahntextDto.getXTextinhalt());
	...
	public void removeBSMahntext(Integer iId, TheClientDto theClientDto) throws EJBExceptionLP {
		// try {
	...
	public void removeBSMahntext(BSMahntextDto bSMahntextDto, TheClientDto theClientDto) throws EJBExceptionLP {
		myLogger.logData(bSMahntextDto);
		if (bSMahntextDto == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_DTO_IS_NULL, new Exception("bSMahntextDto == null"));
		}
		// try {
		if (bSMahntextDto.getMahnstufeIId().intValue() == 1 || bSMahntextDto.getMahnstufeIId().intValue() == 2
				|| bSMahntextDto.getMahnstufeIId().intValue() == 3) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_FINANZ_MAHNTEXTE_1_2_3_DUERFEN_NICHT_GELOESCHT_WERDEN,
					new Exception(
							"Mahntext " + bSMahntextDto.getMahnstufeIId().intValue() + " darf nicht geloescht werden"));
		}
	...
	public void updateBSMahntext(BSMahntextDto bSMahntextDto, TheClientDto theClientDto) throws EJBExceptionLP {
		if (bSMahntextDto != null) {
			Integer iId = bSMahntextDto.getIId();
			try {
	...
	public void updateBSMahntexts(BSMahntextDto[] bSMahntextDtos, TheClientDto theClientDto) throws EJBExceptionLP {
	...
	public BSMahntextDto bsmahntextFindByPrimaryKey(Integer iId) throws EJBExceptionLP {
		try {
			Bsmahntext bsmahntext = em.find(Bsmahntext.class, iId);
			if (bsmahntext == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, "");
	...
	private void setBSMahntextFromBSMahntextDto(Bsmahntext bSMahntext, BSMahntextDto bSMahntextDto) {
	...
	public BSMahnungDto[] bsmahnungFindByBSMahnlaufIId(Integer mahnlaufIId, TheClientDto theClientDto)
			throws EJBExceptionLP {
	...
	public BSMahnungDto[] bsmahnungFindByBSMahnlaufIIdLieferantIId(Integer mahnlaufIId, Integer lieferantIId,
		...
		// @ToDo Ungueltiger Prozeduraufruf oder ungueltiges Argument
		Query query = em.createNamedQuery("BSMahnungfindBymahnlaufIIdLieferantIIdMandantCNr");
		query.setParameter(1, mahnlaufIId);
	...
	public BSMahnstufeDto[] bsmahnstufeFindByMandantCNr(String mandantCNr, TheClientDto theClientDto)
			throws EJBExceptionLP {
		try {
	...
	public BSMahnlaufDto[] bsmahnlaufFindByMandantCNr(String mandantCNr, TheClientDto theClientDto)
			throws EJBExceptionLP {
		// try {
	...
	public BSMahnungDto[] bsmahnungFindByBestellungIId(Integer bestellungIId, TheClientDto theClientDto)
			throws EJBExceptionLP {
		// check(theClientDto);
		// try {
	...
	public BSMahnungDto[] bsmahnungFindByBestellungBSMahnstufe(Integer bestellungIId, Integer mahnstufeIId,
			TheClientDto theClientDto) throws EJBExceptionLP {
		// try {
		Query query = em.createNamedQuery("BSMahnungfindByBestellungBSMahnstufe");
		query.setParameter(1, bestellungIId);
	...
	public LinkedHashMap getAllBSMahnstufen(String mandantCNr, TheClientDto theClientDto) throws EJBExceptionLP {
		LinkedHashMap<Integer, Integer> uMap = new LinkedHashMap<Integer, Integer>();
		try {
			BSMahnstufeDto[] bsmahnstufen = bsmahnstufeFindByMandantCNr(mandantCNr, theClientDto);
	...
	public BSMahntextDto bsmahntextFindByMandantLocaleCNr(String mandantCNr, String localeCNr, Integer bsmahnstufeIId)
			throws EJBExceptionLP {
		try {
			Query query = em.createNamedQuery("BSMahntextfindByMandantLocaleCNr");
			query.setParameter(1, mandantCNr);
			query.setParameter(2, localeCNr);
			query.setParameter(3, bsmahnstufeIId);
			Bsmahntext bsmahntext = (com.lp.server.bestellung.ejb.Bsmahntext) query.getSingleResult();
	...
	public BSMahntextDto createDefaultBSMahntext(Integer bsmahnstufeIId, String sTextinhaltI, TheClientDto theClientDto)
			throws EJBExceptionLP, RemoteException {
		BSMahntextDto oBSMahntextDto = new BSMahntextDto();
		BsmahnstufePK mahnstufePK = new BsmahnstufePK(bsmahnstufeIId, theClientDto.getMandant());
	...
	public Boolean bGibtEsEinenOffenenBSMahnlauf(String mandantCNr, TheClientDto theClientDto) {

		Session session = FLRSessionFactory.getFactory().openSession();

		String sQuery = "SELECT m FROM FLRBSMahnung m WHERE m.flrmahnlauf.mandant_c_nr='" + mandantCNr
				+ "' AND m.t_gedruckt IS NULL";
	 ...
	 * 
	 * @param bsmahnlaufIId Integer
	 * @param theClientDto  String
	 * @throws EJBExceptionLP
	 */
	@TransactionAttribute(TransactionAttributeType.NEVER)
	public void mahneBSMahnlaufRueckgaengig(Integer bsmahnlaufIId, TheClientDto theClientDto) throws EJBExceptionLP {
		BSMahnungDto[] bsmahnungen = bsmahnungFindByBSMahnlaufIId(bsmahnlaufIId, theClientDto);
		for (int i = 0; i < bsmahnungen.length; i++) {
	 ...
	 */
	@TransactionAttribute(TransactionAttributeType.NEVER)
	public void mahneBSMahnungRueckgaengig(Integer mahnungIId, TheClientDto theClientDto) throws EJBExceptionLP {
		...
		try {
			context.getBusinessObject(BSMahnwesenFac.class).updateBSMahnung(bsMahnungDto, theClientDto);
		} catch (RemoteException e) {
	 ...
	 * @throws EJBExceptionLP
	 */
	public BSMahnlaufDto createBSMahnlaufEchteLiefermahnungen(TheClientDto theClientDto) throws EJBExceptionLP {
		...
		// Mahnlauf entfernt
		bsmahnungDto = bsmahnungFindByBSMahnlaufIIdOhneExc(bsmahnlaufIId, theClientDto);
	...
	public BSMahnlaufDto createBSMahnlaufLiefererinnerung(TheClientDto theClientDto) {
		Integer bsmahnlaufIId = null;
		BSMahnlaufDto bsmahnlaufDto = null;
		BSMahnungDto bsmahnungDto[] = null;
	 ...
	 * @throws EJBExceptionLP
	 */
	public BSMahnlaufDto createBSMahnlaufABMahnungen(TheClientDto theClientDto) throws EJBExceptionLP {
		Integer bsmahnlaufIId = null;
		BSMahnlaufDto bsmahnlaufDto = null;
		BSMahnungDto bsmahnungDto[] = null;
	 ...
	 * @return BSMahnungDto
	 */
	public BSMahnungDto[] bsmahnungFindByBestellpositionIIdOhneExc(Integer bestellpositionIId,
	...
	public void mahneBSMahnung(Integer bsmahnungIId, TheClientDto theClientDto) throws EJBExceptionLP {
		try {
			Bsmahnung bsmahnung = em.find(Bsmahnung.class, bsmahnungIId);
			BestellungDto bestellungDto = getBestellungFac().bestellungFindByPrimaryKey(bsmahnung.getBestellungIId());
			// nur dann mahnen, wenn die Bestellung keine mahnsperre hat oder
			// diese schon vorbei ist.
			if (bestellungDto.getTMahnsperreBis() == null || bestellungDto.getTMahnsperreBis().before(getDate())) {
				...
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BESTELLUNG_MAHNSPERRE,
						new Exception("Die Bestellung " + bestellungDto.getCNr()
								+ " darf nicht gemahnt werden: Mahnsperre bis " + bestellungDto.getTMahnsperreBis()));
			}
		} catch (Exception ex) {
	...
	public void mahneBSMahnlauf(Integer mahnlaufIId, TheClientDto theClientDto) throws EJBExceptionLP {
		BSMahnungDto[] bsmahnungen = bsmahnungFindByBSMahnlaufIId(mahnlaufIId, theClientDto);
		for (int i = 0; i < bsmahnungen.length; i++) {
	...
	private BestellungDto[] assembleABundLieferterminBestellungDtos(BestellungDto bestellungDtoABTermine[],
	 ...
	 * ----------------------------------------------------------
	 * 
	 * @param echteLiefermahnung boolean
	 * @param echteABMahnungen   boolean
	 * @param theClientDto       String
	 * @throws EJBExceptionLP
	 * @return Integer ----------------------------------------------------------
	 *         -----------------
	...
	private Integer createMahnungen(boolean echteLiefermahnung, boolean echteABMahnungen, boolean liefererinnerung,
		...
		try {
			...
			BSMahnstufeDto bsmahnstufeDto[] = this.bsmahnstufeFindByMandantCNr(theClientDto.getMandant(), theClientDto);
			...
			crit.add(Restrictions.not(
					Restrictions.in(BestellpositionFac.FLR_BESTELLPOSITION_BESTELLPOSITIONART_C_NR, cPositionsarten)));

			// PJ 16536
			String queryString = "SELECT artgru_i_id FROM FLRMahngruppe m" + " WHERE m.flrartikelgruppe.mandant_c_nr ='"
			...
			for (Iterator<?> iter = list.iterator(); iter.hasNext();) {
				...
				/**
				 * nur Liefermahnungen deren Status bestaetigt ist oder deren Liefertermin vor
				 * heute ist
				 */
				...
				if (bEchteLiefermahnungVorhanden == false) {
					...
					/**
					 * nur ABMahnungen deren Status offen ist ausser der Liefertermin ist vor heute
					 * dann ist es eine Liefermahnung
					 */
							...
							if (flrbespos.getT_lieferterminbestaetigt() == null) {
								...
								for (int i = 0; i < bsmahnstufeDto.length; i++) {
									if (bsmahnstufeDto[i].getIId()
										...
										liefererinnerungen(flrbespos, bsmahnlaufIId, bsmahnstufeDto[i], theClientDto);
	 ...
	 * 
	 * @param flrbespos      FLRBestellpositionReport
	 * @param bsmahnlaufIId  Integer
	 * @param bsmahnstufeDto BSMahnstufeDto[]
	 * @param theClientDto   String
	 * @throws EJBExceptionLP
	 */
	@TransactionAttribute(TransactionAttributeType.NEVER)
	private void echteLiefermahnungen(FLRBestellpositionReport flrbespos, Integer bsmahnlaufIId,
			BSMahnstufeDto bsmahnstufeDto, TheClientDto theClientDto) throws EJBExceptionLP {
		try {
			...
			if (((flrbespos.getBestellpositionstatus_c_nr().equals(BestellpositionFac.BESTELLPOSITIONSTATUS_BESTAETIGT))
					|| (flrbespos.getBestellpositionstatus_c_nr()
							.equals(BestellpositionFac.BESTELLPOSITIONSTATUS_OFFEN))
							&& getDate().after(flrbespos.getT_uebersteuerterliefertermin()))) {
				BestellungDto bestellungDto = getBestellungFac()
						.bestellungFindByPrimaryKeyOhneExc(flrbespos.getFlrbestellung().getI_id());
				anlegenVonEchterLiefermahnung(bestellungDto, bsmahnstufeDto, flrbespos, bsmahnlaufIId, theClientDto);
			}
		} catch (RemoteException ex) {
			throwEJBExceptionLPRespectOld(ex);
		}
	...
	@TransactionAttribute(TransactionAttributeType.NEVER)
	private void liefererinnerungen(FLRBestellpositionReport flrbespos, Integer bsmahnlaufIId,
			BSMahnstufeDto bsmahnstufeDto, TheClientDto theClientDto) throws EJBExceptionLP {
		try {
			// wenn Bestellposition bestaetigt
			// wenn Bestellposition offen und uebersteuerterLiefertermin ist vor
			...
			if (((flrbespos.getBestellpositionstatus_c_nr().equals(BestellpositionFac.BESTELLPOSITIONSTATUS_BESTAETIGT))
					|| flrbespos.getBestellpositionstatus_c_nr().equals(BestellpositionFac.BESTELLPOSITIONSTATUS_OFFEN)
					|| flrbespos.getBestellpositionstatus_c_nr()
							.equals(BestellpositionFac.BESTELLPOSITIONSTATUS_TEILGELIEFERT))
					&& getDate().after(Helper.addiereTageZuTimestamp(tTermin, bsmahnstufeDto.getITage()))) {
				BestellungDto bestellungDto = getBestellungFac()
						.bestellungFindByPrimaryKeyOhneExc(flrbespos.getFlrbestellung().getI_id());
				anlegenVonLiefererinnerung(bestellungDto, bsmahnstufeDto, flrbespos, bsmahnlaufIId, theClientDto);
			}
		} catch (RemoteException ex) {
			throwEJBExceptionLPRespectOld(ex);
		}
	 ...
	 */
	@TransactionAttribute(TransactionAttributeType.NEVER)
	private void anlegenVonEchterLiefermahnung(BestellungDto bestellungDto, BSMahnstufeDto bsmahnstufeDto,
			FLRBestellpositionReport flrbespos, Integer bsmahnlaufIId, TheClientDto theClientDto)
			throws EJBExceptionLP, RemoteException {
		if (bestellungDto != null) {
			// wenn heute - letztesMahndatum kleiner als MahnstufeTage dann
			// mache nichts
			
			if (!(checkZeitraumVonLetzterMahnung(flrbespos, bsmahnstufeDto, theClientDto))) {
				// wenn liefertermin vor heute dann wird hier gemahnt
				Timestamp tLiefertermin = null;
				if (flrbespos.getT_auftragsbestaetigungstermin() != null) {
					// Wenn AB-Termin zieht immer der
					tLiefertermin = flrbespos.getT_auftragsbestaetigungstermin();
				} else {
				...
				if (getDate().after(tLiefertermin)) {
					...
					if ((bsmahnstufeDto == null) || (bsmahnstufeDto.getIId().intValue() == 0)) {
						bsmahnungDto.setMahnstufeIId(new Integer(1));
					} else {
						int nextMahnstufe = bsmahnstufeDto.getIId() + 1;
						BsmahnstufePK mahnstufePK = new BsmahnstufePK(nextMahnstufe, theClientDto.getMandant());
						...
						if ((newBSMahnstufeDto == null)
							...
							bsmahnungDto.setMahnstufeIId(BSMahnwesenFac.MAHNSTUFE_99);
							// Wenn alte Mahnstufe 99 dann gleich auf gedruckt
							// setzen
							if (BSMahnwesenFac.MAHNSTUFE_99 == bsmahnstufeDto.getIId()) {
								if (bsmahnungDto.getTGedruckt() == null) {
									bsmahnungDto.setTGedruckt(getTimestamp());
								}
							}
						} else {
							bsmahnungDto.setMahnstufeIId(newBSMahnstufeDto.getIId());
						}
					}
					if (!(bsmahnungDto.getNOffeneMenge() == null)) {
						if (!(bsmahnungDto.getNOffeneMenge().intValue() < 1)) {
							context.getBusinessObject(BSMahnwesenFac.class).createBSMahnung(bsmahnungDto, theClientDto);
						}
					}
				}
			}
	...
	@TransactionAttribute(TransactionAttributeType.NEVER)
	private void anlegenVonLiefererinnerung(BestellungDto bestellungDto, BSMahnstufeDto bsmahnstufeDto,
			FLRBestellpositionReport flrbespos, Integer bsmahnlaufIId, TheClientDto theClientDto)
			throws EJBExceptionLP, RemoteException {
		if (bestellungDto != null) {
			// mahnen
			BSMahnungDto bsmahnungDto = null;
			BestellpositionDto besposDto = getBestellpositionFac().bestellpositionFindByPrimaryKey(flrbespos.getI_id());
			bsmahnungDto = new BSMahnungDto();
			bsmahnungDto.setMandantCNr(theClientDto.getMandant());
			bsmahnungDto.setMahnlaufIId(bsmahnlaufIId);
			bsmahnungDto.setBestellungIId(bestellungDto.getIId());
	 ...
	 */
	@TransactionAttribute(TransactionAttributeType.NEVER)
	private void echteABMahnungen(FLRBestellpositionReport flrbespos, BSMahnstufeDto bsmahnstufeDto,
			Integer bsmahnlaufIId, TheClientDto theClientDto) throws EJBExceptionLP, RemoteException {
		// wenn status offen und kein AB-Termin
		// dann ist es eine ABMahnung
		if (flrbespos.getBestellpositionstatus_c_nr().equals(BestellpositionFac.BESTELLPOSITIONSTATUS_OFFEN)
				&& (flrbespos.getT_auftragsbestaetigungstermin() == null)) {
			// Wenn vorher bereits Mahnstufe 1 angelegt dann keine AB Mahnung
			BSMahnungDto[] helper = getBSMahnwesenFac().bsmahnungFindByBestellpositionIIdOhneExc(flrbespos.getI_id(),
					theClientDto);
			...
			if (helper.length == 0 || maxStufe == BSMahnwesenFac.MAHNSTUFE_0) {
				...
				Timestamp tPosTerminPlusMahnstufe = new Timestamp(
						flrbespos.getFlrbestellung().getT_belegdatum().getTime()
								+ Helper.calculateDaysBackIntoMilliseconds(bsmahnstufeDto.getITage()));
				tPosTerminPlusMahnstufe = Helper.cutTimestamp(tPosTerminPlusMahnstufe);
				if (Helper.cutTimestamp(getTimestamp()).after(tPosTerminPlusMahnstufe)) {
					// mahnen
					BestellungDto bestellungDto = getBestellungFac()
							.bestellungFindByPrimaryKeyOhneExc(flrbespos.getFlrbestellung().getI_id());
					BestellpositionDto besposDto = getBestellpositionFac()
	 ...
	 * @return boolean
	 */
	private boolean checkZeitraumVonLetzterMahnung(FLRBestellpositionReport flrbespos, BSMahnstufeDto bsmahnstufeDto,
			TheClientDto theClientDto) throws EJBExceptionLP, RemoteException {
		boolean bool = false;
		// letztes Mahndatum holen
		BSMahnungDto[] bsmahnungdto = getBSMahnwesenFac().bsmahnungFindByBestellpositionIIdOhneExc(flrbespos.getI_id(),
				theClientDto);

		//SP7578 Nach Mahnstufe absteigend sortieren
		
		TreeMap<Integer, BSMahnungDto> tmSort = new TreeMap<Integer, BSMahnungDto>();

		for (int i = 0; i < bsmahnungdto.length; i++) {
			tmSort.put(bsmahnungdto[i].getMahnstufeIId(), bsmahnungdto[i]);
		}

		Iterator it = tmSort.descendingKeySet().iterator();

		bsmahnungdto = new BSMahnungDto[tmSort.size()];
		int i = 0;
		while (it.hasNext()) {
			bsmahnungdto[i] = tmSort.get(it.next());

			i++;
		}

		// SP7465 Wenn die letzte Mahnung eine AB-Mahnung /Liefererinnerung war, dann
		// ist diese fuer die echte Liefermahnung nicht relevant
		if (bsmahnungdto.length > 0 && bsmahnungdto[0].getMahnstufeIId().intValue() > BSMahnwesenFac.MAHNSTUFE_0) {
			...
			BSMahnungDto lastMahnung = getBSMahnwesenFac().bsmahnungFindByPrimaryKey(iMahnungIId);
			...
			if (getDate().equals(lastMahnung.getTMahndatum())) {
				...
				if (newBSMahnstufeDto!=null && lastMahnung.getTMahndatum() != null && lastMahnung.getMahnstufeIId() != null
						&& ((getDate().getTime() - lastMahnung.getTMahndatum().getTime()) < Helper
								.calculateDaysBackIntoMilliseconds(newBSMahnstufeDto.getITage()))
			...
			bsmahnstufeDto = getBSMahnwesenFac().bsmahnstufeFindByPrimaryKeyOhneExc(
					new BsmahnstufePK(BSMahnwesenFac.MAHNSTUFE_1, theClientDto.getMandant()));
			Timestamp tMaxPosTermin = new Timestamp(
					getDate().getTime() - Helper.calculateDaysBackIntoMilliseconds(bsmahnstufeDto.getITage()));
	...
	/**
	 * liefert das datum vom ABTermin oder sonst vom uebersteuertenLiefertermin der
	 * Bestellposition
	 * 
	 * @param flrbespos FLRBestellpositionReport
	 * @return Date
	 */
	private Date getRichtigesMahndatumFuerABMahnung(FLRBestellpositionReport flrbespos) {
		// datum vom Beleg oder sonst Datum vom bestaetigen Liefertermin
		java.sql.Date date = null;
		if (flrbespos.getT_auftragsbestaetigungstermin() == null) {
			date = new Date(flrbespos.getT_uebersteuerterliefertermin().getTime());
		} else {
			date = new Date(flrbespos.getT_auftragsbestaetigungstermin().getTime());
		}
		return date;
	}

	public BSMahnungDto[] bsmahnungFindByBSMahnlaufIIdOhneExc(Integer mahnlaufIId, TheClientDto theClientDto) {
		BSMahnungDto bsmahnungDto[] = null;
		// try {
		Query query = em.createNamedQuery("BSMahnungfindByBSMahnlaufIId");
		query.setParameter(1, mahnlaufIId);

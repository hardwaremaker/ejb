...
public class BSMahnstufeDtoAssembler {
	public static BSMahnstufeDto createDto(Bsmahnstufe bSMahnstufe) {
		BSMahnstufeDto bSMahnstufeDto = new BSMahnstufeDto();
		if (bSMahnstufe != null) {
			bSMahnstufeDto.setIId(bSMahnstufe.getPk().getIId());
			bSMahnstufeDto.setMandantCNr(bSMahnstufe.getPk().getMandantCNr());
			bSMahnstufeDto.setITage(bSMahnstufe.getITage());
		}
		return bSMahnstufeDto;
	}

	public static BSMahnstufeDto[] createDtos(Collection<?> bSMahnstufes) {
		List<BSMahnstufeDto> list = new ArrayList<BSMahnstufeDto>();
		if (bSMahnstufes != null) {
			Iterator<?> iterator = bSMahnstufes.iterator();
			while (iterator.hasNext()) {
				list.add(createDto((Bsmahnstufe) iterator.next()));
			}
		}

...
import com.lp.server.personal.fastlanereader.generated.FLRNachrichtenart;
import com.lp.server.personal.fastlanereader.generated.FLRNachrichtenartspr;
...
import com.lp.server.stueckliste.fastlanereader.generated.FLRStklparameter;
import com.lp.server.stueckliste.fastlanereader.generated.FLRStklparameterspr;
 ...
 * @version 1.0
 */
public class StklparameterHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = StklparameterHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				...
				FLRStklparameter flr = (FLRStklparameter) o[0];

				Iterator<?> sprsetIterator = flr
						.getStklparametersprset().iterator();

				rows[row][col++] = flr.getI_id();
				rows[row][col++] = flr.getC_nr();

				String spr = null;
				while (sprsetIterator.hasNext()) {
					FLRStklparameterspr stklparameterspr = (FLRStklparameterspr) sprsetIterator
							.next();
					if (stklparameterspr.getLocale().getC_nr()
							.compareTo(sLocUI) == 0) {
						spr = stklparameterspr.getC_bez();
						break;
					}
				}
				rows[row][col++] = spr;
				rows[row][col++] = flr.getC_wert();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRStklparameter AS stklparameter"
					+ " LEFT JOIN stklparameter.stklparametersprset AS stklparametersprset "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
	...
	private String buildOrderByClause() {
		...
		if (getQuery() != null) {
			...
			if (orderBy.indexOf("i_sort") < 0) {
				...
				orderBy.append(" stklparameter.i_sort ");
				sortAdded = true;
			}
			if (sortAdded) {
				orderBy.insert(0, " ORDER BY ");
	...
	private String getFromClause() {
		return "FROM FLRStklparameter AS stklparameter"
				+ " LEFT JOIN stklparameter.stklparametersprset AS stklparametersprset";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRStklparameter zeitmodell = (FLRStklparameter) scrollableResult
								.get(0);
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			, new String[] { "i_id", "c_nr", "stklparametersprset.c_bez","c_wert" }));

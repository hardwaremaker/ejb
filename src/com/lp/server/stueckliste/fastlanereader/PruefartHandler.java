...
public class PruefartHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final String FLR_PRUEFART = "flrpruefart.";
	public static final String FLR_PRUEFART_FROM_CLAUSE = " from FLRPruefart flrpruefart ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRPruefart pruefart = (FLRPruefart) o[0];

				Iterator<?> sprsetIterator = pruefart
						.getPruefartspr_set().iterator();

				rows[row][col++] = pruefart.getI_id();
				rows[row][col++] = pruefart.getC_nr();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRPruefartspr spr = (FLRPruefartspr) iterUebersetzungenI
					.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRPruefart AS flrpruefart"
					+ " LEFT JOIN flrpruefart.pruefartspr_set AS pruefartspr_set "
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("flrpruefart.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("flrpruefart.c_nr") < 0) {
				...
				orderBy.append(" flrpruefart.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRPruefart AS flrpruefart"
				+ " LEFT JOIN flrpruefart.pruefartspr_set AS pruefartspr_set";
	}
		...
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] {
							...
							QueryParameters.FLR_BREITE_SHARE_WITH_REST,
							QueryParameters.FLR_BREITE_SHARE_WITH_REST },
					new String[] { "flrpruefart.i_id",
							"flrpruefart.c_nr", "pruefartspr_set.c_bez" }));
		}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRPruefart flrPruefart = (FLRPruefart) scrollableResult
								.get(0);

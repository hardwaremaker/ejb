 ...
 * @version 1.0
 */
public class MontageartHandler extends UseCaseHandler {
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(montageart."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" montageart."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("montageart.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("montageart."
					+ StuecklisteFac.FLR_MONTAGEART_C_BEZ) < 0) {
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] { "id", StuecklisteFac.FLR_MONTAGEART_C_BEZ }));
		}

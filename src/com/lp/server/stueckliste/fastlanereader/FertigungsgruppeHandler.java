...
public class FertigungsgruppeHandler extends UseCaseHandler {
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(fertigungsgruppe."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" fertigungsgruppe."
								+ filterKriterien[i].kritName);
					}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("fertigungsgruppe."
				...
				orderBy.append("fertigungsgruppe.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("fertigungsgruppe.i_sort") < 0) {
				...
				orderBy.append(" fertigungsgruppe.i_sort ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "from FLRFertigungsgruppe fertigungsgruppe ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select fertigungsgruppe.i_id from FLRFertigungsgruppe fertigungsgruppe "

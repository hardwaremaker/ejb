...
import com.lp.server.fertigung.fastlanereader.generated.FLRLoslosklasse;
 ...
 * @version 1.0
 */
public class LoslosklasseHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final String FLR_LOSKLASSE = "flrloslosklasse.";
	private static final String FLR_LOSKLASSE_FROM_CLAUSE = " from FLRLoslosklasse flrloslosklasse ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRLoslosklasse losklasse = (FLRLoslosklasse) resultListIterator
						.next();
				rows[row][col++] = losklasse.getI_id();
				String sBez = getFertigungServiceFac()
						.uebersetzeLosklasseOptimal(
								losklasse.getFlrlosklasse().getI_id(), locUI,
								locUI);
				if (sBez == null) {
					// keine Uebersetzung mit dem UI-Locale vorhanden
					LosklasseDto losklasseDto = getFertigungServiceFac()
							.losklasseFindByPrimaryKey(
									losklasse.getFlrlosklasse().getI_id(),
									theClientDto);
					sBez = losklasseDto.getCNr();
				}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append(FLR_LOSKLASSE).append(
						FertigungFac.FLR_LOSKLASSE_I_ID).append(" ASC ");
				sortAdded = true;
			}
			if (orderBy
				...
				orderBy.append(" ").append(FLR_LOSKLASSE).append(

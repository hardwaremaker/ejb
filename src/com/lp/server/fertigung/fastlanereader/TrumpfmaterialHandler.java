 ...
 * @version 1.0
 */
public class TrumpfmaterialHandler extends UseCaseHandler {
	/**
	 * 
	 */
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			String queryString = "SELECT flrlossollmaterial,aspr.c_bez, (SELECT at.c_fehlercode FROM FLRArtikeltrutops at WHERE at.artikel_i_id=flrlossollmaterial.flrartikel.i_id) as fc_artikel, aspr.c_zbez from FLRLossollmaterial flrlossollmaterial  LEFT OUTER JOIN flrlossollmaterial.flrartikelliste.artikelsprset AS aspr  LEFT OUTER JOIN flrlossollmaterial.flrlos.flrkunde.flrpartner AS partnerK  LEFT OUTER JOIN flrlossollmaterial.flrlos.flrauftrag.flrkunde.flrpartner AS partner "
			...
			while (resultListIterator.hasNext()) {
				...
				if (Helper.short2boolean(losmat.getB_nachtraeglich())) {
					rowToAddCandidate[getTableColumnInformation().getViewIndex("lp.art")] = "N";
				} else {
					rowToAddCandidate[getTableColumnInformation().getViewIndex("lp.art")] = "S";
				}

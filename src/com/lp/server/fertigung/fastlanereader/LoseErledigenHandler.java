 ...
 * @version 1.0
 */
public class LoseErledigenHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			if (resultList.size() > 0) {
				...
				session = factory.openSession();
				session = setFilter(session);
				queryString = this.getFromClause() + whereClause + in
			...
			while (resultListIterator.hasNext()) {
				...
				rowToAddCandidate[getTableColumnInformation().getViewIndex(
						"lp.losnr")] = o[1];
				rowToAddCandidate[getTableColumnInformation().getViewIndex(
						"lp.menge")] = getUIObjectBigDecimalNachkommastellen(
								(BigDecimal) o[2], iNachkommastellenLosgroesse);
				rowToAddCandidate[getTableColumnInformation().getViewIndex(
						"lp.artikelnummer")] = o[5];
				rowToAddCandidate[getTableColumnInformation().getViewIndex(
						"lp.bezeichnung")] = o[6];
				rowToAddCandidate[getTableColumnInformation().getViewIndex(
						"fert.los.projekt")] = o[4];
				...
				if (o[12] != null && o[12].equals(new Short((short) 1))) {
					...
					if (lAnzahlZeitbuchungen > 0) {
						rowToAddCandidate[getTableColumnInformation().getViewIndex(
								"Color")] =  new Color(0, 153, 51);// GRUEN
					}
				}
				...
				if (m.size() > 0) {
					...
					rowToAddCandidate[getTableColumnInformation().getViewIndex(
							"lp.status")] =  getStatusMitUebersetzung(sStatus);
					...
					if (filterKriterien[i].kritName.equals("VJ")) {
						...
						try {
							...
							if (bLosnummerAuftragsbezogen == 0) {
								...
								// SP4455
								if (valueBelegnummernfilterAuftragsbezogen
										.length() > 14
										&& !valueBelegnummernfilterAuftragsbezogen
												.startsWith("'")) {
									valueBelegnummernfilterAuftragsbezogen = "'"
	...
	private String getFromClause() {
		return "SELECT distinct flrlos.i_id , flrlos.c_nr, flrlos.n_losgroesse,flrlos.status_c_nr,flrlos.c_projekt, flrlos.flrstueckliste.flrartikel.c_nr, aspr.c_bez, aspr.c_zbez, "
				...
				+ " flrlos.t_produktionsende, flrlos.flrauftrag.c_nr, flrlos.flrauftrag.c_bez, (select count(z.i_id) FROM FLRZeitdaten z WHERE z.i_belegartid=flrlos.i_id AND z.c_belegartnr='"
				+ LocaleFac.BELEGART_LOS
				+ "') , partner.c_name1nachnamefirmazeile1, aspr.c_zbez, flrlos.f_bewertung, flrlos.flrauftrag.c_bestellnummer "
	...
	private TableColumnInformation createColumnInformation(String mandant,
		...
		columns.add("lp.bestellnummer", String.class,
				getTextRespectUISpr("lp.bestellnummer", mandant, locUi),
				QueryParameters.FLR_BREITE_M, FLR_LOS
						+ FertigungFac.FLR_LOSREPORT_FLRAUFTRAG + ".c_bestellnummer");
		...
		columns.add("lp.fehlmenge", String.class,
				getTextRespectUISpr("lp.fehlmenge", mandant, locUi), 1,
				Facade.NICHT_SORTIERBAR);
	...
	private void setupParameters() {
		try {
			...
			iNachkommastellenLosgroesse = getMandantFac()
					.getNachkommastellenLosgroesse(theClientDto.getMandant());

...
public class LosgutschlechtDtoAssembler {
	public static LosgutschlechtDto createDto(Losgutschlecht losgutschlecht) {
		LosgutschlechtDto losgutschlechtDto = new LosgutschlechtDto();
		if (losgutschlecht != null) {
			losgutschlechtDto.setIId(losgutschlecht.getIId());
			losgutschlechtDto.setLossollarbeitsplanIId(losgutschlecht
					.getLossollarbeitsplanIId());
			losgutschlechtDto.setZeitdatenIId(losgutschlecht.getZeitdatenIId());
			losgutschlechtDto.setMaschinenzeitdatenIId(losgutschlecht.getMaschinenzeitdatenIId());
			losgutschlechtDto.setNGut(losgutschlecht.getNGut());
			losgutschlechtDto.setNInarbeit(losgutschlecht.getNInarbeit());
	...
	public static LosgutschlechtDto[] createDtos(Collection<?> losgutschlechts) {
		List<LosgutschlechtDto> list = new ArrayList<LosgutschlechtDto>();
		if (losgutschlechts != null) {
			Iterator<?> iterator = losgutschlechts.iterator();
			while (iterator.hasNext()) {
				list.add(createDto((Losgutschlecht) iterator.next()));
			}
		}

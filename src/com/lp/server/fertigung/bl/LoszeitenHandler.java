...
public class LoszeitenHandler extends UseCaseHandlerTabelle {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private AuftragzeitenDto[] aLoszeitenDtos = null;
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			for (int row = 0; row < getAnzahlZeilen(); row++) {
				...
				FilterKriterium fkAuswertung = aFilterKriterium[FertigungFac.IDX_KRIT_AUSWERTUNG];

				AuftragzeitenDto oAuftragzeitenDto = aLoszeitenDtos[iAktuellerAuftrag];

				// die erste Zeile pro Auftrag ist der Kopf
				boolean doAnzeige = false;

				if (fkAuswertung.kritName.equals(FertigungFac.KRIT_PERSONAL)) {
					if (hmKoepfe.containsKey(oAuftragzeitenDto
						...
						.equals(FertigungFac.KRIT_IDENT)) {
					if (hmKoepfe
				...
				if (doAnzeige) {
					...
					if (fkAuswertung.kritName
							.equals(FertigungFac.KRIT_PERSONAL)) {
				...
				if (oAuftragzeitenDto.getTsBeginn() != null) {
				...
				if (oAuftragzeitenDto.getTsEnde() != null) {
					rows[row][col++] = Helper.formatDatum(new Date(
							oAuftragzeitenDto.getTsEnde().getTime()),
							theClientDto.getLocUi());
					rows[row][col++] = Helper.formatTime(
							oAuftragzeitenDto.getTsEnde(),
							theClientDto.getLocUi());
				} else {
					rows[row][col++] = null;
					rows[row][col++] = null;
	...
	protected long getRowCountFromDataBase() {
		try {
			...
			personalZeitenDtos = null;
			maschinenZeitenDtos = null;
	...
	private void setTableProperties(boolean bFillData) throws Throwable {
		...
		FilterKriterium fkAuftrag = aFilterKriterium[FertigungFac.IDX_KRIT_LOS];

		// enthaelt KRIT_PERSONAL = true || KRIT_IDENT = true
		FilterKriterium fkAuswertung = aFilterKriterium[FertigungFac.IDX_KRIT_AUSWERTUNG];

		if (maschinenZeitenDtos == null) {
		...
		} else if (fkAuswertung.kritName.equals(FertigungFac.KRIT_IDENT)) {
			if (personalZeitenDtos == null) {
				personalZeitenDtos = getZeiterfassungFac()
						.getAllZeitenEinesBeleges(LocaleFac.BELEGART_LOS,
								new Integer(Integer.parseInt(fkAuftrag.value)),
		...
		aLoszeitenDtos = new AuftragzeitenDto[personalZeitenDtos.length
		...
		for (int i = 0; i < aLoszeitenDtos.length; i++) {
			if (fkAuswertung.kritName.equals(FertigungFac.KRIT_PERSONAL)) {
				hmKoepfe.put(aLoszeitenDtos[i].getSPersonalnummer(), null);
			} else if (fkAuswertung.kritName.equals(FertigungFac.KRIT_IDENT)) {
				hmKoepfe.put(aLoszeitenDtos[i].getSArtikelcnr(), null);
			}
		}

		int iAnzahlZeilen = hmKoepfe.size() + aLoszeitenDtos.length * 2;

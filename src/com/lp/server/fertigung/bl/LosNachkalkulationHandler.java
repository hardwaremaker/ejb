	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			for (int i = 0; i < zeit.length; i++) {
				rows[row][1] = zeit[i].getSBezeichnung();
				rows[row][2] = zeit[i].getBdSollmenge();
				rows[row][3] = zeit[i].getBdIstmenge();
				rows[row][4] = zeit[i].getBdSollmenge().subtract(
						zeit[i].getBdIstmenge());
				rows[row][5] = zeit[i].getBdSollmengeMaschine();
				rows[row][6] = zeit[i].getBdIstmengeMaschine();
				rows[row][7] = zeit[i].getBdSollmengeMaschine().subtract(
						zeit[i].getBdIstmengeMaschine());
				rows[row][8] = zeit[i].getBdSollpreis();
				rows[row][9] = zeit[i].getBdIstpreis();
				rows[row][10] = zeit[i].getBdSollpreis().subtract(
			...
			rows[row][1] = nkZeitSumme.getSBezeichnung();
			rows[row][2] = nkZeitSumme.getBdSollmenge();
			rows[row][3] = nkZeitSumme.getBdIstmenge();
			rows[row][4] = nkZeitSumme.getBdSollmenge().subtract(
					nkZeitSumme.getBdIstmenge());
			rows[row][5] = nkZeitSumme.getBdSollmengeMaschine();
			rows[row][6] = nkZeitSumme.getBdIstmengeMaschine();
			rows[row][7] = nkZeitSumme.getBdSollmengeMaschine().subtract(
					nkZeitSumme.getBdIstmengeMaschine());
			rows[row][8] = nkZeitSumme.getBdSollpreis();
			rows[row][9] = nkZeitSumme.getBdIstpreis();
			rows[row][10] = nkZeitSumme.getBdSollpreis().subtract(
			...
			rows[row][1] = mat.getSBezeichnung();
			rows[row][2] = mat.getBdSollmenge();
			rows[row][3] = mat.getBdIstmenge();
			rows[row][4] = mat.getBdSollmenge().subtract(mat.getBdIstmenge());
			rows[row][5] = mat.getBdSollmengeMaschine();
			rows[row][6] = mat.getBdIstmengeMaschine();
			rows[row][7] = mat.getBdSollmengeMaschine().subtract(
					mat.getBdIstmengeMaschine());
			rows[row][8] = mat.getBdSollpreis();
			rows[row][9] = mat.getBdIstpreis();
			rows[row][10] = mat.getBdSollpreis().subtract(mat.getBdIstpreis());
			row++;
			row++;
			rows[row][1] = nkGesamtSumme.getSBezeichnung();
			rows[row][2] = nkGesamtSumme.getBdSollmenge();
			rows[row][3] = nkGesamtSumme.getBdIstmenge();
			rows[row][4] = nkGesamtSumme.getBdSollmengeMaschine().subtract(
					nkGesamtSumme.getBdIstmengeMaschine());
			rows[row][5] = nkGesamtSumme.getBdSollmengeMaschine();
			rows[row][6] = nkGesamtSumme.getBdIstmengeMaschine();
			rows[row][7] = nkGesamtSumme.getBdSollmenge().subtract(
					nkGesamtSumme.getBdIstmenge());
			rows[row][8] = nkGesamtSumme.getBdSollpreis();
			rows[row][9] = nkGesamtSumme.getBdIstpreis();
			rows[row][10] = nkGesamtSumme.getBdSollpreis().subtract(
					nkGesamtSumme.getBdIstpreis());

			if (getFertigungFac().losgutschlechtFindAllFehler(losIId).length > 0) {
				row++;
				row++;
				rows[row][1] = getTextRespectUISpr(
						"fert.nachkalkulation.fehlervorhanden",
						theClientDto.getMandant(), theClientDto.getLocUi());

				rows[row][11] = Color.RED;
			}
			row++;

...
import com.lp.server.lieferschein.fastlanereader.generated.FLRBegruendung;
...
public class VerkettetHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRVerkettet verkettet = (FLRVerkettet) resultListIterator
						.next();

				rows[row][col++] = verkettet.getI_id();
				rows[row][col++] = verkettet.getFlrlieferschein_verkettet()
						.getC_nr();

				String proj_bestellnummer = "";
				if (verkettet.getFlrlieferschein_verkettet()
						.getC_bez_projektbezeichnung() != null) {
					proj_bestellnummer = verkettet
							.getFlrlieferschein_verkettet()
							.getC_bez_projektbezeichnung();
				}

				if (verkettet.getFlrlieferschein_verkettet()
						.getC_bestellnummer() != null) {
					proj_bestellnummer += " | "
							+ verkettet.getFlrlieferschein_verkettet()
									.getC_bestellnummer();
				}
				...
				if (verkettet.getFlrlieferschein_verkettet().getFlrrechnung() != null) {
					rows[row][col++] = verkettet.getFlrlieferschein_verkettet()
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRVerkettet AS verkettet "
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" verkettet." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("verkettet.flrlieferschein_verkettet.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("verkettet.flrlieferschein_verkettet.c_nr") < 0) {
				...
				orderBy.append(" verkettet.flrlieferschein_verkettet.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRVerkettet AS verkettet ";
	}
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					new Class[] { Integer.class, String.class, String.class,
							java.util.Date.class, Icon.class, String.class },
					new String[] {
							"i_id",
							getTextRespectUISpr("ls.lieferscheinnummer",
									mandantCNr, locUI),
							getTextRespectUISpr("ls.projektbestellnummer",
									mandantCNr, locUI),
					...
					new String[] {
							"i_id",
							"flrlieferschein_verkettet.c_nr",
							"flrlieferschein_verkettet."
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRVerkettet verkettet = (FLRVerkettet) scrollableResult
								.get(0);
						Integer c_nr = verkettet.getI_id();

 ...
 */
public class KategorieHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = KategorieHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				FLRKategorie kategorie = (FLRKategorie) resultListIterator
						.next();

				Iterator<?> sprsetIterator = kategorie
						.getKategorie_kategorie_set().iterator();

				rows[row][col++] = kategorie.getPK().getC_nr();
				rows[row][col++] = kategorie.getPK().getC_nr();
				rows[row][col++] = kategorie.getI_sort();
				rows[row][col++] = findSpr(theClientDto.getLocUiAsString(), theClientDto.getMandant(), sprsetIterator);
	...
	private String findSpr(String sLocaleI, String mandantCNr,
			Iterator<?> iterUebersetzungenI) {

		String sUebersetzung = null;
		while (iterUebersetzungenI.hasNext()) {
			FLRKategoriespr belegartspr = (FLRKategoriespr) iterUebersetzungenI
					.next();

			if (belegartspr.getLocale_c_nr().compareTo(sLocaleI) == 0
					&& belegartspr.getMandant_c_nr().compareTo(mandantCNr) == 0) {
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" kategorie." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("kategorie.i_sort ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("kategorie.i_sort") < 0) {
				...
				orderBy.append(" kategorie.i_sort ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRKategorie AS kategorie ";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRKategorie kategorie = (FLRKategorie) scrollableResult
								.get(0);
						String cNr = kategorie.getPK().getC_nr();
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { String.class,
					...
					"kategorie.c_nr", "kategorie.c_nr", "kategorie.i_sort",

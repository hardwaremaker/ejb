 ...
 */
public class ProjektTypHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = ProjektTypHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				FLRTyp typ = (FLRTyp) resultListIterator.next();
				Iterator<?> sprsetIterator = typ.getTyp_typ_set().iterator();
				rows[row][col++] = typ.getPK().getC_nr();
				rows[row][col++] = typ.getPK().getC_nr();
				rows[row][col++] = typ.getI_sort();
				rows[row][col++] = findSpr(sLocUI, typ.getPK().getMandant_c_nr(),
						sprsetIterator);
	...
	private String findSpr(String sLocaleI, String mandantCNr,
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRTypspr typspr = (FLRTypspr) iterUebersetzungenI.next();
			if (typspr.getLocale_c_nr().compareTo(sLocaleI) == 0
					&& typspr.getMandant_c_nr().compareTo(mandantCNr) == 0) {
				sUebersetzung = typspr.getC_bez();
				break;
			}
		}
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" typ." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("typ.i_sort ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("typ.i_sort") < 0) {
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRTyp typ = (FLRTyp) scrollableResult.get(0);
						String c_nr = typ.getPK().getC_nr();
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(new Class[] { String.class,
					...
					new int[] { QueryParameters.FLR_BREITE_SHARE_WITH_REST,
							...
							QueryParameters.FLR_BREITE_SHARE_WITH_REST },
					new String[] { "typ.c_nr", "typ.c_nr", "typ.i_sort",

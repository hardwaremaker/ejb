 ...
 */
public class ProjektStatusHandler extends UseCaseHandler {
	...
	public static final String FLR_PROJEKTSTATUS = "projektstatus.";
	public static final String FLR_PROJEKTSTATUS_FROM_CLAUSE = " from FLRProjektstatus projektstatus ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = ProjektStatusHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				FLRProjektstatus status = (FLRProjektstatus) resultListIterator
						.next();
				rows[row][col++] = status.getStatus_c_nr();
				rows[row][col++] = status.getStatus_c_nr();
				rows[row][col++] = status.getI_sort();
				row++;
				col = 0;
			}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRProjektstatus AS projektstatus"
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("projektstatus.i_sort ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("projektstatus.i_sort") < 0) {
				...
				orderBy.append(" projektstatus.i_sort ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRProjektstatus AS projektstatus";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRProjektstatus status = (FLRProjektstatus) scrollableResult
								.get(0);
						String c_nr = status.getStatus_c_nr();
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] { "projektstatus.status_c_nr",
							"projektstatus.status_c_nr", "projektstatus.i_sort" }));
		}

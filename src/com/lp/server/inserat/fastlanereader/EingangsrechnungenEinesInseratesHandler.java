...
public class EingangsrechnungenEinesInseratesHandler extends UseCaseHandler {
	/**
	 * 
	 */
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			String queryString = "SELECT flrinserater,(SELECT count(*) FROM FLRFinanzBelegbuchung as bb WHERE bb.belegart_c_nr='"
					+ LocaleFac.BELEGART_EINGANGSRECHNUNG
					+ "' AND bb.i_belegiid=eingangsrechnung.i_id  ) from FLRInserater as flrinserater left join flrinserater.flreingangsrechnung as eingangsrechnung "
			...
			while (resultListIterator.hasNext()) {
				...
				FLRInserater inserater = (FLRInserater) o[0];
				FLREingangsrechnung eingangsrechnung = inserater
				...
				rows[row][col++] = inserater.getN_betrag();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			session = factory.openSession();
			String queryString = "select count(*) from FLRInserater as flrinserater left join flrinserater.flreingangsrechnung as eingangsrechnung  "
					+ " left join eingangsrechnung.flrlieferant.flrpartner.flrlandplzort as flrlandplzort "
					+ " left join eingangsrechnung.flrlieferant.flrpartner.flrlandplzort.flrort as flrort "
					+ " left join eingangsrechnung.flrlieferant.flrpartner.flrlandplzort.flrland as flrland "
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" lower(flrinserater"
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				// +" DESC,"+
				"flrinserater.flreingangsrechnung."
						+ EingangsrechnungFac.FLR_ER_C_NR + " DESC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("flrinserater.flreingangsrechnung.c_nr") < 0) {
				...
				orderBy.append(" flrinserater.flreingangsrechnung.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select flrinserater.i_id from FLRInserater as flrinserater left join flrinserater.flreingangsrechnung as eingangsrechnung  "
					+ " left join eingangsrechnung.flrlieferant.flrpartner.flrlandplzort as flrlandplzort "
					+ " left join eingangsrechnung.flrlieferant.flrpartner.flrlandplzort.flrort as flrort "
					+ " left join eingangsrechnung.flrlieferant.flrpartner.flrlandplzort.flrland as flrland  "
						+ this.buildWhereClause() + this.buildOrderByClause();
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							getTextRespectUISpr("lp.nettobetrag", mandantCNr,
									locUI),
					...
					new String[] {
							...
							"eingangsrechnung."+EingangsrechnungFac.FLR_ER_EINGANGSRECHNUNGART_C_NR,
							"eingangsrechnung."+EingangsrechnungFac.FLR_ER_C_NR,
							"eingangsrechnung."+EingangsrechnungFac.FLR_ER_FLRLIEFERANT
									+ "."
							...
							// LKZ
									...
									"eingangsrechnung."+EingangsrechnungFac.FLR_ER_D_BELEGDATUM,
									"eingangsrechnung."+EingangsrechnungFac.FLR_ER_STATUS_C_NR,
									"eingangsrechnung."+EingangsrechnungFac.FLR_ER_C_LIEFERANTENRECHNUNGSNUMMER,
									"eingangsrechnung."+EingangsrechnungFac.FLR_ER_N_BETRAGFW,
									"eingangsrechnung."+EingangsrechnungFac.FLR_ER_WAEHRUNG_C_NR,
									"eingangsrechnung."+EingangsrechnungFac.FLR_ER_T_FIBUUEBERNAHME }));
		}

 ...
 * @version 1.0
 */
public class AgstklarbeitsplanHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRAgstklarbeitsplan stuecklistearbeitsplan = (FLRAgstklarbeitsplan) (o[0]);
				...
				double dGesamt = dStueckzeit + dRuestzeit;
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].isBIgnoreCase()) {
						where.append(" upper(agstklarbeitsplan."
								+ filterKriterien[i].kritName + ")");
					} else {
						where.append(" agstklarbeitsplan."
								+ filterKriterien[i].kritName);
					}
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName
						...
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append("agstklarbeitsplan."
				...
				orderBy.append("agstklarbeitsplan."
						+ AngebotstklFac.FLR_AGSTKLARBEITSPLAN_I_ARBEITSGANG
						+ " ,agstklarbeitsplan."
						+ AngebotstklFac.FLR_AGSTKLARBEITSPLAN_I_UNTERARBEITSGANG
						+ " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("agstklarbeitsplan."
					+ AngebotstklFac.FLR_AGSTKLARBEITSPLAN_I_ARBEITSGANG) < 0) {
	...
	private String getFromClause() {
		return "from FLRAgstklarbeitsplan agstklarbeitsplan "
				+ " left join agstklarbeitsplan.flrmaschine as flrmaschine ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select agstklarbeitsplan.i_id from FLRAgstklarbeitsplan agstklarbeitsplan "
			...
			if (super.getTableInfo() == null) {
				...
				setTableInfo(new TableInfo(
						...
						new String[] {
								...
								AngebotstklFac.FLR_AGSTKLARBEITSPLAN_FLRMASCHINE
										+ "."
										+ ZeiterfassungFac.FLR_MASCHINE_C_IDENTIFIKATIONSNR,
								AngebotstklFac.FLR_AGSTKLARBEITSPLAN_FLRARTIKEL
										+ ".c_nr",
								Facade.NICHT_SORTIERBAR,
								AngebotstklFac.FLR_AGSTKLARBEITSPLAN_L_RUESTZEIT,
								AngebotstklFac.FLR_AGSTKLARBEITSPLAN_L_STUECKZEIT,
								Facade.NICHT_SORTIERBAR }));
			}

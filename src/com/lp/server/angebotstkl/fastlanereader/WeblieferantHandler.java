...
import com.lp.server.angebotstkl.fastlanereader.generated.FLRWeblieferant;
 ...
 */
public class WeblieferantHandler extends UseCaseHandler {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final String FLR_FROM_CLAUSE = " from FLRWeblieferant flrWeblieferant "
			+ " LEFT JOIN flrWeblieferant.flrwebpartner AS flrwebpartner "
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			String queryString = "SELECT flrWeblieferant " + this.getFromClause() + this.buildWhereClause()
			...
			while (resultListIterator.hasNext()) {
				FLRWeblieferant flr = (FLRWeblieferant) resultListIterator
						.next();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append(" flrWeblieferant.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf(" flrWeblieferant.i_sort") < 0) {
				...
				orderBy.append(" ").append(" flrWeblieferant.i_sort").append(" ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select flrWeblieferant.i_id "

...
public class ProvisionsempfaengerHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = getPersonalFac().istPersonalAusgetreten(
						personal.getI_id(), getTimestamp(), theClientDto);

				rows[row][col++] = Helper.short2boolean(personal
						.getB_versteckt());
	...
	private String buildWhereClause() {
		...
		return " WHERE personal.mandant_c_nr='"
				+ theClientDto.getMandant()
				+ "' AND personal.i_id IN (SELECT kunde.personal_i_id_bekommeprovision FROM FLRKunde kunde)";
	}
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							...
							getTextRespectUISpr("pers.kurzzeichen", mandantCNr,
									locUI),
							getTextRespectUISpr("pers.ausgetreten", mandantCNr,
									locUI),
							getTextRespectUISpr("pers.versteckt", mandantCNr,
									locUI), },
					new int[] {
					...
					new String[] {
							...
							Facade.NICHT_SORTIERBAR, "b_versteckt" }));
		}

...
import com.lp.server.partner.fastlanereader.generated.FLRPartnerkommentarart;
 ...
 * @version 1.0
 */
public class PartnerkommentarartHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRPartnerkommentarart art = (FLRPartnerkommentarart) resultListIterator
						.next();

				rows[row][col++] = art.getI_id();
				rows[row][col++] = art.getC_bez();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRPartnerkommentarart AS partnerkommentarart "
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("partnerkommentarart.c_bez ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("partnerkommentarart.i_id") < 0) {
				...
				orderBy.append(" partnerkommentarart.i_id ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRPartnerkommentarart AS partnerkommentarart";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRPartnerkommentarart partnerkommentarart = (FLRPartnerkommentarart) scrollableResult
								.get(0);
						Integer iId = partnerkommentarart.getI_id();
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			...
			new String[] { "FLRPartnerkommentarart.i_id",

			"FLRPartnerkommentarart.c_bez" }));

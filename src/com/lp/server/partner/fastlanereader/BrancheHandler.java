...
import com.lp.server.partner.fastlanereader.generated.FLRBranche;
import com.lp.server.partner.fastlanereader.generated.FLRBranchespr;
 ...
 */
public class BrancheHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				Iterator<?> sprsetIterator = branche.getBranche_branchespr_set()
						.iterator();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRBranchespr branchespr = (FLRBranchespr) iterUebersetzungenI
					.next();
			if (branchespr.getLocale().getC_nr().compareTo(sLocaleI) == 0) {
				sUebersetzung = branchespr.getC_bez();
				break;
			}
		}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRBranche AS branche"
					+ " LEFT JOIN branche.branche_branchespr_set AS branche_branchespr_set "
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" branche." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("branche.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("branche.c_nr") < 0) {
	...
	private String getFromClause() {
		return " FROM FLRBranche AS branche"
				+ " LEFT JOIN branche.branche_branchespr_set AS branche_branchespr_set";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRBranche branche = (FLRBranche) scrollableResult
								.get(0);

...
public class PartnerTelefonzeitenHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = PartnerTelefonzeitenHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = HelperServer.formatPersonAusFLRPErsonal(reise.getFlrpersonal());
				rows[row][col++] = reise.getT_von();
				rows[row][col++] = reise.getT_bis();

				if (reise.getFlransprechpartner() != null) {
					String firma = reise.getFlransprechpartner().getFlrpartneransprechpartner()
							.getC_name1nachnamefirmazeile1();
					if (reise.getFlransprechpartner().getFlrpartneransprechpartner()
							.getC_name2vornamefirmazeile2() != null) {
						firma += " " + reise.getFlransprechpartner().getFlrpartneransprechpartner()
				...
				if (reise.getX_kommentarext() != null || reise.getX_kommentarint() != null) {

					String text = "";

					if (reise.getX_kommentarext() != null ) {
						text += "<b>Kommentar extern:</b>\n" + reise.getX_kommentarext();
						text = text.replaceAll("\n", "<br>");
					}

					if (reise.getX_kommentarint() != null) {
						text += "<br><b>Kommentar intern:</b>\n" + reise.getX_kommentarint();
						text = text.replaceAll("\n", "<br>");
					}
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].kritName.equals("c_volltext")) {
						...
						where.append(" " + filterKriterien[i].operator);
						where.append(" " + filterKriterien[i].value.toLowerCase() + ")");
					} else if (filterKriterien[i].kritName.equals(
						...
						if (bSuchenInklusiveKbez) {
							...
							where.append(" " + filterKriterien[i].value.toLowerCase());
							where.append(
									" OR lower(telefonzeiten.flransprechpartner.flrpartneransprechpartner.c_kbez)");
							where.append(" " + filterKriterien[i].operator);
							where.append(" " + filterKriterien[i].value.toLowerCase() + ")");
						} else {
		...
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(
					new Class[] { Integer.class, String.class, java.sql.Timestamp.class, java.sql.Timestamp.class,
							String.class, String.class, java.sql.Timestamp.class, java.sql.Timestamp.class,
							Color.class },
					new String[] { "Id",
							getTextRespectUISpr("lp.personal", theClientDto.getMandant(),
							...
							getTextRespectUISpr("part.ansprechpartner", theClientDto.getMandant(),
									theClientDto.getLocUi()),
					...
					new int[] { -1, // diese Spalte wird ausgeblendet
							...
							QueryParameters.FLR_BREITE_XL, QueryParameters.FLR_BREITE_SHARE_WITH_REST,
							QueryParameters.FLR_BREITE_XM, QueryParameters.FLR_BREITE_XM, },

					new String[] { "i_id",
							ZeiterfassungFac.FLR_TELEFONZEITEN_FLRPERSONAL + ".flrpartner.c_name1nachnamefirmazeile1",
							ZeiterfassungFac.FLR_TELEFONZEITEN_T_VON, ZeiterfassungFac.FLR_TELEFONZEITEN_T_BIS,
							ZeiterfassungFac.FLR_TELEFONZEITEN_FLRANSPRECHPARTNER + ".flrpartneransprechpartner."

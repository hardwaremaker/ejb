...
public class BankHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = bank.getFlrpartner()
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if(filterKriterien[i].kritName.equals("c_blz")){
						where.append(" ( lower(bank.c_blz) LIKE "+filterKriterien[i].value.toLowerCase()+" OR   lower(bank.c_bic) LIKE "+filterKriterien[i].value.toLowerCase()+")) ");
					} else {
						if (filterKriterien[i].isBIgnoreCase()) {
							where.append(" lower(bank."
									+ filterKriterien[i].kritName + ")");
						} else {
							where.append(" bank." + filterKriterien[i].kritName);
						}
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("bank." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy
						.append("bank.flrpartner.c_name1nachnamefirmazeile1 ASC ");
				sortAdded = true;
			}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select bank.i_id from FLRBank bank "

...
import com.lp.server.partner.fastlanereader.generated.FLRIdentifikation;
 ...
 * @version 1.0
 */
public class IdentifikationHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRIdentifikation massnahme = (FLRIdentifikation) o[0];
	...
	private String getFromClause() {
		return "from FLRIdentifikation if LEFT JOIN if.sprset AS sprset";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				session = setFilter(session);
				String queryString = "select if.i_id from FLRIdentifikation if  LEFT JOIN if.sprset AS sprset"

...
public class BankDtoAssembler {
	public static BankDto createDto(Bank bank) {
	...
	public static BankDto[] createDtos(Collection<?> banks) {
		List<BankDto> list = new ArrayList<BankDto>();
		if (banks != null) {
			Iterator<?> iterator = banks.iterator();
			while (iterator.hasNext()) {
				list.add(createDto((Bank) iterator.next()));
			}
		}
		BankDto[] returnArray = new BankDto[list.size()];
		return (BankDto[]) list.toArray(returnArray);
	}
}

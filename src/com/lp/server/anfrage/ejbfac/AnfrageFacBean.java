...
public class AnfrageFacBean extends Facade implements AnfrageFac, IAktivierbar {
	 ...
	 */
	public Integer createAnfrage(AnfrageDto anfrageDtoI, TheClientDto theClientDto) throws EJBExceptionLP {
		...
		try {
			...
			LpBelegnummerFormat f = getBelegnummerGeneratorObj().getBelegnummernFormat(anfrageDtoI.getMandantCNr());

			Integer iGeschaeftsjahr = getParameterFac().getGeschaeftsjahr(anfrageDtoI.getMandantCNr(),
					anfrageDtoI.getTBelegdatum());

			LpBelegnummer bnr = getBelegnummerGeneratorObj().getNextBelegNr(iGeschaeftsjahr, PKConst.PK_ANFRAGE,
			...
			if (anfrageDtoI.getNBankspeseninanfragewaehrung() == null) {
				anfrageDtoI.setNBankspeseninanfragewaehrung(BigDecimal.ZERO);
			}
			if (anfrageDtoI.getNZollkosteninanfragewaehrung() == null) {
				anfrageDtoI.setNZollkosteninanfragewaehrung(BigDecimal.ZERO);
			}
			if (anfrageDtoI.getNSonstigespeseninanfragewaehrung() == null) {
				anfrageDtoI.setNSonstigespeseninanfragewaehrung(BigDecimal.ZERO);
			}

			Anfrage anfrage = new Anfrage(anfrageDtoI.getIId(), anfrageDtoI.getCNr(), anfrageDtoI.getMandantCNr(),
					anfrageDtoI.getStatusCNr(), anfrageDtoI.getArtCNr(), anfrageDtoI.getBelegartCNr(),
					anfrageDtoI.getTBelegdatum(), anfrageDtoI.getWaehrungCNr(),
					anfrageDtoI.getFWechselkursmandantwaehrungzubelegwaehrung(), anfrageDtoI.getKostenstelleIId(),
					anfrageDtoI.getFAllgemeinerRabattsatz(), anfrageDtoI.getNTransportkosteninanfragewaehrung(),
					anfrageDtoI.getBelegtextIIdKopftext(), anfrageDtoI.getBelegtextIIdFusstext(),
					anfrageDtoI.getPersonalIIdAnlegen(), anfrageDtoI.getPersonalIIdAendern(),
					anfrageDtoI.getNZollkosteninanfragewaehrung(), anfrageDtoI.getNBankspeseninanfragewaehrung(),
					anfrageDtoI.getNBankspeseninanfragewaehrung());
	 ...
	 * @throws EJBExceptionLP Ausnahme
	 */
	public void storniereAnfrage(Integer iIdAnfrageI, TheClientDto theClientDto) throws EJBExceptionLP {
		...
		if (anfrage == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
					"Fehlerb beim stornieren der Anfrage. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
		}
	...
	public Long[] getAngefragteUndAngeboteneMenge(Integer artikelIId, TheClientDto theClientDto) {
		...
		Long[] angefrageUndAngeboten = new Long[] { 0L, 0L };

		int iBereitsAngefragt = 90;
		try {
			ParametermandantDto param = getParameterFac().getMandantparameter(theClientDto.getMandant(),
					ParameterFac.KATEGORIE_ANFRAGE, ParameterFac.PARAMETER_BEREITS_ANGEFRAGT);
			iBereitsAngefragt = (Integer) param.getCWertAsObject();
		} catch (RemoteException e) {
			throwEJBExceptionLPRespectOld(e);
		}

		java.sql.Date dDatumFuerBereitsAngefragt = Helper
				.addiereTageZuDatum(new java.sql.Date(System.currentTimeMillis()), -iBereitsAngefragt);

		Session session = FLRSessionFactory.getFactory().openSession();

		String queryString = "SELECT count(*) FROM FLRAnfrageposition afp WHERE afp.flranfrage.t_belegdatum >'"
				+ Helper.formatDateWithSlashes(dDatumFuerBereitsAngefragt) + "' AND afp.flrartikel.i_id=" + artikelIId
				+ " AND afp.flranfrage.anfragestatus_c_nr<>'" + LocaleFac.STATUS_STORNIERT + "'";
		...
		if (resultListIteratorFTGruppe.hasNext()) {
			angefrageUndAngeboten[0] = (Long) resultListIteratorFTGruppe.next();
		}

		session = FLRSessionFactory.getFactory().openSession();

		queryString = "SELECT count(*) FROM FLRAnfragepositionlieferdaten afp WHERE afp.flranfrageposition.flranfrage.t_belegdatum >'"
				+ Helper.formatDateWithSlashes(dDatumFuerBereitsAngefragt)
				+ "' AND afp.flranfrageposition.flrartikel.i_id=" + artikelIId
				+ " AND afp.n_anliefermenge > 0 AND afp.flranfrageposition.flranfrage.anfragestatus_c_nr<>'"
		...
		if (resultListIteratorFTGruppe.hasNext()) {
			angefrageUndAngeboten[1] = (Long) resultListIteratorFTGruppe.next();
		}

		return angefrageUndAngeboten;
	}

	/**
	 * Eine bestehende Anfrage aktualisieren.
	 * 
	 * @param anfrageDtoI     die Anfrage
	...
	public void updateAnfrage(AnfrageDto anfrageDtoI, String waehrungOriCNrI, boolean bAufAngelegtZuruecksetzen,
		...
		try {
			...
			// berechnet werden
			if (waehrungOriCNrI != null && !waehrungOriCNrI.equals(anfrageDtoI.getWaehrungCNr())) {
				AnfragepositionDto[] aAnfragepositionDto = getAnfragepositionFac()
						.anfragepositionFindByAnfrage(anfrageDtoI.getIId(), theClientDto);

				BigDecimal ffWechselkurs = getLocaleFac().getWechselkurs2(waehrungOriCNrI, anfrageDtoI.getWaehrungCNr(),
						theClientDto);

				for (int i = 0; i < aAnfragepositionDto.length; i++) {
					if (aAnfragepositionDto[i].getNMenge() != null && aAnfragepositionDto[i].getNRichtpreis() != null) {
						// die Positionswerte neu berechnen und abspeichern
						BigDecimal nRichtpreisInNeuerWaehrung = aAnfragepositionDto[i].getNRichtpreis()
								.multiply(ffWechselkurs);

						aAnfragepositionDto[i].setNRichtpreis(nRichtpreisInNeuerWaehrung);

						getAnfragepositionFac().updateAnfrageposition(aAnfragepositionDto[i], theClientDto);
						...
						if (anfragepositionlieferdatenDto != null) {
							BigDecimal nNettogesamtpreis = anfragepositionlieferdatenDto.getNNettogesamtpreis()
									.multiply(ffWechselkurs);

							anfragepositionlieferdatenDto.setNNettogesamtpreis(nNettogesamtpreis);

							getAnfragepositionFac().updateAnfragepositionlieferdaten(anfragepositionlieferdatenDto,
									theClientDto);

							getAnfragepositionFac().befuelleZusaetzlichesPreisfeld(
									anfragepositionlieferdatenDto.getIId(), theClientDto);
						}
					}
				}
			}
			...
			if (anfrage == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
						"Fehler bei Update Anfrage. Es gibt keine anfrage mit iid " + anfrageDtoI.getIId()
								+ " anfrageDto.toString:" + anfrageDtoI.toString());
			}

			Integer iGJAlt = getParameterFac().getGeschaeftsjahr(theClientDto.getMandant(), anfrage.getTBelegdatum());
			...
			if (!iGJNeu.equals(iGJAlt)) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BELEG_DARF_NICHT_IN_EIN_ANDERES_GJ_UMDATIERT_WERDEN,
						new Exception("Es wurde versucht, die AF " + anfrageDtoI.getCNr() + " auf "
								+ anfrageDtoI.getTBelegdatum() + " (GJ " + iGJNeu + ") umzudatieren"));
			}

			setAnfrageFromAnfrageDto(anfrage, anfrageDtoI);
			pruefeUndSetzeAnfragestatusBeiAenderung(anfrageDtoI.getIId(), bAufAngelegtZuruecksetzen, theClientDto);
			// }
			// catch (FinderException ex) {
			// throw new
	...
	/**
	 * Die Kopfdaten einer Anfrage duerfen nur in bestimmten Stati geaendert werden.
	 * <br>
	 * Nachdem eine Anfrage geaendert wurde, befindet er sich im Status ANGELEGT.
	 * 
	 * @param iIdAnfrageI PK der Anfrage
	 ...
	 * @throws EJBExceptionLP Ausnahme
	 */
	public void manuellErledigen(Integer iIdAnfrageI, Integer erledigungsgrundIId, TheClientDto theClientDto)
			throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		if (anfrage == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
					"Fehler beim manuell erledigen der Anfrage. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
		}

		if (anfrage.getAnfragestatusCNr().equals(AnfrageServiceFac.ANFRAGESTATUS_ERFASST)
				|| anfrage.getAnfragestatusCNr().equals(AnfrageServiceFac.ANFRAGESTATUS_OFFEN)) {
			...
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_STATUS, new Exception(
					"Anfrage kann nicht manuell erledigt werden, Status : " + anfrage.getAnfragestatusCNr()));
		}
		// }
		// catch (FinderException ex) {
		// throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
	 ...
	 * @throws EJBExceptionLP
	 */
	public void erledigungAufheben(Integer iIdAnfrageI, TheClientDto theClientDto) throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		if (anfrage == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
					"Fehlerb beim Aufheben der erledigung. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
		}

		if (anfrage.getAnfragestatusCNr().equals(AnfrageServiceFac.ANFRAGESTATUS_ERLEDIGT)) {
			if (anfrage.getPersonalIIdManuellerledigt() != null && anfrage.getTManuellerledigt() != null) {

				// Wenn bereits Lieferdaten erfasst wurden, dann ist der Status
				// erfasst
				boolean bErfasst = false;
				try {
					AnfragepositionDto[] anfPosDtos = getAnfragepositionFac().anfragepositionFindByAnfrage(iIdAnfrageI,
							theClientDto);

					for (AnfragepositionDto anfPosDto : anfPosDtos) {
						AnfragepositionlieferdatenDto lfDto = getAnfragepositionFac()
								.anfragepositionlieferdatenFindByAnfragepositionIIdOhneExc(anfPosDto.getIId());

						if (lfDto != null && Helper.short2boolean(lfDto.getBErfasst())) {
							bErfasst = true;
							break;
						}
				...
				myLogger.logKritisch(
						"Status Erledigt wurde aufgehoben, obwohl die Anfrage nicht manuell erledigt wurde, AnfrageIId: "
								+ iIdAnfrageI);
			}
		} else {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_STATUS,
					new Exception("Die Erledigung der Anfrage kann nicht aufgehoben werden, Status: "
	...
	public AnfrageDto anfrageFindByPrimaryKey(Integer iIdAnfrageI, TheClientDto theClientDto) throws EJBExceptionLP {
		// check2(cNrUserI);
		// checkAnfrageIId(iIdAnfrageI);
		...
		if (anfrageDto == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
					"Fehler bei AnfrageFindByPrimaryKey. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
		}
	...
	public AnfrageDto[] anfrageFindByMandantCNr(String cNrMandantI, TheClientDto theClientDto) throws EJBExceptionLP {
		// check2(cNrUserI);

		if (cNrMandantI == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_PARAMETER_IS_NULL, new Exception("cNrMandantI == null"));
		}
	...
	public AnfrageDto[] anfrageFindByAnfrageIIdLiefergruppenanfrage(Integer iIdAnfrageLiefergruppenanfrageI,
		...
		myLogger.info("iIdAnfrageLiefergruppenanfrageI: " + iIdAnfrageLiefergruppenanfrageI);

		AnfrageDto[] aAnfrageDto = null;

		// try {
		Query query = em.createNamedQuery("AnfragefindByAnfrageIIdLiefergruppenanfrage");
	...
	public AnfrageDto[] anfrageFindByAnsprechpartnerlieferantIIdMandantCNr(Integer iAnsprechpartnerLieferantIId,
			String cNrMandantI, TheClientDto theClientDto) throws EJBExceptionLP {

		if (iAnsprechpartnerLieferantIId == null || cNrMandantI == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_PARAMETER_IS_NULL,
					new Exception("iAnsprechpartnerLieferantIId == null || cNrMandantI == null"));
		}

		myLogger.info("iAnsprechpartnerLieferantIId: " + iAnsprechpartnerLieferantIId);

		AnfrageDto[] aAnfrageDto = null;

		// try {
		Query query = em.createNamedQuery("AnfragefindByAnsprechpartnerlieferantIIdMandantCNr");
		...
		if (aAnfrageDto.length == 0) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FIND,
					new Exception("Es wurden keine Anfragen mit der iAnsprechpartnerLieferantIId "
	...
	public AnfrageDto[] anfrageFindByAnsprechpartnerlieferantIIdMandantCNrOhneExc(Integer iAnsprechpartnerLieferantIId,
	...
	public AnfrageDto[] anfrageFindByLieferantIIdAnfrageadresseMandantCNr(Integer iLieferantIIdAnfrageadresseI,
			String cNrMandantI, TheClientDto theClientDto) throws EJBExceptionLP {

		if (iLieferantIIdAnfrageadresseI == null || cNrMandantI == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_PARAMETER_IS_NULL,
					new Exception("iLieferantIIdAnfrageadresseI == null || cNrMandantI == null"));
		}

		myLogger.info("iLieferantIIdAnfrageadresseI: " + iLieferantIIdAnfrageadresseI);

		AnfrageDto[] aAnfrageDto = null;

		// try {
		Query query = em.createNamedQuery("AnfragefindByLieferantIIdAnfrageadresseMandantCNr");
	...
	public AnfrageDto[] anfrageFindByLieferantIIdAnfrageadresseMandantCNrOhneExc(Integer iLieferantIIdAnfrageadresseI,
			String cNrMandantI, TheClientDto theClientDto) {
	...
	private void setAnfrageFromAnfrageDto(Anfrage anfrage, AnfrageDto anfrageDto) {
		...
		anfrage.setLieferantIIdAnfrageadresse(anfrageDto.getLieferantIIdAnfrageadresse());
		anfrage.setAnsprechpartnerIIdLieferant(anfrageDto.getAnsprechpartnerIIdLieferant());
		...
		anfrage.setFWechselkursmandantwaehrungzuanfragewaehrung(
				anfrageDto.getFWechselkursmandantwaehrungzubelegwaehrung());
		...
		anfrage.setFAllgemeinerrabattsatz(anfrageDto.getFAllgemeinerRabattsatz());
		...
		anfrage.setNGesamtanfragewertinanfragewaehrung(anfrageDto.getNGesamtwertinbelegwaehrung());
		anfrage.setNTransportkosteninanfragewaehrung(anfrageDto.getNTransportkosteninanfragewaehrung());
		...
		anfrage.setPersonalIIdManuellerledigt(anfrageDto.getPersonalIIdManuellerledigt());
		anfrage.setTManuellerledigt(anfrageDto.getTManuellerledigt());
		anfrage.setAnfrageIIdLiefergruppenanfrage(anfrageDto.getAnfrageIIdLiefergruppenanfrage());
		anfrage.setCLieferartort(anfrageDto.getCLieferartort());
		anfrage.setProjektIId(anfrageDto.getProjektIId());
		anfrage.setAnfrageerledigungsgrundIId(anfrageDto.getAnfrageerledigungsgrundIId());
		anfrage.setPartnerIIdLieferadresse(anfrageDto.getPartnerIIdLieferadresse());
		anfrage.setAnsprechpartnerIIdLieferadresse(anfrageDto.getAnsprechpartnerIIdLieferadresse());
		anfrage.setTAbgabetermin(anfrageDto.getTAbgabetermin());
		anfrage.setNBankspeseninanfragewaehrung(anfrageDto.getNBankspeseninanfragewaehrung());
		anfrage.setNZollkosteninanfragewaehrung(anfrageDto.getNZollkosteninanfragewaehrung());
		anfrage.setNSonstigespeseninanfragewaehrung(anfrageDto.getNSonstigespeseninanfragewaehrung());
		anfrage.setPersonalIIdAnfrager(anfrageDto.getPersonalIIdAnfrager());
		anfrage.setTPreisgueltigab(anfrageDto.getTPreisgueltigab());
		em.merge(anfrage);
		em.flush();
	}
	...
	private void checkAnfrageDto(AnfrageDto anfrageDtoI) throws EJBExceptionLP {
		if (anfrageDtoI == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_DTO_IS_NULL, new Exception("anfrageDtoI == null"));
		}
	 ...
	 * Nachdem eine Anfrage geaendert wurde, befindet sie sich im Status 'Angelegt'.
	 * 
	 * @param iIdAnfrageI  PK der Anfrage
	 * @param theClientDto String
	 * @throws EJBExceptionLP
	 */
	public void pruefeUndSetzeAnfragestatusBeiAenderung(Integer iIdAnfrageI, boolean bSetzeAufAngelegt,
			TheClientDto theClientDto) throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
	 ...
	 */
	public BigDecimal berechneNettowertGesamt(Integer iIdAnfrageI, TheClientDto theClientDto) throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		try {
			...
			if (anfrage == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
						"Fehler bei berechneNettoGesamtwert. Es gibt keine ANfrage mit iid " + iIdAnfrageI);
			}

			// Step 1: Wenn der status STORNIERT ist, 0 zurueckgeben obwohl der
			// Anfragewert noch in der Tabelle steht
			if (anfrage.getAnfragestatusCNr().equals(AnfrageServiceFac.ANFRAGESTATUS_STORNIERT)) {
				...
				for (int i = 0; i < aAnfragepositionDto.length; i++) {
					...
					if (aAnfragepositionDto[i].getPositionsartCNr().equals(AnfrageServiceFac.ANFRAGEPOSITIONART_IDENT)
						...
						// beruecksichtigen
						if (anfragepositionlieferdatenDto != null
								&& anfragepositionlieferdatenDto.getNAnliefermenge() != null
								&& anfragepositionlieferdatenDto.getNAnliefermenge().doubleValue() > 0) {

							BigDecimal bdAnliefermenge = anfragepositionlieferdatenDto.getNAnliefermenge();

							// Grundlage ist der Nettogesamtwertminusrabatt der
							// Position in Anfragewaehrung
							BigDecimal wertDerPosition = bdAnliefermenge
									.multiply(anfragepositionlieferdatenDto.getNNettogesamtpreisminusrabatt());
	 ...
	 * @throws EJBExceptionLP Ausnahme
	 */
	public void updateAnfrageKonditionen(AnfrageDto anfrageDto, TheClientDto theClientDto) throws EJBExceptionLP {
	...
	public void updateAnfrageLieferKonditionen(Integer iIdAnfrageI, TheClientDto theClientDto) throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		try {
			...
			for (int i = 0; i < aAnfragepositionDto.length; i++) {
				AnfragepositionlieferdatenDto anfragepositionlieferdatenDto = getAnfragepositionFac()
						.anfragepositionlieferdatenFindByAnfragepositionIIdOhneExc(aAnfragepositionDto[i].getIId());

				if (anfragepositionlieferdatenDto != null) {
					getAnfragepositionFac().befuelleZusaetzlichesPreisfeld(anfragepositionlieferdatenDto.getIId(),
							theClientDto);
				}
			}
			...
			if (anfrage == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
						"Fehler bei updateAnfrageKonditionen. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
			}
			...
			if (!anfrage.getAnfragestatusCNr().equals(AnfrageServiceFac.ANFRAGESTATUS_ANGELEGT)) {
				anfrage.setNGesamtanfragewertinanfragewaehrung(berechneNettowertGesamt(iIdAnfrageI, theClientDto));
				em.merge(anfrage);
			}
		} catch (RemoteException ex) {
			// exccatch: immer so!
	 ...
	 * 
	 * @param iIdAnfrageI  PK der Anfrage
	 * @param timestampI   der Zeitpunkt des Drucks
	 * @param theClientDto der aktuelle Benutzer
	 * @throws EJBExceptionLP Ausnahme
	 */
	public void setzeDruckzeitpunkt(Integer iIdAnfrageI, Timestamp timestampI, TheClientDto theClientDto)
			throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		if (anfrage == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
					"Fehler bei setzeDruckzeitpunkt. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
		}
	 ...
	 * @throws EJBExceptionLP Ausnahme
	 */
	public String getAnfragekennung(Integer iIdAnfrageI, Locale localeI, TheClientDto theClientDto)
			throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		try {
			...
			if (anfrage == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
						"Fehler bei getAnfragenkennung. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
			}

			if (anfrage.getAnfrageartCNr().equals(AnfrageServiceFac.ANFRAGEART_LIEFERANT)) {
				buff.append(getTextRespectUISpr("anf.anfragekennung", theClientDto.getMandant(), localeI));
			} else if (anfrage.getAnfrageartCNr().equals(AnfrageServiceFac.ANFRAGEART_LIEFERGRUPPE)) {
				buff.append(getTextRespectUISpr("anf.liefergruppenanfragekennung", theClientDto.getMandant(), localeI));
			}

			buff.append(" ");

			BelegartDto belegartDto = getLocaleFac().belegartFindByPrimaryKey(anfrage.getBelegartCNr(), theClientDto);
	 ...
	 * @throws EJBExceptionLP Ausnahme
	 */
	public void stornoAufheben(Integer iIdAnfrageI, TheClientDto theClientDto) throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		if (anfrage == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
					"Fehler bei stornoAufheben. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
		}
		...
		if (anfrage.getAnfragestatusCNr().equals(AnfrageServiceFac.ANFRAGESTATUS_STORNIERT)) {
			...
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_STATUS, new Exception(
					"Storno der Bestellung kann nicht aufgehoben werden, Status : " + anfrage.getAnfragestatusCNr()));
		}
		// }
		// catch (FinderException ex) {
		// throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
	 ...
	 * 
	 * @param cNrStatusI   der neue Status der Anfrage
	 * @param iIdAnfrageI  PK der Anfrage
	 * @param theClientDto der aktuelle Benutzer
	 * @throws EJBExceptionLP Ausnahme
	 */
	public void setzeAnfragestatus(String cNrStatusI, Integer iIdAnfrageI, TheClientDto theClientDto) {
		checkAnfrageIId(iIdAnfrageI);

		if (cNrStatusI == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_PARAMETER_IS_NULL, new Exception("cNrStatusI == null"));
		}
		try {
			...
			if (cNrStatusI.equals(AnfrageServiceFac.ANFRAGESTATUS_OFFEN)
					|| cNrStatusI.equals(AnfrageServiceFac.ANFRAGESTATUS_ERFASST)
					|| cNrStatusI.equals(AnfrageServiceFac.ANFRAGESTATUS_ERLEDIGT)) {
				if (getAnfragepositionFac().getAnzahlMengenbehafteteAnfragepositionen(iIdAnfrageI, theClientDto) == 0) {
					ArrayList<Object> al = new ArrayList<Object>();
					al.add(iIdAnfrageI);
					throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BELEG_HAT_KEINE_POSITIONEN, al, new Exception(
			...
			if (anfrage == null) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
						"Fehler bei setzeAnfrageStatus. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
			}
	...
	public void setzeNettogesamtwert(Integer iIdAnfrageI, BigDecimal nNettogesamtwertI, TheClientDto theClientDto)
			throws EJBExceptionLP {
		checkAnfrageIId(iIdAnfrageI);
		...
		if (anfrage == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY,
					"Fehler bei setzeNEttoGesamtwert. Es gibt keine Anfrage mit iid " + iIdAnfrageI);
		}
	...
	public BigDecimal berechneEinkaufswertIst(Integer iIdAnfrageI, String sArtikelartI, TheClientDto theClientDto) {

		BigDecimal bdEKwertIstO = Helper.getBigDecimalNull();
		AnfragepositionDto[] aAnfragepositionDtos = null;
		try {
			aAnfragepositionDtos = getAnfragepositionFac().anfragepositionFindByAnfrage(iIdAnfrageI, theClientDto);
		} catch (RemoteException ex) {
			throwEJBExceptionLPRespectOld(ex);
		}
		for (int i = 0; i < aAnfragepositionDtos.length; i++) {
			...
			if (aAnfragepositionDtos[i].getNMenge() != null && aAnfragepositionDtos[i].getArtikelIId() != null) {

				if (aAnfragepositionDtos[i].getPositioniIdArtikelset() == null) {

					ArtikelDto oArtikelDto = getArtikelFac()
							.artikelFindByPrimaryKey(aAnfragepositionDtos[i].getArtikelIId(), theClientDto);

					BigDecimal preis = new BigDecimal(0);

					AnfragepositionlieferdatenDto lfDaten = null;
					try {
						lfDaten = getAnfragepositionFac().anfragepositionlieferdatenFindByAnfragepositionIIdOhneExc(
								aAnfragepositionDtos[i].getIId());
					} catch (RemoteException e) {
						throwEJBExceptionLPRespectOld(e);
					}

					if (lfDaten != null && lfDaten.getNNettogesamtpreisminusrabatt() != null) {
						preis = lfDaten.getNNettogesamtpreisminusrabatt();
					}

					BigDecimal bdBeitragDieserPosition = aAnfragepositionDtos[i].getNMenge().multiply(preis);
	...
	public Integer erzeugeAnfrageAusAnfrage(Integer anfrageIIdVorhanden, TheClientDto theClientDto) {
		...
		AnfrageDto anfrageBasisDto = anfrageFindByPrimaryKey(anfrageIIdVorhanden, theClientDto);

		Integer iIdAnfrageKopie = null;

		try {
			AnfrageDto anfrageDto = (AnfrageDto) anfrageBasisDto.clone();

			anfrageDto.setFWechselkursmandantwaehrungzubelegwaehrung(new Double(getLocaleFac().getWechselkurs2(
					getMandantFac().mandantFindByPrimaryKey(theClientDto.getMandant(), theClientDto).getWaehrungCNr(),
					anfrageDto.getWaehrungCNr(), theClientDto).doubleValue()));

			anfrageDto.setTBelegdatum(Helper.cutTimestamp(new java.sql.Timestamp(System.currentTimeMillis())));
			...
			anfrageDto.setPersonalIIdAnfrager(theClientDto.getIDPersonal());
			...
			AnfragepositionDto[] aBSpositionBasis = getAnfragepositionFac()
					.anfragepositionFindByAnfrage(anfrageIIdVorhanden, theClientDto);

			for (int i = 0; i < aBSpositionBasis.length; i++) {
				AnfragepositionDto afpositionDto = (AnfragepositionDto) aBSpositionBasis[i].clone();

				// PJ19954 ausser vorhandene Ersatzartikel
				if (afpositionDto.getAnfragepositionIdZugehoerig() == null) {
					afpositionDto.setBelegIId(iIdAnfrageKopie);

					Integer anfragepostitionIId = getAnfragepositionFac().createAnfrageposition(afpositionDto, true,
							theClientDto);

					getBelegartmediaFac().kopiereBelegartmedia(QueryParameters.UC_ID_ANFRAGEPOSITION,
							aBSpositionBasis[i].getIId(), QueryParameters.UC_ID_ANFRAGEPOSITION, anfragepostitionIId,
							theClientDto);
	...
	public ArrayList<Integer> erzeugeAnfragenAusLiefergruppenanfrage(Integer iIdAnfrageI,
			ArrayList<Integer> bereitserzeugteAnfragen, TheClientDto theClientDto) {
		checkAnfrageIId(iIdAnfrageI);
		...
		AnfrageDto liefergruppenanfrageDto = anfrageFindByPrimaryKey(iIdAnfrageI, theClientDto);
		...
		try {
			...
			ParametermandantDto param = getParameterFac().getMandantparameter(theClientDto.getMandant(),
					ParameterFac.KATEGORIE_ANFRAGE, ParameterFac.PARAMETER_ANFRAGE_ANSP_VORBESETZEN);
			bAnsprechpartnerVorbesetzen = (Boolean) param.getCWertAsObject();

			aAnfragepositionDto = getAnfragepositionFac().anfragepositionFindByAnfrage(iIdAnfrageI, theClientDto);

			aLieferantDto = getLieferantFac()
					.lieferantFindByLiefergruppeIId(liefergruppenanfrageDto.getLiefergruppeIId(), theClientDto);
		...
		if (aLieferantDto != null && aLieferantDto.length > 0) {
			...
			for (int i = 0; i < aLieferantDto.length; i++) {
				AnfrageDto anfrageDto = (AnfrageDto) liefergruppenanfrageDto.clone();

				// PJ 16726 Wenn der Lieferant einen andere Waehrung hat,
				// dann diese verwenden
				if (!anfrageDto.getWaehrungCNr().equals(aLieferantDto[i].getWaehrungCNr())) {
					anfrageDto.setWaehrungCNr(aLieferantDto[i].getWaehrungCNr());
				...
				try {
					...
					if (bAnsprechpartnerVorbesetzen == true) {
						AnsprechpartnerDto anspDto = getAnsprechpartnerFac()
								.ansprechpartnerFindErstenEinesPartnersOhneExc(aLieferantDto[i].getPartnerIId(),
										theClientDto);
						if (anspDto != null) {
							anfrageDto.setAnsprechpartnerIIdLieferant(anspDto.getIId());
						}
					}
				...
				anfrageDto.setLieferantIIdAnfrageadresse(aLieferantDto[i].getIId());

				anfrageDto.setLiefergruppeIId(null);
				anfrageDto.setAnfrageIIdLiefergruppenanfrage(liefergruppenanfrageDto.getIId());

				// SP9607
				anfrageDto.setTAnliefertermin(liefergruppenanfrageDto.getTAnliefertermin());
				anfrageDto.setTAbgabetermin(liefergruppenanfrageDto.getTAbgabetermin());

				anfrageDto.setPersonalIIdAnfrager(liefergruppenanfrageDto.getPersonalIIdAnfrager());

				if (aLieferantDto[i].getLieferartIId() != null) {
					anfrageDto.setLieferartIId(aLieferantDto[i].getLieferartIId());
				}
				if (aLieferantDto[i].getZahlungszielIId() != null) {
					anfrageDto.setZahlungszielIId(aLieferantDto[i].getZahlungszielIId());
				}
				if (aLieferantDto[i].getIdSpediteur() != null) {
					anfrageDto.setSpediteurIId(aLieferantDto[i].getIdSpediteur());
				}

				iIdLetzteErzeugteAnfrage = getAnfrageFac().createAnfrage(anfrageDto, theClientDto);

				bereitserzeugteAnfragen.add(iIdLetzteErzeugteAnfrage);

				for (int j = 0; j < aAnfragepositionDto.length; j++) {
					AnfragepositionDto anfragepositionDto = (AnfragepositionDto) aAnfragepositionDto[j].clone();
					anfragepositionDto.setBelegIId(iIdLetzteErzeugteAnfrage);

					if (anfragepositionDto.getNRichtpreis() != null
							&& !liefergruppenanfrageDto.getWaehrungCNr().equals(aLieferantDto[i].getWaehrungCNr())) {
						anfragepositionDto.setNRichtpreis(anfragepositionDto.getNRichtpreis()
								.multiply(new BigDecimal(anfrageDto.getFWechselkursmandantwaehrungzubelegwaehrung())));
					}

					// PJ20771
					if (anfragepositionDto.getArtikelIId() != null) {
						ArtikellieferantDto alDto = getArtikelFac().getArtikelEinkaufspreis(
								anfragepositionDto.getArtikelIId(), aLieferantDto[i].getIId(), BigDecimal.ONE,
								anfrageDto.getWaehrungCNr(), new java.sql.Date(anfrageDto.getTBelegdatum().getTime()),
								theClientDto);

						if (alDto != null && Helper.short2boolean(alDto.getBNichtLieferbar())) {
							continue;
						}
					}
			...
			liefergruppenanfrageDto.setStatusCNr(AnfrageServiceFac.ANFRAGESTATUS_ERLEDIGT);
			getAnfrageFac().setzeAnfragestatus(AnfrageServiceFac.ANFRAGESTATUS_ERLEDIGT,
	...
	public ArrayList<Integer> getAngelegteAnfragenNachUmwandlungDerLiefergruppenanfragen(Integer liefergruppeIId,
		...
		try {
			...
			c.add(Restrictions.eq(AnfrageFac.FLR_ANFRAGE_ANFRAGEART_C_NR, AnfrageServiceFac.ANFRAGEART_LIEFERGRUPPE));
			...
			c.add(Restrictions.or(
					...
					Restrictions.eq(AnfrageFac.FLR_ANFRAGE_ANFRAGESTATUS_C_NR, AnfrageServiceFac.ANFRAGESTATUS_OFFEN)));
		...
		try {
			...
			// Filter nach Status: nur angelegte
			c.add(Restrictions.eq(AnfrageFac.FLR_ANFRAGE_ANFRAGESTATUS_C_NR, AnfrageServiceFac.ANFRAGESTATUS_ANGELEGT));
			// nach RE-Nummer sortieren
			c.addOrder(Order.asc("c_nr"));
			// Query ausfuehren
			List<?> list = c.list();
	...
	public void erzeugeAnfragenAusBestellvorschlag(
			...
			TheClientDto theClientDto, Integer standortIId, ArrayList<Integer> alMarkierte) throws EJBExceptionLP {

		if (bestellvorschlagUeberleitungKriterienDtoI == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_PARAMETER_IS_NULL,
					new Exception("bestellvorschlagUeberleitungKriterienDtoI == null"));
		}
		...
		try {
			...
			BestellvorschlagDto[] aBestellvorschlagDto = getListeBestellvorschlaege(
					bestellvorschlagUeberleitungKriterienDtoI, alMarkierte, theClientDto);
			...
			if (lagerminJeLager) {
				...
				while (itStandorte.hasNext()) {
					...
					if (standortIId == null || standortIId.equals(partnerIIdStandort)) {
						...
						befuelleAnfrageUndPositionen(bestellvorschlagUeberleitungKriterienDtoI, theClientDto, bbDtos,
								partnerIIdStandort);
					}
				}
			} else {
				befuelleAnfrageUndPositionen(bestellvorschlagUeberleitungKriterienDtoI, theClientDto,
						aBestellvorschlagDto, null);
			}
	...
	public void befuelleAnfrageUndPositionen(
			BestellvorschlagUeberleitungKriterienDto bestellvorschlagUeberleitungKriterienDtoI,
			TheClientDto theClientDto, BestellvorschlagDto[] aBestellvorschlagDto, Integer partnerIIdStandort)
		...
		if (aBestellvorschlagDto != null && aBestellvorschlagDto.length > 0) {
			...
			while (iIndexAbarbeitung < aBestellvorschlagDto.length) {
				lieferantDto = getLieferantFac().lieferantFindByPrimaryKey(
						aBestellvorschlagDto[iIndexAbarbeitung].getILieferantId(), theClientDto);

				tLiefertermin = aBestellvorschlagDto[iIndexAbarbeitung].getTLiefertermin();
				...
				anfrageDto.setTBelegdatum(new Timestamp(System.currentTimeMillis()));
				anfrageDto.setLieferantIIdAnfrageadresse(lieferantDto.getIId());
				// SP3131
				if (bAnsprechpartnerVorbesetzen == true) {
					AnsprechpartnerDto anspDto = getAnsprechpartnerFac()
							.ansprechpartnerFindErstenEinesPartnersOhneExc(lieferantDto.getPartnerIId(), theClientDto);
					if (anspDto != null) {
						anfrageDto.setAnsprechpartnerIIdLieferant(anspDto.getIId());
					}
				}
				...
				Integer iPartnerIIdLIeferadresse = getMandantFac()
						.mandantFindByPrimaryKey(theClientDto.getMandant(), theClientDto).getPartnerIIdLieferadresse();

				if (partnerIIdStandort != null) {
					iPartnerIIdLIeferadresse = partnerIIdStandort;
				}

				anfrageDto.setPartnerIIdLieferadresse(iPartnerIIdLIeferadresse);
				...
				anfrageDto.setFWechselkursmandantwaehrungzubelegwaehrung(
						new Double(getLocaleFac().getWechselkurs2(theClientDto.getSMandantenwaehrung(),
								lieferantDto.getWaehrungCNr(), theClientDto).doubleValue()));
				anfrageDto.setKostenstelleIId(bestellvorschlagUeberleitungKriterienDtoI.getKostenstelleIId());
				anfrageDto.setFAllgemeinerRabattsatz(
						lieferantDto.getNRabatt() == null ? new Double(0) : lieferantDto.getNRabatt());
				anfrageDto.setLieferartIId(lieferantDto.getLieferartIId());
				anfrageDto.setZahlungszielIId(lieferantDto.getZahlungszielIId());
				...
				anfrageDto.setIId(getAnfrageFac().createAnfrage(anfrageDto, theClientDto));
				...
				while (bPositionGehoertZuDieserAnfrage) {
					if (bestellvorschlagUeberleitungKriterienDtoI.getBBelegprolieferantprotermin()) {
						...
						if (iIndexAbarbeitung < aBestellvorschlagDto.length
								&& lieferantDto.getIId()
										.equals(aBestellvorschlagDto[iIndexAbarbeitung].getILieferantId())
								&& tLiefertermin.equals(aBestellvorschlagDto[iIndexAbarbeitung].getTLiefertermin())) {
						...
						if (iIndexAbarbeitung < aBestellvorschlagDto.length && lieferantDto.getIId()
								.equals(aBestellvorschlagDto[iIndexAbarbeitung].getILieferantId())) {
							bPositionGehoertZuDieserAnfrage = true;
						} else {
							bPositionGehoertZuDieserAnfrage = false;
					...
					if (bPositionGehoertZuDieserAnfrage) {
						...
						ArtikelDto artikelDto = getArtikelFac().artikelFindByPrimaryKey(
								aBestellvorschlagDto[iIndexAbarbeitung].getIArtikelId(), theClientDto);

						AnfragepositionDto anfragepositionDto = new AnfragepositionDto();
						anfragepositionDto.setBelegIId(anfrageDto.getIId());
						anfragepositionDto.setISort(new Integer(iSortPosition));
						anfragepositionDto.setPositionsartCNr(AnfrageServiceFac.ANFRAGEPOSITIONART_IDENT);
						anfragepositionDto.setArtikelIId(artikelDto.getIId());
						anfragepositionDto.setBArtikelbezeichnunguebersteuert(new Short((short) 0));
						anfragepositionDto.setNMenge(aBestellvorschlagDto[iIndexAbarbeitung].getNZubestellendeMenge());
						anfragepositionDto.setEinheitCNr(artikelDto.getEinheitCNr());
						if (bestellvorschlagUeberleitungKriterienDtoI.getBRichtpreisUebernehmen()
								&& aBestellvorschlagDto[iIndexAbarbeitung].getNNettogesamtpreis() != null) {
							anfragepositionDto
									.setNRichtpreis(aBestellvorschlagDto[iIndexAbarbeitung].getNNettogesamtpreis());
						} else {
							anfragepositionDto.setNRichtpreis(new BigDecimal(0));
						}
						getAnfragepositionFac().createAnfrageposition(anfragepositionDto,
								bestellvorschlagUeberleitungKriterienDtoI.isBErsatztypenUebernehmen(), theClientDto);
		...
		for (int i = 0; i < aBestellvorschlagDto.length; i++) {
			getBestellvorschlagFac().removeBestellvorschlag(aBestellvorschlagDto[i]);
		}
	}
	...
	private BestellvorschlagDto[] getListeBestellvorschlaege(BestellvorschlagUeberleitungKriterienDto kritDtoI,
			ArrayList<Integer> alMarkierte, TheClientDto theClientDto) throws EJBExceptionLP {
		...
		try {
			...
			if (kritDtoI.isbNurMarkierte()) {
				crit.add(Restrictions.in("i_id", alMarkierte));
			}
			...
			if (kritDtoI.getBBelegeinlieferant()) {
				...
				crit.addOrder(Property.forName("lieferant_i_id").asc()); // UW->JE
				// Konstante
				crit.addOrder(Property.forName(BestellvorschlagFac.FLR_BESTELLVORSCHLAG_T_LIEFERTERMIN).asc()); // UW->JE
																												// Konstante
				critArtikel.addOrder(Property.forName("c_nr").asc());
			}
	...
	public void setzeVersandzeitpunktAufJetzt(Integer anfrageIId, String druckart) {
		if (anfrageIId != null) {
			Anfrage anfrage = em.find(Anfrage.class, anfrageIId);
			anfrage.setTVersandzeitpunkt(new Timestamp(System.currentTimeMillis()));
	...
	public Integer getKundeIdDesBelegs(BelegDto belegDto, TheClientDto theClientDto)
			throws EJBExceptionLP, RemoteException {
		return null; // Eine Anfrage hat keinen Kunden weil ich ja beim
						// Lieferanten anfrage
	}
	...
	public void pruefeAktivierbarRecht(Integer belegId, TheClientDto theClientDto)
			throws EJBExceptionLP, RemoteException {
		boolean rechtCUD = getTheJudgeFac().hatRecht(RechteFac.RECHT_ANF_ANFRAGE_CUD, theClientDto);
		if (!(rechtCUD)) {

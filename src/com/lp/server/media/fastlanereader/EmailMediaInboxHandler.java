...
public class EmailMediaInboxHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				// flrjoin: 1
				FLRMediaInbox inbox = (FLRMediaInbox) resultListIterator.next();

				rows[row][col++] = inbox.getI_id();
				rows[row][col++] = inbox.getFlrmedia().getT_emaildate();
				rows[row][col++] = inbox.getFlrmedia().getC_from();
				rows[row][col++] = inbox.getFlrmedia().getC_subject() ;
				rows[row][col++] = inbox.getB_versteckt() > 0 ? LocaleFac.STATUS_GESPERRT : null ;
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			session = factory.openSession();
			String queryString = "SELECT count(*) FROM FLRMediaInbox AS flrmediainbox "
					+ " LEFT JOIN flrmediainbox.flrmedia AS flremailmeta "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
...
//					}
	...
	private String buildOrderByClause() {
		...
		if (getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append(EmailMediaFac.FLR_MEDIAINBOX_EMAILMETA_DATEFROM 
						+ " DESC ");
				sortAdded = true;
			}
			if (orderBy.indexOf(EmailMediaFac.FLR_MEDIAINBOX_MEDIA_ID) < 0) {
				...
				orderBy.append(" " + EmailMediaFac.FLR_MEDIAINBOX_MEDIA_ID + " ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return "SELECT flrmediainbox from FLRMediaInbox AS flrmediainbox "
				+ " LEFT JOIN flrmediainbox.flrmedia AS flremailmeta " ;
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRMediaInbox flrmediainbox = (FLRMediaInbox) scrollableResult.get(0) ;
//						Integer id = (Integer) scrollableResult.getInteger(0);
						Integer id = flrmediainbox.getI_id() ;
	...
	public TableInfo getTableInfo() {
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					new Class[] { 
							Integer.class, Timestamp.class, String.class, String.class, Icon.class},
					new String[] {
							"i_id",
							getTextRespectUISpr("media.date.from", mandantCNr, locUI),
							getTextRespectUISpr("media.from", mandantCNr, locUI),
							getTextRespectUISpr("media.subject", mandantCNr, locUI),
							getTextRespectUISpr("media.storno", mandantCNr, locUI)},
					new int[] {
					...
					new String[] {
							"i_id",
							EmailMediaFac.FLR_MEDIAINBOX_EMAILMETA_DATEFROM,
							EmailMediaFac.FLR_MEDIAINBOX_EMAILMETA_FROM,
							EmailMediaFac.FLR_MEDIAINBOX_EMAILMETA_SUBJECT,
							EmailMediaFac.FLR_MEDIAINBOX_VERSTECKT}));
		}

...
import com.lp.server.auftrag.fastlanereader.generated.FLRAuftragbegruendung;
...
public class AuftragbegruendungHandler extends UseCaseHandler {
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRAuftragbegruendung AS begruendung "
					+ buildWhereClause();
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("begruendung.c_nr ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("begruendung.c_nr") < 0) {
				...
				orderBy.append(" begruendung.c_nr ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRAuftragbegruendung AS begruendung ";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRAuftragbegruendung lieferscheinart = (FLRAuftragbegruendung) scrollableResult
								.get(0);
						Integer c_nr = lieferscheinart.getI_id();

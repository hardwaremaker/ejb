...
import com.lp.server.auftrag.fastlanereader.generated.FLRAuftragdokument;
 ...
 * @version 1.0
 */
public class AuftragdokumentHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				FLRAuftragdokument nachrichtenart = (FLRAuftragdokument) resultListIterator
						.next();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(buildWhereClausePart("auftragdokument.",
							filterKriterien[i]));
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append("auftragdokument." + kriterien[i].kritName);
						orderBy.append(" ");
				...
				orderBy.append("auftragdokument.c_bez"
						+ " ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("auftragdokument.c_bez ") < 0) {
	...
	private String getFromClause() {
		return "from FLRAuftragdokument auftragdokument ";
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select auftragdokument.i_id from FLRAuftragdokument auftragdokument "

 ...
 */
public class ZeitplanHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final String FLR_ZEITPLAN = "flrzeitplan.";
	public static final String FLR_ZEITPLAN_FROM_CLAUSE = " from FLRZeitplan flrzeitplan ";
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (kriterien[i].isKrit) {
						...
						sortAdded = true;
						orderBy.append(FLR_ZEITPLAN + kriterien[i].kritName);
						orderBy.append(" ");
			...
			if (orderBy.indexOf(FLR_ZEITPLAN
	...
	private String getFromClause() {
		return FLR_ZEITPLAN_FROM_CLAUSE;
	}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select " + FLR_ZEITPLAN
						+ AuftragteilnehmerFac.FLR_AUFTRAGTEILNEHMER_I_ID
						+ FLR_ZEITPLAN_FROM_CLAUSE + this.buildWhereClause()
						+ this.buildOrderByClause();
				Query query = session.createQuery(queryString);

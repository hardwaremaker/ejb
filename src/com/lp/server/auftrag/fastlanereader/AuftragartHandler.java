 ...
 * @version 1.0
 */
public class AuftragartHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final String FLR_AUFTRAGART = "flrauftragart.";
	public static final String FLR_AUFTRAGART_FROM_CLAUSE = " from FLRAuftragart flrauftragart ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				Iterator<?> sprsetIterator = auftragart
						.getAuftragart_auftragart_set().iterator();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRAuftragartspr auftragartspr = (FLRAuftragartspr) iterUebersetzungenI
					.next();
			if (auftragartspr.getLocale().getC_nr().compareTo(sLocaleI) == 0) {
				sUebersetzung = auftragartspr.getC_bez();
				break;
			}
		}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRAuftragart AS auftragart"
					+ " LEFT JOIN auftragart.auftragart_auftragart_set AS auftragart_auftragart_set "
					+ buildWhereClause();
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" auftragart." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("auftragart.i_sort ASC ");

				sortAdded = true;
			}
			if (orderBy.indexOf("auftragart.i_sort") < 0) {
				...
				orderBy.append(" auftragart.i_sort ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRAuftragart AS auftragart"
				+ " LEFT JOIN auftragart.auftragart_auftragart_set AS auftragart_auftragart_set";
	}
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { String.class,
					...
					"auftragart.c_nr", "auftragart.c_nr", "auftragart.i_sort",
					"auftragart_auftragart_set.c_bez" }));
		}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRAuftragart auftragart = (FLRAuftragart) scrollableResult
								.get(0);

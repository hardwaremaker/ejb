...
import com.lp.server.auftrag.fastlanereader.generated.FLRMeilenstein;
...
public class MeilensteinHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final String FLR_MEILENSTEIN = "flrmeilenstein.";
	public static final String FLR_MEILENSTEIN_FROM_CLAUSE = " from FLRMeilenstein flrmeilenstein ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				FLRMeilenstein meilenstein = (FLRMeilenstein) o[0];

				Iterator<?> sprsetIterator = meilenstein
						.getMeilensteinspr_set().iterator();

				rows[row][col++] = meilenstein.getI_id();
				rows[row][col++] = meilenstein.getC_nr();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRMeilensteinspr meilensteinspr = (FLRMeilensteinspr) iterUebersetzungenI
					.next();
			if (meilensteinspr.getLocale().getC_nr().compareTo(sLocaleI) == 0) {
				sUebersetzung = meilensteinspr.getC_bez();
				break;
			}
		}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRMeilenstein AS flrmeilenstein"
					+ " LEFT JOIN flrmeilenstein.meilensteinspr_set AS meilensteinspr_set "
					+ buildWhereClause();
	...
	private String getFromClause() {
		return " FROM FLRMeilenstein AS flrmeilenstein"
				+ " LEFT JOIN flrmeilenstein.meilensteinspr_set AS meilensteinspr_set";
	}
		...
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { Integer.class,
					...
					new String[] {
							...
							QueryParameters.FLR_BREITE_SHARE_WITH_REST,
							QueryParameters.FLR_BREITE_SHARE_WITH_REST },
					new String[] { "flrmeilenstein.i_id",
							"flrmeilenstein.c_nr", "meilensteinspr_set.c_bez" }));
		}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRMeilenstein meilenstein = (FLRMeilenstein) scrollableResult
								.get(0);
						Integer i_id = meilenstein.getI_id();

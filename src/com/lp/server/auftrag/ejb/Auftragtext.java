...
@NamedQueries( { @NamedQuery(name = "AuftragtextfindByMandantLocaleMediaartCNr", query = "SELECT OBJECT (o) FROM Auftragtext o WHERE o.mandantCNr=?1 AND o.localeCNr=?2 AND o.mediaartCNr=?3") })
@Entity
@Table(name = "AUFT_AUFTRAGTEXT")
public class Auftragtext implements Serializable {
	@Id
	@Column(name = "I_ID")
	private Integer iId;
	...
	public Auftragtext(Integer iId, String mandantCNr, String localeCNr,
			String mediaartCNr, String xTextinhalt) {
		setIId(iId);
		setMandantCNr(mandantCNr);
		setLocaleCNr(localeCNr);
		setMediaartCNr(mediaartCNr);
		setXTextinhalt(xTextinhalt);
	}

...
@Stateless
public class RechnungServiceFacBean extends Facade implements
		RechnungServiceFac {
	@PersistenceContext
	...
	public Integer createRechnungtext(RechnungtextDto rechnungtextDto,
		...
		rechnungtextDto.setIId(getPKGeneratorObj().getNextPrimaryKey(
				PKConst.PK_RECHNUNGTEXT));
		try {
			Rechnungtext rechnungtext = new Rechnungtext(
					rechnungtextDto.getIId(), rechnungtextDto.getMandantCNr(),
					rechnungtextDto.getLocaleCNr(), rechnungtextDto.getCNr(),
					rechnungtextDto.getCTextinhalt());
	...
	public Integer createGutschrifttext(GutschrifttextDto gutschrifttextDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		...
		gutschrifttextDto.setIId(getPKGeneratorObj().getNextPrimaryKey(
				PKConst.PK_GUTSCHRIFTTEXT));
		try {
			Gutschrifttext text = new Gutschrifttext(
					gutschrifttextDto.getIId(),
					gutschrifttextDto.getMandantCNr(),
					gutschrifttextDto.getLocaleCNr(),
					gutschrifttextDto.getCNr(),
					gutschrifttextDto.getCTextinhalt());
			em.persist(text);
			em.flush();
	...
	public Integer createGutschriftsgrund(
			GutschriftsgrundDto gutschriftsgrundDto, TheClientDto theClientDto)
			throws EJBExceptionLP {
		myLogger.logData(gutschriftsgrundDto);
		if (gutschriftsgrundDto == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_DTO_IS_NULL,
					new Exception("gutschriftsgrundDto == null"));
		}
		gutschriftsgrundDto.setIId(getPKGeneratorObj().getNextPrimaryKey(
				PKConst.PK_GUTSCHRIFTSGRUND));
		try {
			Gutschriftsgrund grund = new Gutschriftsgrund(
					gutschriftsgrundDto.getCNr(), gutschriftsgrundDto.getIId());
			em.persist(grund);
			em.flush();
			setGutschriftsgrundFromGutschriftsgrundDto(grund,
					gutschriftsgrundDto);
	...
	public void removeGutschrifttext(GutschrifttextDto gutschrifttextDto,
		...
		if (gutschrifttextDto == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_DTO_IS_NULL,
					new Exception("gutschrifttextDto == null"));
		}
		try {
			if (gutschrifttextDto != null) {
	...
	public void updateRechnungtext(RechnungtextDto rechnungtextDto,
		...
		if (rechnungtextDto != null) {
			...
			try {
				...
				setRechnungtextFromRechnungtextDto(rechnungtext,
						rechnungtextDto);
			} catch (Exception e) {
	...
	public void updateGutschrifttext(GutschrifttextDto gutschrifttextDto,
		...
		if (gutschrifttextDto != null) {
			Integer iId = gutschrifttextDto.getIId();
			try {
				Gutschrifttext rechnungtext = em
						.find(Gutschrifttext.class, iId);
				setGutschrifttextFromGutschrifttextDto(rechnungtext,
						gutschrifttextDto);
			} catch (Exception e) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_UPDATE, e);
			}
	...
	public GutschriftsgrundDto gutschriftsgrundFindByPrimaryKey(Integer iId)
		...
		try {
			Gutschriftsgrund gutschriftsgrund = em.find(Gutschriftsgrund.class,
					iId);
	...
	public RechnungtextDto rechnungtextFindByMandantLocaleCNr(String pMandant,
		...
		try {
			Query query = em
					.createNamedQuery("RechnungtextfindByMandantLocaleCNr");
	...
	public GutschrifttextDto gutschrifttextFindByMandantLocaleCNr(
		...
		try {
			Query query = em
					.createNamedQuery("GutschrifttextfindByMandantLocaleCNr");
			query.setParameter(1, pMandant);
			query.setParameter(2, pSprache);
			query.setParameter(3, pText);
	...
	private void setRechnungtextFromRechnungtextDto(Rechnungtext rechnungtext,
			RechnungtextDto rechnungtextDto) {
	...
	private void setGutschrifttextFromGutschrifttextDto(
			Gutschrifttext gutschrifttext, GutschrifttextDto gutschrifttextDto) {
	...
	private void setGutschriftsgrundFromGutschriftsgrundDto(
			Gutschriftsgrund grund, GutschriftsgrundDto grundDto) {
	...
	private GutschrifttextDto assembleGutschrifttextDto(
			Gutschrifttext gutschrifttext) {
		return GutschrifttextDtoAssembler.createDto(gutschrifttext);
	}

	private GutschriftsgrundDto assembleGutschriftsgrundDto(
			Gutschriftsgrund gutschriftsgrund) {
		return GutschriftsgrundDtoAssembler.createDto(gutschriftsgrund);
	}

	private RechnungtextDto[] assembleRechnungtextDtos(
			Collection<?> rechnungtexts) {
	...
	public RechnungtextDto createDefaultRechnungtext(String sMediaartI,
		...
		oRechnungtextDto.setIId(createRechnungtext(oRechnungtextDto,
				theClientDto));
	...
	public GutschrifttextDto createDefaultGutschrifttext(String sMediaartI,
		...
		gutschrifttextDto.setIId(createGutschrifttext(gutschrifttextDto,
				theClientDto));
	...
	/**
	 * Lesen aller in der DB vorhandenen RechnungsPositionsarten inkl.
	 * uebersetzung
	 * 
	 * @param locale1
	 *            Locale
			...
			for (int i = 0; i < allArten.length; i++) {
				String sUebersetzung = getSystemMultilanguageFac()
						.uebersetzePositionsartOptimal(
								allArten[i].getPositionsartCNr(), locale1,
								locale2);
	...
	/**
	 * Lesen aller in der DB vorhandenen Gutschriftositionsarten inkl.
	 * uebersetzung
	 * 
	 * @param locale1
	 *            Locale
	...
	public Map<String, String> getAllGutschriftpositionsart(Locale locale1,
		...
		LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
		Query query = em
				.createNamedQuery("GutschriftpositionsartfindAllEnable");
		Collection<?> c = query.getResultList();
	...
	public Map<String, String> getAllProformarechnungpositionsart(
		...
		LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
		Query query = em
				.createNamedQuery("ProformarechnungpositionsartfindAll");
		Collection<?> c = query.getResultList();
	...
	private void setGutschriftsgrundsprFromGutschriftsgrundsprDto(
			Gutschriftsgrundspr gutschriftsgrundspr,
	...
	private void setZahlungsartsprFromZahlungsartsprDto(
			Zahlungsartspr zahlungsartspr, ZahlungsartsprDto zahlungsartsprDto) {
		zahlungsartspr.setCBez(zahlungsartsprDto.getCBez());
		em.merge(zahlungsartspr);
		em.flush();
	}

	private RechnungartsprDto assembleRechnungartsprDto(
			Rechnungartspr rechnungartspr) {
		return RechnungartsprDtoAssembler.createDto(rechnungartspr);
	}

	private RechnungartsprDto[] assembleRechnungartsprDtos(
			Collection<?> rechnungartsprs) {
		...
		if (rechnungartsprs != null) {
			...
			while (iterator.hasNext()) {
				Rechnungartspr rechnungartspr = (Rechnungartspr) iterator
						.next();
	...
	public Map<String, String> uebersetzeRechnungartOptimal(
		...
		for (int i = 0; i < pArray.length; i++) {
			...
			String value = uebersetzeRechnungartOptimal(pArray[i].getCNr(),
					locale1, locale2);
	...
	/**
	 * Uebersetzt eine Rechnungart optimal. 1.Versuch: mit locale1 2.Versuch:
	...
	public String uebersetzeRechnungartOptimal(String cNr, Locale locale1,
		...
		if (cNr == null || locale1 == null || locale2 == null) {
			throw new EJBExceptionLP(
					EJBExceptionLP.FEHLER_PARAMETER_IS_NULL,
					new Exception(
							"cNr == null || locale1 == null || locale2 == null"));
		}
	...
	private String uebersetzeRechnungart(String cNr, Locale locale) {
		...
		Rechnungartspr rechnungartspr = em.find(Rechnungartspr.class,
				new RechnungartsprPK(cNr, cLocale));
		if (rechnungartspr == null) {
			return null;
		}
	...
	/**
	 * Uebersetzt eine Zahlungsart optimal. 1.Versuch: mit locale1 2.Versuch:
	 * mit locale2 3.Versuch: cNr
	 * 
	 * @param cNr
	 *            String
	...
	private String uebersetzeZahlungsart(String cNr, Locale locale) {
		...
		Zahlungsartspr zahlungsartspr = em.find(Zahlungsartspr.class,
				new ZahlungsartsprPK(cNr, cLocale));
	...
	private ZahlungsartsprDto assembleZahlungsartsprDto(
			Zahlungsartspr zahlungsartspr) {
		return ZahlungsartsprDtoAssembler.createDto(zahlungsartspr);
	}

	private ZahlungsartsprDto[] assembleZahlungsartsprDtos(
			Collection<?> zahlungsartsprs) {
		...
		if (zahlungsartsprs != null) {
			...
			while (iterator.hasNext()) {
				Zahlungsartspr zahlungsartspr = (Zahlungsartspr) iterator
						.next();
	 ...
	 * 
	 * @param gutschriftpositionsartDto
	...
	public void createGutschriftpositionsart(
		...
		try {
			Gutschriftpositionsart gutschriftpositionsart = new Gutschriftpositionsart(
					gutschriftpositionsartDto.getPositionsartCNr(),
					gutschriftpositionsartDto.getISort(),
					gutschriftpositionsartDto.getBVersteckt());
			em.persist(gutschriftpositionsart);
			em.flush();
			setGutschriftpositionsartFromGutschriftpositionsartDto(
					gutschriftpositionsart, gutschriftpositionsartDto);
		} catch (Exception e) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_ANLEGEN, e);
		}
	}
	...
	public void updateGutschriftpositionsart(
		...
		if (gutschriftpositionsartDto != null) {
			String positionsartCNr = gutschriftpositionsartDto
					.getPositionsartCNr();
			try {
				Gutschriftpositionsart gutschriftpositionsart = em.find(
						Gutschriftpositionsart.class, positionsartCNr);
				setGutschriftpositionsartFromGutschriftpositionsartDto(
						gutschriftpositionsart, gutschriftpositionsartDto);
			} catch (Exception e) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_UPDATE, e);
	...
	public GutschriftpositionsartDto gutschriftpositionsartFindByPrimaryKey(
		...
		if (positionsartCNr == null) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_PARAMETER_IS_NULL,
					new Exception("positionsartCNr == null"));
		}
		try {
			Gutschriftpositionsart gutschriftpositionsart = em.find(
					Gutschriftpositionsart.class, positionsartCNr);
	...
	private void setGutschriftpositionsartFromGutschriftpositionsartDto(
			Gutschriftpositionsart gutschriftpositionsart,
			GutschriftpositionsartDto gutschriftpositionsartDto) {
		gutschriftpositionsart.setISort(gutschriftpositionsartDto.getISort());
		gutschriftpositionsart.setBVersteckt(gutschriftpositionsartDto
				.getBVersteckt());
		em.merge(gutschriftpositionsart);
		em.flush();
	}

	private GutschriftpositionsartDto assembleGutschriftpositionsartDto(
			Gutschriftpositionsart gutschriftpositionsart) {
		return GutschriftpositionsartDtoAssembler
				.createDto(gutschriftpositionsart);
	}

	private GutschriftpositionsartDto[] assembleGutschriftpositionsartDtos(
			Collection<?> gutschriftpositionsarts) {
		...
		if (gutschriftpositionsarts != null) {
			...
			while (iterator.hasNext()) {
				Gutschriftpositionsart gutschriftpositionsart = (Gutschriftpositionsart) iterator
						.next();
				list.add(assembleGutschriftpositionsartDto(gutschriftpositionsart));
			}
		}
		GutschriftpositionsartDto[] returnArray = new GutschriftpositionsartDto[list
				.size()];
	 ...
	 * 
	 * @param rechnungpositionsartDto
	 *            RechnungpositionsartDto
	 * @param theClientDto
	 *            String
	 * @throws EJBExceptionLP
	 */
	public void createRechnungpositionsart(
		...
		try {
			Rechnungpositionsart rechnungpositionsart = new Rechnungpositionsart(
					rechnungpositionsartDto.getPositionsartCNr(),
					rechnungpositionsartDto.getISort(),
					rechnungpositionsartDto.getBVersteckt());
			em.persist(rechnungpositionsart);
			em.flush();
			setRechnungpositionsartFromRechnungpositionsartDto(
					rechnungpositionsart, rechnungpositionsartDto);
		} catch (Exception e) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_ANLEGEN, e);
		}
	}
	...
	public void updateRechnungpositionsart(
		...
		try {
			Rechnungpositionsart rechnungpositionsart = em.find(
					Rechnungpositionsart.class, positionsartCNr);
			setRechnungpositionsartFromRechnungpositionsartDto(
					rechnungpositionsart, rechnungpositionsartDto);
		} catch (Exception e) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_UPDATE, e);
	...
	private void setRechnungpositionsartFromRechnungpositionsartDto(
			Rechnungpositionsart rechnungpositionsart,
			RechnungpositionsartDto rechnungpositionsartDto) {
		rechnungpositionsart.setISort(rechnungpositionsartDto.getISort());
		rechnungpositionsart.setBVersteckt(rechnungpositionsartDto
				.getBVersteckt());
		em.merge(rechnungpositionsart);
		em.flush();
	}

	private RechnungpositionsartDto assembleRechnungpositionsartDto(
			Rechnungpositionsart rechnungpositionsart) {
		return RechnungpositionsartDtoAssembler.createDto(rechnungpositionsart);
	}

	private RechnungpositionsartDto[] assembleRechnungpositionsartDtos(
			Collection<?> rechnungpositionsarts) {
		...
		if (rechnungpositionsarts != null) {
			...
			while (iterator.hasNext()) {
				Rechnungpositionsart rechnungpositionsart = (Rechnungpositionsart) iterator
						.next();
				list.add(assembleRechnungpositionsartDto(rechnungpositionsart));
			}
		}
		RechnungpositionsartDto[] returnArray = new RechnungpositionsartDto[list
				.size()];
	 ...
	 * 
	 * @param proformarechnungpositionsartDto
	...
	public void createProformarechnungpositionsart(
		...
		try {
			Proformarechnungpositionsart proformarechnungpositionsart = new Proformarechnungpositionsart(
					proformarechnungpositionsartDto.getPositionsartCNr(),
					proformarechnungpositionsartDto.getISort());
			em.persist(proformarechnungpositionsart);
			em.flush();
			setProformarechnungpositionsartFromProformarechnungpositionsartDto(
					proformarechnungpositionsart,
					proformarechnungpositionsartDto);
		} catch (Exception e) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_ANLEGEN, e);
		}
	...
	public void updateProformarechnungpositionsart(
		...
		if (proformarechnungpositionsartDto != null) {
			String positionsartCNr = proformarechnungpositionsartDto
					.getPositionsartCNr();
			try {
				Proformarechnungpositionsart proformarechnungpositionsart = em
						.find(Proformarechnungpositionsart.class,
								positionsartCNr);
				setProformarechnungpositionsartFromProformarechnungpositionsartDto(
						proformarechnungpositionsart,
						proformarechnungpositionsartDto);
	...
	public ProformarechnungpositionsartDto proformarechnungpositionsartFindByPrimaryKey(
		...
		try {
			Proformarechnungpositionsart proformarechnungpositionsart = em
					.find(Proformarechnungpositionsart.class, positionsartCNr);
	...
	private void setProformarechnungpositionsartFromProformarechnungpositionsartDto(
			...
			ProformarechnungpositionsartDto proformarechnungpositionsartDto) {
		proformarechnungpositionsart.setISort(proformarechnungpositionsartDto
				.getISort());
	...
	private ProformarechnungpositionsartDto assembleProformarechnungpositionsartDto(
			Proformarechnungpositionsart proformarechnungpositionsart) {
		return ProformarechnungpositionsartDtoAssembler
				.createDto(proformarechnungpositionsart);
	}
	...
	private ProformarechnungpositionsartDto[] assembleProformarechnungpositionsartDtos(
		...
		ProformarechnungpositionsartDto[] returnArray = new ProformarechnungpositionsartDto[list
				.size()];
		...
		if (rechnungartDto != null) {
			...
			try {
				...
				if (rechnungartDto.getRechnungartsprDto() != null) {
					rechnungartDto.getRechnungartsprDto().setRechnungartCNr(
							rechnungartDto.getCNr());
					rechnungartDto.getRechnungartsprDto().setSpracheCNr(
							theClientDto.getLocUiAsString());
					updateRechnungartspr(rechnungartDto.getRechnungartsprDto(),
							theClientDto);
				}
			} catch (Exception e) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_UPDATE, e);
	...
	public void updateGutschriftsgrund(GutschriftsgrundDto gutschriftsgrundDto,
		...
		if (gutschriftsgrundDto != null) {
			...
			try {
				Gutschriftsgrund gutschriftsgrund = em.find(
						Gutschriftsgrund.class, iId);
				setGutschriftsgrundFromGutschriftsgrundDto(gutschriftsgrund,
						gutschriftsgrundDto);
				// sprache
				if (gutschriftsgrundDto.getGutschriftsgrundsprDto() != null) {
					gutschriftsgrundDto.getGutschriftsgrundsprDto()
							.setGutschriftsgrundIId(
									gutschriftsgrundDto.getIId());
					gutschriftsgrundDto.getGutschriftsgrundsprDto()
							.setLocaleCNr(theClientDto.getLocUiAsString());
					updateGutschriftsgrundspr(
							gutschriftsgrundDto.getGutschriftsgrundsprDto(),
							theClientDto);
				}
			} catch (Exception e) {
		...
		if (zahlungsartDto != null) {
			...
			try {
				...
				if (zahlungsartDto.getZahlungsartsprDto() != null) {
					zahlungsartDto.getZahlungsartsprDto().setZahlungsartCNr(
							zahlungsartDto.getCNr());
					zahlungsartDto.getZahlungsartsprDto().setSpracheCNr(
							theClientDto.getLocUiAsString());
					updateZahlungsartspr(zahlungsartDto.getZahlungsartsprDto(),
							theClientDto);
				}
			} catch (Exception e) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_UPDATE, e);
	...
	public void createRechnungtyp(RechnungtypDto rechnungtypDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		try {
			Rechnungtyp rechnungtyp = new Rechnungtyp(rechnungtypDto.getCNr(),
					rechnungtypDto.getISort());
	...
	private void setRechnungtypFromRechnungtypDto(Rechnungtyp rechnungtyp,
			RechnungtypDto rechnungtypDto) {
	 ...
	 * @throws EJBExceptionLP
	 */
	private Map<String, String> getAllRechnungart(String rechnungstypCNr,
			Locale locale1, Locale locale2) throws EJBExceptionLP {
		RechnungtypDto typDto = rechnungtypFindByPrimaryKey(rechnungstypCNr);
		RechnungartDto[] allArten = rechnungartFindByRechnungtyp(typDto
				.getCNr());
	...
	public Map getAllZahlungsarten(Locale locale1, Locale locale2)
		...
		try {
			return getRechnungServiceFac().uebersetzeZahlungsartOptimal(arten,
					locale1, locale2);
		} catch (RemoteException ex) {
	...
	public Map<String, String> getAllRechnungartRechnung(Locale locale1,
			Locale locale2) throws EJBExceptionLP {
		return getAllRechnungart(RechnungFac.RECHNUNGTYP_RECHNUNG, locale1,
				locale2);
	}
	...
	public Map<String, String> getAllRechnungartGutschrift(Locale locale1,
			Locale locale2) throws EJBExceptionLP {
		return getAllRechnungart(RechnungFac.RECHNUNGTYP_GUTSCHRIFT, locale1,
				locale2);
	}
	...
	public Map<String, String> getAllRechnungartProformarechnung(
			Locale locale1, Locale locale2) throws EJBExceptionLP {
		return getAllRechnungart(RechnungFac.RECHNUNGTYP_PROFORMARECHNUNG,
				locale1, locale2);
	}

	public void createRechnungart(RechnungartDto rechnungartDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		try {
			Rechnungart rechnungart = new Rechnungart(rechnungartDto.getCNr(),
					rechnungartDto.getRechnungtypCNr(),
	...
	public RechnungartDto rechnungartFindByPrimaryKey(String cNr,
		...
		try {
			...
			rechnungartDto.getRechnungartsprDto().setSpracheCNr(
					theClientDto.getLocUiAsString());
			rechnungartDto.getRechnungartsprDto().setCBez(
					uebersetzeRechnungart(cNr, theClientDto.getLocUi()));
	...
	private void setRechnungartFromRechnungartDto(Rechnungart rechnungart,
			RechnungartDto rechnungartDto) {
	...
	private void setZahlungsartFromZahlungsartDto(Zahlungsart zahlungsart,
			ZahlungsartDto zahlungsartDto) {
	...
	public void createZahlungsart(ZahlungsartDto zahlungsartDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		try {
			Zahlungsart zahlungsart = new Zahlungsart(zahlungsartDto.getCNr(),
					zahlungsartDto.getISort());
	...
	public void createRechnungstatus(RechnungstatusDto rechnungstatusDto,
		...
		try {
			Rechnungstatus rechnungstatus = new Rechnungstatus(
					rechnungstatusDto.getStatusCNr(),
					rechnungstatusDto.getISort());
			em.persist(rechnungstatus);
			em.flush();
			setRechnungstatusFromRechnungstatusDto(rechnungstatus,
					rechnungstatusDto);
		} catch (Exception e) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_ANLEGEN, e);
		}
	...
	public void removeRechnungstatus(RechnungstatusDto rechnungstatusDto,
		...
		if (rechnungstatusDto != null) {
			...
			try {
				Rechnungstatus toRemove = em.find(Rechnungstatus.class,
						statusCNr);
				if (toRemove == null) {
					throw new EJBExceptionLP(
							EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, "");
	...
	public void removeGutschriftsgrund(GutschriftsgrundDto gutschriftsgrundDto,
		...
		if (gutschriftsgrundDto != null) {
			...
			try {
				Query query = em
						.createNamedQuery("GutschriftsgrundsprfindByGutschriftsgrundIId");
				query.setParameter(1, gutschriftsgrundDto.getIId());
				Collection<?> c = query.getResultList();
				// Erst alle SPRs dazu loeschen.
				for (Iterator<?> iter = c.iterator(); iter.hasNext();) {
					Gutschriftsgrundspr item = (Gutschriftsgrundspr) iter
							.next();
					em.remove(item);
				}

				Gutschriftsgrund toRemove = em
						.find(Gutschriftsgrund.class, iid);
				if (toRemove == null) {
					throw new EJBExceptionLP(
							EJBExceptionLP.FEHLER_BEI_FINDBYPRIMARYKEY, "");
				}
	...
	public void updateRechnungstatus(RechnungstatusDto rechnungstatusDto,
		...
		if (rechnungstatusDto != null) {
			String statusCNr = rechnungstatusDto.getStatusCNr();
			try {
				Rechnungstatus rechnungstatus = em.find(Rechnungstatus.class,
						statusCNr);
				setRechnungstatusFromRechnungstatusDto(rechnungstatus,
						rechnungstatusDto);
			} catch (Exception e) {
	...
	public RechnungstatusDto rechnungstatusFindByPrimaryKey(String statusCNr)
			throws EJBExceptionLP {
		Rechnungstatus rechnungstatus = em
				.find(Rechnungstatus.class, statusCNr);
	...
	private void setRechnungstatusFromRechnungstatusDto(
			Rechnungstatus rechnungstatus, RechnungstatusDto rechnungstatusDto) {
		rechnungstatus.setISort(rechnungstatusDto.getISort());
		em.merge(rechnungstatus);
		em.flush();
	}

	private RechnungstatusDto assembleRechnungstatusDto(
			Rechnungstatus rechnungstatus) {
		return RechnungstatusDtoAssembler.createDto(rechnungstatus);
	}

	private RechnungstatusDto[] assembleRechnungstatusDtos(
			Collection<?> rechnungstatuss) {
		...
		if (rechnungstatuss != null) {
			...
			while (iterator.hasNext()) {
				Rechnungstatus rechnungstatus = (Rechnungstatus) iterator
						.next();
	...
	public ZahlungsartDto zahlungsartFindByPrimaryKey(String cNrI,
		...
		try {
			...
			zahlungsartDto.getZahlungsartsprDto().setSpracheCNr(
					theClientDto.getLocUiAsString());
			zahlungsartDto.getZahlungsartsprDto().setCBez(
					uebersetzeZahlungsart(cNrI, theClientDto.getLocUi()));
	...
	private void updateRechnungartspr(RechnungartsprDto rechnungartsprDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		// log
		...
		if (rechnungartsprDto != null) {
			...
			try {
				...
				if (rechnungartSpr == null) {
					...
					setRechnungartsprFromRechnungartsprDto(rechnungartSpr,
							rechnungartsprDto);
				}
	...
	private void updateGutschriftsgrundspr(
			GutschriftsgrundsprDto gutschriftsgrundsprDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		// log
		...
		if (gutschriftsgrundsprDto != null) {
			...
			try {
				GutschriftsgrundsprPK pk = new GutschriftsgrundsprPK(iId,
						localeCNr);
				Gutschriftsgrundspr spr = em
						.find(Gutschriftsgrundspr.class, pk);
				if (spr == null) {
					// diese Uebersetzung gibt es nocht nicht
					createGutschriftsgrundspr(gutschriftsgrundsprDto,
							theClientDto);
				} else {
					setGutschriftsgrundsprFromGutschriftsgrundsprDto(spr,
							gutschriftsgrundsprDto);
				}
	...
	private void updateZahlungsartspr(ZahlungsartsprDto zahlungsartsprDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		// log
		...
		if (zahlungsartsprDto != null) {
			...
			try {
				...
				if (zahlungsartSpr == null) {
					...
					setZahlungsartsprFromZahlungsartsprDto(zahlungsartSpr,
							zahlungsartsprDto);
				}
			} catch (Exception e) {
				throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_UPDATE, e);
	...
	private void createRechnungartspr(RechnungartsprDto rechnungartsprDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		if (rechnungartsprDto == null) {
			return;
		}
		try {
			Rechnungartspr rechnungartspr = new Rechnungartspr(
					rechnungartsprDto.getRechnungartCNr(),
	...
	private void createGutschriftsgrundspr(
			GutschriftsgrundsprDto gutschriftsgrundsprDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		if (gutschriftsgrundsprDto == null) {
			return;
		...
		try {
			Gutschriftsgrundspr gutschriftsgrundspr = new Gutschriftsgrundspr(
					gutschriftsgrundsprDto.getGutschriftsgrundIId(),
					gutschriftsgrundsprDto.getLocaleCNr());
			em.persist(gutschriftsgrundspr);
			em.flush();
			setGutschriftsgrundsprFromGutschriftsgrundsprDto(
					gutschriftsgrundspr, gutschriftsgrundsprDto);
		} catch (Exception e) {
			throw new EJBExceptionLP(EJBExceptionLP.FEHLER_BEIM_ANLEGEN, e);
		}
	}

	private void createZahlungsartspr(ZahlungsartsprDto zahlungsartsprDto,
			TheClientDto theClientDto) throws EJBExceptionLP {
		if (zahlungsartsprDto == null) {
			return;
		}
		try {
			Zahlungsartspr zahlungsartspr = new Zahlungsartspr(
					zahlungsartsprDto.getZahlungsartCNr(),

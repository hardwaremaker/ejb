...
public class GutschriftHandler extends UseCaseHandler {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				rows[row][col++] = gutschrift.getFlrrechnungart() == null ? null
						: gutschrift.getFlrrechnungart().getC_nr().substring(0,
								1);
				rows[row][col++] = gutschrift.getC_nr();
				rows[row][col++] = gutschrift.getFlrkunde() == null ? null
						: gutschrift.getFlrkunde().getFlrpartner()
								.getC_name1nachnamefirmazeile1();
				FLRLandplzort anschrift = gutschrift.getFlrkunde()
				...
				if (iAnlegerStattVertreterAnzeigen == 1) {
					if (gutschrift.getFlrpersonalanleger() != null) {
						rows[row][col++] = gutschrift.getFlrpersonalanleger()
								.getC_kurzzeichen();
					} else {
						rows[row][col++] = null;
					}
					...
					if (gutschrift.getFlrpersonalaenderer() != null) {
						rows[row][col++] = gutschrift.getFlrpersonalaenderer()
								.getC_kurzzeichen();
					} else {
						rows[row][col++] = null;
					}
					...
					if (gutschrift.getFlrvertreter() != null) {
						rows[row][col++] = gutschrift.getFlrvertreter()
								.getC_kurzzeichen();
					} else {
						rows[row][col++] = null;
					}
				...
				rows[row][col++] = getStatusMitUebersetzung(sStatus, gutschrift
						.getT_versandzeitpunkt(), gutschrift.getC_versandtype());
				if (bDarfPreiseSehen) {
					if (bBruttoStattNetto == false) {
						rows[row][col++] = gutschrift.getN_wertfw();
					} else {

						if (gutschrift.getN_wertfw() != null
								&& gutschrift.getN_wertustfw() != null) {
							rows[row][col++] = gutschrift.getN_wertfw().add(
									gutschrift.getN_wertustfw());
						} else {
							rows[row][col++] = null;
						}
				...
				if (gutschrift.getRechnung_i_id_zurechnung() != null) {
					RechnungDto reDto = getRechnungFac()
							.rechnungFindByPrimaryKey(
									gutschrift.getRechnung_i_id_zurechnung());
					reCNr = reDto.getCNr();
				} else {
					reCNr = "";
				}
				...
				if (bHatFibu == true) {
					...
					rows[row][col++] = new Boolean(gutschrift
							.getT_fibuuebernahme() != null);
				}
				row++;
				col = 0;
			}
					...
					if (filterKriterien[i].kritName
						...
						if (bSuchenInklusiveKbez) {
							if (filterKriterien[i].isBIgnoreCase()) {
								where.append(" (upper(" + FLR_GUTSCHRIFT
										+ filterKriterien[i].kritName + ")");
								where.append(" " + filterKriterien[i].operator);
								where.append(" "
										+ filterKriterien[i].value
												.toUpperCase());
								where.append("OR upper(" + FLR_GUTSCHRIFT
								...
								where.append(" (" + FLR_GUTSCHRIFT
										+ filterKriterien[i].kritName);
								where.append(" " + filterKriterien[i].operator);
								where.append(" " + filterKriterien[i].value);
								where.append("OR " + FLR_GUTSCHRIFT
										+ "flrkunde.flrpartner.c_kbez");
								where.append(" " + filterKriterien[i].operator);
								where.append(" " + filterKriterien[i].value
										+ ") ");
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("gutschrift."
						...
						+ "gutschrift." + RechnungFac.FLR_RECHNUNG_C_NR
						+ " DESC ");
				sortAdded = true;
			}
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien,
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select gutschrift."
						+ RechnungFac.FLR_RECHNUNG_I_ID
						+ " from FLRRechnung gutschrift "
						+ this.buildWhereClause() + this.buildOrderByClause();
				Query query = session.createQuery(queryString);
		...
		if (super.getTableInfo() == null) {
			...
			setTableInfo(new TableInfo(
					...
					new String[] {
							"Id",
							" ",
							getTextRespectUISpr("lp.gutnr", mandantCNr, locUI),
					...
					new int[] { -1, 2, QueryParameters.FLR_BREITE_M, -1, -1,
					...
					new String[] {
							...
							// Vertreter nicht sortierbar, da brauchen wir noch
							// einen join

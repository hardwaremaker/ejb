...
import com.lp.server.rechnung.fastlanereader.generated.FLRVerrechnungsmodell;
 ...
 * @version 1.0
 */
public class VerrechnungsmodellHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = VerrechnungsmodellHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				FLRVerrechnungsmodell zeitmodell = (FLRVerrechnungsmodell) resultListIterator.next();
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRVerrechnungsmodell AS modell "
					+ buildWhereClause();
			Query query = session.createQuery(queryString);
	...
	private String getFromClause() {
		return "FROM FLRVerrechnungsmodell AS modell";
	}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRVerrechnungsmodell zeitmodell = (FLRVerrechnungsmodell) scrollableResult
								.get(0);

...
import com.lp.server.auftrag.fastlanereader.FLRAuftragPositionSichtAuftragNumberAdapter;
...
public class VorkasseSichtAuftragHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			PositionNumberAdapter adapter = new FLRAuftragPositionSichtAuftragNumberAdapter(
			...
			while (resultListIterator.hasNext()) {
				FLRPositionenSichtAuftrag position = (FLRPositionenSichtAuftrag) resultListIterator.next();
				...
				if (flrartikel != null) {
					einheitCNr = flrartikel.getEinheit_c_nr() == null ? "" : flrartikel.getEinheit_c_nr().trim();
					ident = flrartikel.getC_nr();
					referenznummer = flrartikel.getC_referenznr();
				}
				...
				if (position.getTyp_c_nr() == null) {
					if (position.getPositionart_c_nr().equals(LocaleFac.POSITIONSART_TEXTEINGABE)) {
						...
						sBezeichnung = "[" + getTextRespectUISpr("lieferscheinsichtauftrag.intelligentezwischensumme",
				...
				if (position.getN_nettogesamtpreis() != null) {
					...
					BigDecimal zeilensummme=position.getN_nettogesamtpreis().multiply(position.getN_menge());
					
					rows[row][col++] = zeilensummme;
					
					if (rechnungIId != null) {

						BigDecimal bdSumme=getRechnungFac().getSummeVorkasseposition(rechnungIId, position.getI_id(), theClientDto);
						
						rows[row][col++] = zeilensummme.subtract(bdSumme);
						rows[row][col++] =  Helper.getProzentsatzBD(zeilensummme,  zeilensummme.subtract(bdSumme), 2);

						VorkassepositionDto vpDto = getRechnungFac()
								.vorkassepositionFindByRechnungIIdAuftragspositionIId(rechnungIId, position.getI_id(),
										theClientDto);

						if (vpDto != null) {
							rows[row][col++] = vpDto.getNBetrag();
							rows[row][col++] =  Helper.getProzentsatzBD(zeilensummme, vpDto.getNBetrag(), 2);
			...
			System.out.println("Duration pageAt PositionSichtAuftragHandler : " + (stopTime - startTime) + " for "
					+ rows.length + " rows");
		} catch (Exception e) {
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					if (filterKriterien[i].kritName.equals("RECHNUNG_I_ID")) {
						rechnungIId = Integer.parseInt(filterKriterien[i].value);
					}
				}
			}
			if (filterAdded) {
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				for (int i = 0; i < kriterien.length; i++) {
					if (!kriterien[i].kritName.endsWith(Facade.NICHT_SORTIERBAR)) {
						if (kriterien[i].isKrit) {
							...
							sortAdded = true;
							orderBy.append(FLR_POSITIONENSICHTAUFTRAG + kriterien[i].kritName);
							orderBy.append(" ");
				...
				orderBy.append(FLR_POSITIONENSICHTAUFTRAG).append(AuftragpositionFac.FLR_AUFTRAGPOSITION_I_SORT)
						.append(" ASC "); // Sortierung wie bei den
											// Auftragpositionen
				sortAdded = true;
			}
			if (orderBy.indexOf(
					FLR_POSITIONENSICHTAUFTRAG + AuftragpositionFac.FLR_AUFTRAGPOSITIONSICHTAUFTRAG_I_ID) < 0) {
				...
				orderBy.append(" ").append(FLR_POSITIONENSICHTAUFTRAG)
						.append(AuftragpositionFac.FLR_AUFTRAGPOSITIONSICHTAUFTRAG_I_ID).append(" ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	public QueryResult sort(SortierKriterium[] sortierKriterien, Object selectedId) throws EJBExceptionLP {
		...
		if (selectedId != null && ((Integer) selectedId).intValue() >= 0) {
			...
			try {
				session = factory.openSession();
				String queryString = "select " + FLR_POSITIONENSICHTAUFTRAG
						+ AuftragpositionFac.FLR_AUFTRAGPOSITIONSICHTAUFTRAG_I_ID
						+ FLR_POSITIONENSICHTAUFTRAG_FROM_CLAUSE + this.buildWhereClause() + this.buildOrderByClause();
				Query query = session.createQuery(queryString);
	...
	private TableColumnInformation createColumnInformation(String mandant, Locale locUi) {
		...
		try {
			...
			columns.add("lp.einheit", String.class, getTextRespectUISpr("lp.einheit", mandant, locUi),
					QueryParameters.FLR_BREITE_XS, AuftragpositionFac.FLR_AUFTRAGPOSITIONSICHTAUFTRAG_FLRARTIKEL + "."
							+ ArtikelFac.FLR_ARTIKEL_EINHEIT_C_NR);
			columns.add("lp.ident", String.class, getTextRespectUISpr("lp.artikel", mandant, locUi), getUIBreiteIdent(),
					AuftragpositionFac.FLR_AUFTRAGPOSITIONSICHTAUFTRAG_FLRARTIKEL + "." + ArtikelFac.FLR_ARTIKEL_C_NR);
			...
			columns.add("rech.anzahlungsposition.offenbetrag",
					super.getUIClassBigDecimalNachkommastellen(iNachkommastellenPreis),
					getTextRespectUISpr("rech.anzahlungsposition.offenbetrag", mandant, locUi),
					QueryParameters.FLR_BREITE_M, Facade.NICHT_SORTIERBAR);

			columns.add("rech.anzahlungsposition.offenprozent",
					super.getUIClassBigDecimalNachkommastellen(2),
					getTextRespectUISpr("rech.anzahlungsposition.offenprozent", mandant, locUi),
					QueryParameters.FLR_BREITE_M, Facade.NICHT_SORTIERBAR);

			columns.add("rech.anzahlungsposition.betrag",
					super.getUIClassBigDecimalNachkommastellen(iNachkommastellenPreis),
					getTextRespectUISpr("rech.anzahlungsposition.betrag", mandant, locUi), QueryParameters.FLR_BREITE_M,
					Facade.NICHT_SORTIERBAR);

			columns.add("rech.anzahlungsposition.prozent",
					super.getUIClassBigDecimalNachkommastellen(2),
					getTextRespectUISpr("rech.anzahlungsposition.prozent", mandant, locUi),
					QueryParameters.FLR_BREITE_M, Facade.NICHT_SORTIERBAR);

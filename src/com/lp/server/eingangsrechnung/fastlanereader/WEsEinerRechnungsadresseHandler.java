...
public class WEsEinerRechnungsadresseHandler extends UseCaseHandler {
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			int pageSize = WEsEinerRechnungsadresseHandler.PAGE_SIZE;
			...
			while (resultListIterator.hasNext()) {
				...
				Integer wareneingangIId=(Integer)o[0];
				
				WareneingangDto weDto=getWareneingangFac().wareneingangFindByPrimaryKey(wareneingangIId);
			
				
				rows[row][col++] = wareneingangIId;
				rows[row][col++] =o[1];
				rows[row][col++] = o[2];
				rows[row][col++] = getWareneingangFac()
						.getWareneingangWertsumme(
								getWareneingangFac()
										.wareneingangFindByPrimaryKey(
												wareneingangIId),
								theClientDto);
				rows[row][col++] = weDto.getTLieferscheindatum();
				if (bDarfPreiseSehen) {
					BigDecimal bdTransportkosten = new BigDecimal(0);
					if (weDto.getNTransportkosten() != null) {
						bdTransportkosten = bdTransportkosten.add(weDto
								.getNTransportkosten());
					}
					if (weDto.getNBankspesen() != null) {
						bdTransportkosten = bdTransportkosten.add(weDto
								.getNBankspesen());
					}
					if (weDto.getNSonstigespesen() != null) {
						bdTransportkosten = bdTransportkosten.add(weDto
								.getNSonstigespesen());
					}
					if (weDto.getNZollkosten() != null) {
						bdTransportkosten = bdTransportkosten.add(weDto
								.getNZollkosten());
					}

					rows[row][col++] = bdTransportkosten;
					rows[row][col++] = weDto.getDGemeinkostenfaktor();
				} else {
					rows[row][col++] = null;
					rows[row][col++] = null;
				}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			session = factory.openSession();
			String queryString = "SELECT count(distinct wareneingang.i_id) FROM FLRWareneingangspositionen AS wareneingangpositionen LEFT OUTER JOIN wareneingangpositionen.flrwareneingang wareneingang "
	...
	private String getFromClause() {
		...
		return "SELECT distinct wareneingang.i_id, wareneingang.flrbestellung.c_nr,wareneingang.c_lieferscheinnr,wareneingang.t_lieferscheindatum, wareneingang.i_sort, wareneingang.n_transportkosten, wareneingang.t_wareneingansdatum, wareneingang.f_gemeinkostenfaktor FROM FLRWareneingangspositionen AS wareneingangpositionen  LEFT OUTER JOIN wareneingangpositionen.flrwareneingang wareneingang";
		...
		if (super.getTableInfo() == null) {
			try {
				int iNachkommastellenPreis = getMandantFac()
						.getNachkommastellenPreisWE(theClientDto.getMandant());
				...
				setTableInfo(new TableInfo(
						new Class[] {
								...
								java.util.Date.class, BigDecimal.class,
								...
								getTextRespectUISpr("bes.lieferscheinnr",
										theClientDto.getMandant(),
										theClientDto.getLocUi()),
								getTextRespectUISpr("er.wareneingaenge.wewert",
										theClientDto.getMandant(),
										theClientDto.getLocUi()),
								...
								getTextRespectUISpr("bes.wedatum",
										...
										theClientDto.getLocUi()),
								getTextRespectUISpr(
										"er.wareneingaenge.preiseerfasst",
										theClientDto.getMandant(),
								...
								"i_id",
								WareneingangFac.FLR_WE_FLRBESTELLUNG + ".c_nr",

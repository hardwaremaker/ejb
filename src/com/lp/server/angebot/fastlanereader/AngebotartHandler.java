...
import com.lp.server.angebot.fastlanereader.generated.FLRAngebotart;
import com.lp.server.angebot.fastlanereader.generated.FLRAngebotartspr;
 ...
 * @version 1.0
 */
public class AngebotartHandler extends UseCaseHandler {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final String FLR_ANGEBOTART = "flrangebotart.";
	public static final String FLR_ANGEBOTART_FROM_CLAUSE = " from FLRAngebotart flrangebotart ";
	...
	public QueryResult getPageAt(Integer rowIndex) throws EJBExceptionLP {
		...
		try {
			...
			while (resultListIterator.hasNext()) {
				...
				Iterator<?> sprsetIterator = angebotart
						.getAngebotart_angebotart_set().iterator();
	...
	private String findSpr(String sLocaleI, Iterator<?> iterUebersetzungenI) {
		...
		while (iterUebersetzungenI.hasNext()) {
			FLRAngebotartspr angebotartspr = (FLRAngebotartspr) iterUebersetzungenI
					.next();
			if (angebotartspr.getLocale().getC_nr().compareTo(sLocaleI) == 0) {
				sUebersetzung = angebotartspr.getC_bez();
				break;
			}
		}
	...
	protected long getRowCountFromDataBase() {
		...
		try {
			...
			String queryString = "SELECT COUNT(*) FROM FLRAngebotart AS angebotart"
					+ " LEFT JOIN angebotart.angebotart_angebotart_set AS angebotart_angebotart_set "
		...
		if (this.getQuery() != null && this.getQuery().getFilterBlock() != null
			...
			for (int i = 0; i < filterKriterien.length; i++) {
				if (filterKriterien[i].isKrit) {
					...
					filterAdded = true;
					where.append(" angebotart." + filterKriterien[i].kritName);
	...
	private String buildOrderByClause() {
		...
		if (this.getQuery() != null) {
			...
			if (kriterien != null && kriterien.length > 0) {
				...
				orderBy.append("angebotart.i_sort ASC ");
				sortAdded = true;
			}
			if (orderBy.indexOf("angebotart.i_sort") < 0) {
				...
				orderBy.append(" angebotart.i_sort ");
				sortAdded = true;
			}
			if (sortAdded) {
	...
	private String getFromClause() {
		return " FROM FLRAngebotart AS angebotart"
				+ " LEFT JOIN angebotart.angebotart_angebotart_set AS angebotart_angebotart_set";
	}
	...
	public TableInfo getTableInfo() {
		if (super.getTableInfo() == null) {
			setTableInfo(new TableInfo(new Class[] { String.class,
					...
					"angebotart.c_nr", "angebotart.c_nr", "angebotart.i_sort",
					"angebotart_angebotart_set.c_bez" }));
		}
			...
			try {
				...
				if (scrollableResult != null) {
					...
					while (scrollableResult.next()) {
						FLRAngebotart angebotart = (FLRAngebotart) scrollableResult
								.get(0);

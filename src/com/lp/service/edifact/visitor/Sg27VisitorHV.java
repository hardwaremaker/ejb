...
public class Sg27VisitorHV implements IVisitor {
	protected final ILPLogger log = LPLogService.getInstance().getLogger(Sg27VisitorHV.class); 

	private final IEjbService service;
	private final IDelforRepository repository;
	
	private String lastScc;
	
	public Sg27VisitorHV(IEjbService service, IDelforRepository repository) {
	...
	public void visit(DtmSegment dtmSegment) {
		...
		if(Helper.isOneOf(dtmInfo.getFunctionCode(), "2", "64")) {
			addPosition();			
		}
	}
	...
	public void visit(QtySegment qtySegment) {
		...
		if(qtyInfo.getAmount().signum() == 0) {
			repository.apply(new Date());
			addPosition();
		}
	}

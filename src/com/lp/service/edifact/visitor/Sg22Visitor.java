...
public class Sg22Visitor implements IVisitor {
	protected final ILPLogger log = LPLogService.getInstance().getLogger(Sg22Visitor.class); 
	...
	public void visit(DtmSegment dtmSegment) {
		log.warn("visit dtm " + dtmSegment.get().getDate().toString());
	}
	...
	public void visit(NadSegment nadSegment) {
		log.warn("visit nad " + nadSegment.get().getPartyFunctionCode() + ":" + nadSegment.get().getPartyIdentifier());				
	}
	...
	public void visit(QtySegment qtySegment) {
		log.warn("visit qty " + qtySegment.get().getQuantity());
	}

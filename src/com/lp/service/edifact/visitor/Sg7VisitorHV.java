...
public class Sg7VisitorHV implements IVisitor {
	protected final ILPLogger log = LPLogService.getInstance().getLogger(Sg7VisitorHV.class); 

	private IEjbService service;
	private IDelforRepository repository;
	
	public Sg7VisitorHV(IEjbService service, IDelforRepository repository) {
	...
	private void findKunde(NadSegment nadSegment) {
		...
		if("SE".equals(nadInfo.getPartyFunctionCode())) {
			log.warn("ignored 'SE' NAD Segment");
			return;
		}
		...
		repository.addError(new EJBLineNumberExceptionLP(0, EJBLineNumberExceptionLP.SEVERITY_ERROR, 
				new EJBExceptionLP(EJBExceptionLP.FEHLER_FORECAST_IMPORT_KUNDE_NICHT_GEFUNDEN, 
						clientInfo, null)));

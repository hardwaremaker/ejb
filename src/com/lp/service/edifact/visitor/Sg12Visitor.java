...
public class Sg12Visitor implements IVisitor {
	protected final ILPLogger log = LPLogService.getInstance().getLogger(Sg12Visitor.class); 
	...
	public void visit(DtmSegment dtmSegment) {
		log.info("visit dtm " + dtmSegment.get().getDate().toString());
	}
	...
	public void visit(LinSegment linSegment) {
		log.info("visit lin " + linSegment.get().getLineItemIdentifier() + ":" + linSegment.get().getItemIdentifier());				
	}
	...
	public void visit(RffSegment rffSegment) {
		log.info("visit rff " + rffSegment.get().getFunctionCode() + ":" + rffSegment.get().getIdentifier());
	}
